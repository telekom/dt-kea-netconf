...
typedef std::shared_ptr<asiolink::UDPEndpoint> UDPEndpointPtr;
...
private:
    ...
    std::shared_ptr<boost::asio::ip::udp::socket> asio_socket_;

    /// @brief NameChangeUDPSocket listening socket
    std::shared_ptr<NameChangeUDPSocket> socket_;

    /// @brief Pointer to the receive callback
    std::shared_ptr<UDPCallback> recv_callback_;
...
private:
    ...
    std::shared_ptr<isc::asiolink::UDPEndpoint> server_endpoint_;

    /// @brief Pointer to the send callback
    std::shared_ptr<UDPCallback> send_callback_;
