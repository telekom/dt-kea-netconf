...
uint8_t Pkt4::getType() const {
    ...
    std::shared_ptr<OptionInt<uint8_t> > type_opt =
        std::dynamic_pointer_cast<OptionInt<uint8_t> >(generic);
    if (type_opt) {
...
void Pkt4::setType(uint8_t dhcp_type) {
    ...
    if (opt) {
        ...
        // should be used.
        std::shared_ptr<OptionInt<uint8_t> > type_opt =
            std::dynamic_pointer_cast<OptionInt<uint8_t> >(opt);
        if (type_opt) {
...
Pkt4::toText() const {
    ...
    output << ", tid=0x" << hex << transid_ << dec;
