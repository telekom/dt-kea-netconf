...
#ifdef TERASTREAM
using PrivacyHash = std::vector<uint8_t>;
using PrivacyHashPtr = std::shared_ptr<PrivacyHash>;

struct UpdatePrivacyHash {
    UpdatePrivacyHash(PrivacyHashPtr privacy_hash,
                      uint64_t const zone,
                      uint64_t const router,
                      uint64_t const port,
                      uint64_t const user_port) {
        *reinterpret_cast<uint64_t*>(&((*privacy_hash)[0 * sizeof(uint64_t)])) = htobe64(zone);
        *reinterpret_cast<uint64_t*>(&((*privacy_hash)[1 * sizeof(uint64_t)])) = htobe64(router);
        *reinterpret_cast<uint64_t*>(&((*privacy_hash)[2 * sizeof(uint64_t)])) = htobe64(port);
        *reinterpret_cast<uint64_t*>(&((*privacy_hash)[3 * sizeof(uint64_t)])) = htobe64(user_port);
    }

private:
    /// @brief non-copyable
    /// @{
    UpdatePrivacyHash(UpdatePrivacyHash const&) = delete;
    UpdatePrivacyHash& operator=(UpdatePrivacyHash const&) = delete;
    /// @}
};
