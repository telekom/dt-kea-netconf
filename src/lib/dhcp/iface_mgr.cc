...
IfaceMgr::getLocalAddress(const IOAddress& remote_addr, const uint16_t port) {
    ...
    std::unique_ptr<const UDPEndpoint>
...
Pkt4Ptr IfaceMgr::receive4(uint32_t timeout_sec,
...
Pkt4Ptr IfaceMgr::receive4Indirect(uint32_t timeout_sec,
                                   uint32_t timeout_usec /* = 0 */) {
    ...
    int const result(select(maxfd + 1, &sockets, NULL, NULL, &select_timeout));
...
Pkt4Ptr IfaceMgr::receive4Direct(uint32_t timeout_sec,
                                 uint32_t timeout_usec /* = 0 */) {
    // Sanity check for microsecond timeout.
    if (timeout_usec >= 1000000) {
        isc_throw(BadValue, "fractional timeout must be shorter than"
    ...
    std::unique_ptr<SocketInfo> candidate;
    fd_set sockets;
    int maxfd = 0;
...
Pkt6Ptr IfaceMgr::receive6(uint32_t timeout_sec,
                           uint32_t timeout_usec /* = 0 */) {
    if (isDHCPReceiverRunning()) {
...
Pkt6Ptr
IfaceMgr::receive6Direct(uint32_t timeout_sec,
                         uint32_t timeout_usec /* = 0 */) {
    // Sanity check for microsecond timeout.
    if (timeout_usec >= 1000000) {
        isc_throw(BadValue, "fractional timeout must be shorter than"
...
Pkt6Ptr
IfaceMgr::receive6Indirect(uint32_t timeout_sec,
                           uint32_t timeout_usec /* = 0 */) {
    // Sanity check for microsecond timeout.
    if (timeout_usec >= 1000000) {
        isc_throw(BadValue, "fractional timeout must be shorter than"
...
IfaceMgr::receiveDHCP4Packets() {
    ...
    for (;;) {
        ...
        int const result(select(maxfd + 1, &rd_set, 0, 0, 0));
...
bool
IfaceMgr::configureDHCPPacketQueue(uint16_t family, data::ElementPtr queue_control) {
    if (isDHCPReceiverRunning()) {
