...
public:
    ...
    typedef std::shared_ptr<PktType> PktTypePtr;
...
#ifdef TERASTREAM
    /// @brief Sets privacy hash.
    ///
    /// @param privacy hash.
    void setPrivacyHash(PrivacyHashPtr hash) {
        privacy_hash_.reset(new PrivacyHash(*hash));
    };

    /// @brief Returns privacy hash.
    ///
    /// @return privacy hash
    PrivacyHashPtr getPrivacyHash() const {
        return (privacy_hash_);
    };
#endif  // TERASTREAM
...
typedef std::shared_ptr<isc::dhcp::Pkt> PktPtr;
