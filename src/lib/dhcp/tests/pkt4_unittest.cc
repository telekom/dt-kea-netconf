...
public:
    ...
    Pkt4Ptr generateTestPacket1() {
        ...
        std::shared_ptr<Pkt4> pkt(new Pkt4(DHCPDISCOVER, dummyTransid));
    ...
    void verifyParsedOptions(const Pkt4Ptr& pkt) {
        ...
        std::shared_ptr<Option> x = pkt->getOption(12);
        ASSERT_TRUE(x); // option 1 should exist
        OptionStringPtr option12 = std::static_pointer_cast<OptionString>(x);
        ...
        // the appropriate conversion.
        OptionStringPtr option14 = std::static_pointer_cast<OptionString>(x);
...
TEST_F(Pkt4Test, fixedFields) {
    ...
    std::shared_ptr<Pkt4> pkt = generateTestPacket1();
...
TEST_F(Pkt4Test, fixedFieldsUnpack) {
    ...
    std::shared_ptr<Pkt4> pkt(new Pkt4(&expectedFormat[0],
                                         expectedFormat.size()));
...
TEST_F(Pkt4Test, options) {
    unique_ptr<Pkt4> pkt(new Pkt4(DHCPOFFER, 0));
    ...
    std::shared_ptr<Option> opt1(new Option(Option::V4, 12, payload[0]));
    std::shared_ptr<Option> opt3(new Option(Option::V4, 14, payload[1]));
    std::shared_ptr<Option> opt2(new Option(Option::V4, 60, payload[2]));
    std::shared_ptr<Option> opt5(new Option(Option::V4,128, payload[3]));
    std::shared_ptr<Option> opt4(new Option(Option::V4,254, payload[4]));
...
TEST_F(Pkt4Test, metaFields) {
    ...
    unique_ptr<Pkt4> pkt(new Pkt4(DHCPOFFER, 1234));
...
TEST_F(Pkt4Test, toText) {
    ...
    EXPECT_EQ("local_address=192.0.2.34:67, remote_address=192.10.33.4:68, "
              "msg_type=DHCPDISCOVER (1), transid=0x9ef, tid=0x9ef,\n"
              "options:\n"
    ...
    EXPECT_EQ("local_address=192.0.2.34:67, remote_address=192.10.33.4:68, "
              "msg_type=(missing), transid=0x9ef, tid=0x9ef, "
...
TEST_F(Pkt4Test, nullTerminatedOptions) {
    ...
    std::shared_ptr<Pkt4> pkt(new Pkt4(&test_msg[0], test_msg.size()));
