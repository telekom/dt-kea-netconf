...
void testCopyAssign(const OpType& op_type,
                    std::shared_ptr<OptionType>& option,
                    std::shared_ptr<OptionType>& option_copy) {
    ...
    case COPY:
        ...
        break;
    case CLONE:
        option_copy = std::dynamic_pointer_cast<OptionType>(option->clone());
...
void testOptionAddrLst(const OpType& op_type,
    ...
    typedef std::shared_ptr<OptionType> OptionTypePtr;
...
void testOptionClientFqdn(const OpType& op_type,
    ...
    Option4ClientFqdnPtr option4 =
        std::dynamic_pointer_cast<Option4ClientFqdn>(option);
    if (option4) {
        option4->setRcode(64);
    }
    ...
    Option4ClientFqdnPtr option_copy4 =
        std::dynamic_pointer_cast<Option4ClientFqdn>(option_copy);
    if (option_copy4) {
