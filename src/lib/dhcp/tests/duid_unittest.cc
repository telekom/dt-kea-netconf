...
TEST(DuidTest, constructor) {
    ...
    unique_ptr<DUID> duid1(new DUID(data1, sizeof(data1)));
    unique_ptr<DUID> duid2(new DUID(data2));
...
TEST(DuidTest, size) {
    ...
    unique_ptr<DUID> duidmaxsize1(new DUID(data, MAX_DUID_LEN));
    unique_ptr<DUID> duidmaxsize2(new DUID(data2));

    EXPECT_THROW(
        unique_ptr<DUID> toolarge1(new DUID(data, MAX_DUID_LEN + 1)),
        BadValue);
    ...
    EXPECT_THROW(
        unique_ptr<DUID> toolarge2(new DUID(data2)),
        BadValue);

    // empty duids are not allowed
    vector<uint8_t> empty;
    EXPECT_THROW(
        unique_ptr<DUID> emptyDuid(new DUID(empty)),
        BadValue);

    EXPECT_THROW(
        unique_ptr<DUID> emptyDuid2(new DUID(data, 0)),
        BadValue);
}
...
TEST(DuidTest, getType) {
    ...
    unique_ptr<DUID> duid_llt(new DUID(llt, sizeof(llt)));
    unique_ptr<DUID> duid_en(new DUID(en, sizeof(en)));
    unique_ptr<DUID> duid_ll(new DUID(ll, sizeof(ll)));
    unique_ptr<DUID> duid_uuid(new DUID(uuid, sizeof(uuid)));
    unique_ptr<DUID> duid_invalid(new DUID(invalid, sizeof(invalid)));
...
TEST(DuidTest, operators) {
    ...
    unique_ptr<DUID> duid1(new DUID(data1, sizeof(data1)));
    unique_ptr<DUID> duid2(new DUID(data2, sizeof(data2)));
    unique_ptr<DUID> duid3(new DUID(data3, sizeof(data3)));
    unique_ptr<DUID> duid4(new DUID(data4, sizeof(data4)));
...
TEST(ClientIdTest, constructor) {
    ...
    unique_ptr<ClientId> id1(new ClientId(data1, sizeof(data1)));
    vector<uint8_t> vecdata = id1->getClientId();
    EXPECT_TRUE(data2 == vecdata);

    // checks for vector-based constructor
    unique_ptr<ClientId> id2(new ClientId(data2));
...
TEST(ClientIdTest, size) {
    ...
    unique_ptr<ClientId> duidmaxsize1(new ClientId(data, MAX_CLIENT_ID_LEN));
    unique_ptr<ClientId> duidmaxsize2(new ClientId(data2));

    EXPECT_THROW(
        unique_ptr<ClientId> toolarge1(new ClientId(data, MAX_CLIENT_ID_LEN + 1)),
        BadValue);
    ...
    EXPECT_THROW(
        unique_ptr<ClientId> toolarge2(new ClientId(data2)),
        BadValue);

    // empty client-ids are not allowed
    vector<uint8_t> empty;
    EXPECT_THROW(
        unique_ptr<ClientId> empty_client_id1(new ClientId(empty)),
        BadValue);

    EXPECT_THROW(
        unique_ptr<ClientId> empty_client_id2(new ClientId(data, 0)),
        BadValue);

    // client-id must be at least 2 bytes long
    vector<uint8_t> shorty(1,17); // just a single byte with value 17
    EXPECT_THROW(
        unique_ptr<ClientId> too_short_client_id1(new ClientId(shorty)),
        BadValue);
    EXPECT_THROW(
        unique_ptr<ClientId> too_short_client_id1(new ClientId(data, 1)),
        BadValue);
}
...
TEST(ClientIdTest, operators) {
    ...
    unique_ptr<ClientId> id1(new ClientId(data1, sizeof(data1)));
    unique_ptr<ClientId> id2(new ClientId(data2, sizeof(data2)));
    unique_ptr<ClientId> id3(new ClientId(data3, sizeof(data3)));
    unique_ptr<ClientId> id4(new ClientId(data4, sizeof(data4)));
