...
typedef std::shared_ptr<NakedPkt6> NakedPkt6Ptr;
...
TEST_F(Pkt6Test, constructor) {
    ...
    unique_ptr<Pkt6> pkt1(new Pkt6(data, sizeof(data)));
...
Pkt6Ptr capture2() {
    ...
    pkt->setIndex(2);
    pkt->setIface("eth0");
    return (std::dynamic_pointer_cast<Pkt6>(pkt));
}
...
TEST_F(Pkt6Test, addGetDelOptions) {
    unique_ptr<Pkt6> parent(new Pkt6(DHCPV6_SOLICIT, random()));
...
TEST_F(Pkt6Test, Timestamp) {
    std::unique_ptr<Pkt6> pkt(new Pkt6(DHCPV6_SOLICIT, 0x020304));
...
TEST_F(Pkt6Test, relayUnpack) {
    ...
    std::shared_ptr<OptionCustom> custom = std::dynamic_pointer_cast<OptionCustom>(opt);
    ...
    EXPECT_EQ(8, opt->len());
    custom = std::dynamic_pointer_cast<OptionCustom>(opt);
    ...
    std::shared_ptr<Option6IA> ia =
        std::dynamic_pointer_cast<Option6IA>(opt);
    ASSERT_TRUE(ia);
    ...
    std::shared_ptr<OptionInt<uint16_t> > elapsed =
        std::dynamic_pointer_cast<OptionInt<uint16_t> > (opt);
    ASSERT_TRUE(elapsed);
    EXPECT_EQ(0, elapsed->getValue());

    ASSERT_TRUE(opt = msg->getOption(D6O_ORO));
    std::shared_ptr<OptionIntArray<uint16_t> > oro =
        std::dynamic_pointer_cast<OptionIntArray<uint16_t> > (opt);
...
TEST_F(Pkt6Test, relayPack) {
    ...
    unique_ptr<Pkt6> parent(new Pkt6(DHCPV6_ADVERTISE, 0x020304));
    ...
    // Create second packet,based on assembled data from the first one
    unique_ptr<Pkt6> clone(new Pkt6(static_cast<const uint8_t*>(
                                    parent->getBuffer().getData()),
                                    parent->getBuffer().getLength()));
...
TEST_F(Pkt6Test, getRelayOption) {
    NakedPkt6Ptr msg(std::dynamic_pointer_cast<NakedPkt6>(capture2()));
    ASSERT_TRUE(msg);
...
TEST_F(Pkt6Test, getAnyRelayOption) {
    ...
    std::unique_ptr<NakedPkt6> msg(new NakedPkt6(DHCPV6_ADVERTISE, 0x020304));
...
TEST_F(Pkt6Test, toText) {
    ...
    string expected =
        ...
        "msgtype=1(SOLICIT), transid=0x6b4fe2, tid=0x6b4fe2\n"
...
TEST_F(Pkt6Test, getMAC_DOCSIS_Modem) {
    ...
    OptionVendorPtr vendor = std::dynamic_pointer_cast<
...
TEST_F(Pkt6Test, truncatedVendorLength) {
    ...
    OptionVendorPtr vivso = std::dynamic_pointer_cast<OptionVendor>(x);
    ASSERT_TRUE(vivso);
...
TEST_F(Pkt6Test, testSkipThisOptionError) {
    ...
    ASSERT_TRUE(opstr = std::dynamic_pointer_cast<OptionString>(opt));
...
TEST_F(Pkt6Test, lqQueryOption) {
    ...
    ASSERT_TRUE(opt);
    OptionCustomPtr clone_query = std::dynamic_pointer_cast<OptionCustom>(opt);
    ...
    ASSERT_TRUE(opt);
    Option6IAAddrPtr clone_iaaddr = std::dynamic_pointer_cast<Option6IAAddr>(opt);
    ...
    ASSERT_TRUE(opt);
    OptionUint16ArrayPtr clone_oro = std::dynamic_pointer_cast<OptionUint16Array>(opt);
...
TEST_F(Pkt6Test, clientDataOption) {
    ...
    ASSERT_TRUE(opt);
    OptionCustomPtr clone_cd_option = std::dynamic_pointer_cast<OptionCustom>(opt);
    ...
    opt = clone_cd_option->getOption(D6O_IAADDR);
    ASSERT_TRUE(opt);
    clone_iaaddr = std::dynamic_pointer_cast<Option6IAAddr>(opt);
    ASSERT_TRUE(clone_iaaddr);
    ...
    ASSERT_TRUE(opt);
    Option6IAPrefixPtr clone_iaprefix = std::dynamic_pointer_cast<Option6IAPrefix>(opt);
    ...
    opt = clone_cd_option->getOption(D6O_IAPREFIX);
    ASSERT_TRUE(opt);
    clone_iaprefix = std::dynamic_pointer_cast<Option6IAPrefix>(opt);
    ASSERT_TRUE(clone_iaprefix);
    ...
    ASSERT_TRUE(opt);
    OptionUint32Ptr clone_cltt = std::dynamic_pointer_cast<OptionUint32>(opt);
...
TEST_F(Pkt6Test, relayDataOption) {
    ...
    ASSERT_TRUE(opt);
    OptionCustomPtr clone_rd_option = std::dynamic_pointer_cast<OptionCustom>(opt);
