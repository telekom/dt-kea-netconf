...
public:
    ...
    void basicTest8(const Option::Universe u) {
        ...
        ASSERT_NO_THROW(
            opt = std::shared_ptr<OptionInt<T> >(new OptionInt<T>(u,
                                                                    TEST_OPT_CODE,
                                                                    buf_.begin(),
                                                                    buf_.begin() + 1))
        );
...
TEST_F(OptionIntTest, useInvalidType) {
    EXPECT_THROW(
        std::unique_ptr<OptionInt<bool> >(new OptionInt<bool>(Option::V6,
                                                                D6O_ELAPSED_TIME, 10)),
        InvalidDataType
    );

    EXPECT_THROW(
        std::unique_ptr<OptionInt<int64_t> >(new OptionInt<int64_t>(Option::V6,
                                                                      D6O_ELAPSED_TIME, 10)),
        InvalidDataType
    );
...
TEST_F(OptionIntTest, setValueUint8) {
    std::shared_ptr<OptionInt<uint8_t> > opt(new OptionInt<uint8_t>(Option::V6,
                                                                      D6O_PREFERENCE, 123));
...
TEST_F(OptionIntTest, setValueInt8) {
    std::shared_ptr<OptionInt<int8_t> > opt(new OptionInt<int8_t>(Option::V6,
                                                                    D6O_PREFERENCE, -123));
    // Check if constructor initialized the option value correctly.
    EXPECT_EQ(-123, opt->getValue());
    // Override the value.
...
TEST_F(OptionIntTest, setValueUint16) {
    std::shared_ptr<OptionInt<uint16_t> > opt(new OptionInt<uint16_t>(Option::V6,
...
TEST_F(OptionIntTest, setValueInt16) {
    std::shared_ptr<OptionInt<int16_t> > opt(new OptionInt<int16_t>(Option::V6,
...
TEST_F(OptionIntTest, setValueUint32) {
    std::shared_ptr<OptionInt<uint32_t> > opt(new OptionInt<uint32_t>(Option::V6,
                                                                        D6O_CLT_TIME, 123));
    // Check if constructor initialized the option value correctly.
    EXPECT_EQ(123, opt->getValue());
    // Override the value.
...
TEST_F(OptionIntTest, setValueInt32) {
    std::shared_ptr<OptionInt<int32_t> > opt(new OptionInt<int32_t>(Option::V6,
                                                                      D6O_CLT_TIME, -120100));
    // Check if constructor initialized the option value correctly.
...
TEST_F(OptionIntTest, packSuboptions4) {
    std::shared_ptr<OptionInt<uint16_t> > opt(new OptionInt<uint16_t>(Option::V4,
                                                                        TEST_OPT_CODE,
                                                                        0x0102));
...
TEST_F(OptionIntTest, packSuboptions6) {
    ...
    std::shared_ptr<Option6IAAddr> addr1(
...
TEST_F(OptionIntTest, unpackSuboptions6) {
    ...
    std::shared_ptr<OptionInt<uint16_t> > opt;
    EXPECT_NO_THROW(
        opt = std::shared_ptr<
    ...
    ASSERT_TRUE(subopt);
    std::shared_ptr<Option6IAAddr> addr(std::dynamic_pointer_cast<Option6IAAddr>(subopt));
    ASSERT_TRUE(addr);
