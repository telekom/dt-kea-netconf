...
TEST_F(LibDhcpTest, packOptions6) {
    ...
    std::shared_ptr<OptionInt<uint32_t> >
...
TEST_F(LibDhcpTest, unpackOptions6) {
    ...
    std::shared_ptr<OptionIntArray<uint16_t> > opt_oro =
        std::dynamic_pointer_cast<OptionIntArray<uint16_t> >(x->second);
    ...
    // uint16_t value.
    std::shared_ptr<OptionInt<uint16_t> > opt_elapsed_time =
        std::dynamic_pointer_cast<OptionInt<uint16_t> >(x->second);
    // This value will be NULL if cast was unsuccessful. This is the case
    // when returned option has different type than expected.
...
TEST_F(LibDhcpTest, unpackSubOptions6) {
    ...
    ASSERT_EQ(1, options.size());
    std::shared_ptr<OptionInt<uint32_t> > option_foobar =
        std::dynamic_pointer_cast<OptionInt<uint32_t> >(options.begin()->
                                                          second);
    ...
    std::shared_ptr<OptionInt<uint16_t> > option_foo =
        std::dynamic_pointer_cast<OptionInt<uint16_t> >(option_foobar->
                                                          getOption(1));
    ASSERT_TRUE(option_foo);
    EXPECT_EQ(1, option_foo->getType());
    EXPECT_EQ(0x0102, option_foo->getValue());
    // Finally, there should be a low level option under option_foo.
    std::shared_ptr<OptionInt<uint8_t> > option_bar =
        std::dynamic_pointer_cast<OptionInt<uint8_t> >(option_foo->getOption(1));
...
TEST_F(LibDhcpTest, unpackOptions4) {
    ...
    OptionStringPtr option12 = std::static_pointer_cast<OptionString>(x->second);
    ...
    OptionStringPtr option14 = std::static_pointer_cast<OptionString>(x->second);
    ...
    OptionVendorPtr vivsi = std::dynamic_pointer_cast<OptionVendor>(x->second);
    ASSERT_TRUE(vivsi);
    ...
    Option4AddrLstPtr tftp =
        std::dynamic_pointer_cast<Option4AddrLst>(suboptions.begin()->second);
    ASSERT_TRUE(tftp);
    ...
    OptionCustomPtr rai = std::dynamic_pointer_cast<OptionCustom>(x->second);
    ASSERT_TRUE(rai);
...
TEST_F(LibDhcpTest, unpackPadEnd) {
    ...
    // Get suboption 0.
    std::shared_ptr<OptionInt<uint8_t> > sub0 =
        std::dynamic_pointer_cast<OptionInt<uint8_t> >
            (option->getOption(0));
    ASSERT_TRUE(sub0);
    ...
    // Get suboption 255.
    std::shared_ptr<OptionInt<uint8_t> > sub255 =
        std::dynamic_pointer_cast<OptionInt<uint8_t> >
    ...
    // Get suboption 1.
    std::shared_ptr<OptionString> sub =
        std::dynamic_pointer_cast<OptionString>(option->getOption(1));
    ASSERT_TRUE(sub);
...
TEST_F(LibDhcpTest, option43Pad) {
    ...
    // Get suboption 2.
    std::shared_ptr<OptionInt<uint8_t> > sub2 =
        std::dynamic_pointer_cast<OptionInt<uint8_t> >
...
TEST_F(LibDhcpTest, fqdnList) {
    ...
    ASSERT_NO_THROW(option = def->optionFactory(Option::V4,
                                                ...
                                                fqdn_buf.end()));
    ASSERT_TRUE(option);
    OptionCustomPtr names = std::dynamic_pointer_cast<OptionCustom>(option);
    ASSERT_TRUE(names);
...
TEST_F(LibDhcpTest, vendorClass6) {
    ...
    std::shared_ptr<OptionVendorClass> vclass =
        std::dynamic_pointer_cast<OptionVendorClass>(options.begin()->second);
    ASSERT_TRUE(vclass);
