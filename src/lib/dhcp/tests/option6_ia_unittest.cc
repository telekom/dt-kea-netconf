...
TEST_F(Option6IATest, simple) {
    unique_ptr<Option6IA> ia(new Option6IA(D6O_IA_NA, 1234));
...
TEST_F(Option6IATest, suboptionsPack) {
    ...
    unique_ptr<Option6IA> ia(new Option6IA(D6O_IA_NA, 0x13579ace));
    ia->setT1(0x2345);
    ia->setT2(0x3456);
...
TEST_F(Option6IATest, pdSuboptionsPack) {
    ...
    std::shared_ptr<Option6IAPrefix> addr1(
...
TEST_F(Option6IATest, suboptionsUnpack) {
    ...
    Option6IAAddrPtr addr =
        std::dynamic_pointer_cast<Option6IAAddr>(subopt);
    ASSERT_TRUE(addr);
