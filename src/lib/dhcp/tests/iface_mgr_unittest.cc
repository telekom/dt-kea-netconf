...
public:
    ...
    /// to be enabled.
    void sendReceive6Test(data::ElementPtr dhcp_queue_control, bool exp_queue_enabled) {
        unique_ptr<NakedIfaceMgr> ifacemgr(new NakedIfaceMgr());
    ...
    /// @param exp_queue_enabled flag that indicates if packet queuing is expected
    /// to be enabled.
    void sendReceive4Test(data::ElementPtr dhcp_queue_control, bool exp_queue_enabled) {
        unique_ptr<NakedIfaceMgr> ifacemgr(new NakedIfaceMgr());
        ...
        std::shared_ptr<Pkt4> sendPkt(new Pkt4(DHCPDISCOVER, 1234) );
    ...
    void purgeExternalSockets4Test(bool use_queue = false) {
        ...
        if (use_queue) {
            bool queue_enabled = false;
            data::ElementPtr config = makeQueueConfig(PacketQueueMgr4::DEFAULT_QUEUE_TYPE4, 500);
    ...
    void purgeExternalSockets6Test(bool use_queue = false) {
        ...
        if (use_queue) {
            bool queue_enabled = false;
            data::ElementPtr config = makeQueueConfig(PacketQueueMgr6::DEFAULT_QUEUE_TYPE6, 500);
...
TEST_F(IfaceMgrTest, dhcp6Sniffer) {
    ...
    std::unique_ptr<NakedIfaceMgr> ifacemgr = new NakedIfaceMgr();
...
TEST_F(IfaceMgrTest, clearIfaces) {
    ...
    std::shared_ptr<TestPktFilter> custom_packet_filter(new TestPktFilter());
...
TEST_F(IfaceMgrTest, packetQueue4) {
    ...
    data::ElementPtr config = makeQueueConfig(PacketQueueMgr4::DEFAULT_QUEUE_TYPE4, 2000);
...
TEST_F(IfaceMgrTest, packetQueue6) {
    ...
    // Verify that we can create a queue with default factory.
    data::ElementPtr config = makeQueueConfig(PacketQueueMgr6::DEFAULT_QUEUE_TYPE6, 2000);
...
TEST_F(IfaceMgrTest, setPacketFilter) {
    ...
    std::unique_ptr<NakedIfaceMgr> iface_mgr(new NakedIfaceMgr());
    ASSERT_TRUE(iface_mgr);

    // Try to set NULL packet filter object and make sure it is rejected.
    std::shared_ptr<TestPktFilter> custom_packet_filter;
...
TEST_F(IfaceMgrTest, setPacketFilter6) {
    ...
    // Try to set NULL packet filter object and make sure it is rejected.
    std::shared_ptr<PktFilter6Stub> custom_packet_filter;
    EXPECT_THROW(iface_mgr->setPacketFilter(custom_packet_filter),
                 isc::dhcp::InvalidPacketFilter);
...
TEST_F(IfaceMgrTest, checkPacketFilterRawSocket) {
    ...
    std::unique_ptr<NakedIfaceMgr> iface_mgr1(new NakedIfaceMgr());
    ASSERT_TRUE(iface_mgr1);
    std::unique_ptr<NakedIfaceMgr> iface_mgr2(new NakedIfaceMgr());
...
TEST_F(IfaceMgrTest, openSockets6LinkLocal) {
    ...
    std::shared_ptr<PktFilter6Stub> filter(new PktFilter6Stub());
    ASSERT_TRUE(filter);
