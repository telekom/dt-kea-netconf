...
typedef std::shared_ptr<ParkingLotHandle> ParkingLotHandlePtr;
...
public:
    ...
    ParkingLotPtr getParkingLotPtr(const int hook_index) {
        ...
        if (parking_lots_.count(hook_index) == 0) {
            parking_lots_[hook_index] = std::make_shared<ParkingLot>();
        }
...
typedef std::shared_ptr<ParkingLots> ParkingLotsPtr;
