...
class HooksManagerTest : public ::testing::Test, public HooksCommonTestClass {
public:
    /// @brief Constructor
    ///
    ...
    void executeCallCallouts(int r0, int d1, int r1, int d2, int r2, int d3, int r3) {
        ...
        HooksManager::callCallouts(isc::hooks::ServerHooks::CONTEXT_CREATE, *handle);
...
TEST_F(HooksManagerTest, LoadLibraries) {
    ...
    library_names.push_back({std::string(FULL_CALLOUT_LIBRARY), "parameters", data::ElementPtr()});
    library_names.push_back({std::string(BASIC_CALLOUT_LIBRARY), "parameters", data::ElementPtr()});
...
TEST_F(HooksManagerTest, LoadLibrariesWithError) {
    ...
    library_names.push_back(
        {std::string(INCORRECT_VERSION_LIBRARY), "parameters", data::ElementPtr()});
    library_names.push_back({std::string(BASIC_CALLOUT_LIBRARY), "parameters", data::ElementPtr()});
...
TEST_F(HooksManagerTest, CalloutHandleLoadLibrary) {
    ...
    new_library_names.push_back(
        {std::string(BASIC_CALLOUT_LIBRARY), "parameters", data::ElementPtr()});
...
int testPreCallout(CalloutHandle& handle) {
    handle.setArgument("result", static_cast<int>(1027));
    return (0);
}

int testPostCallout(CalloutHandle& handle) {
    int result;
    handle.getArgument("result", result);
    result *= 2;
    handle.setArgument("result", result);
...
TEST_F(HooksManagerTest, PrePostCalloutTest) {
    ...
    HooksManager::preCalloutsLibraryHandle().registerCallout("hookpt_two", testPreCallout);
...
TEST_F(HooksManagerTest, prepareUnload) {
    ...
    HookLibsCollection library_names;
    library_names.push_back(
        {std::string(UNLOAD_CALLOUT_LIBRARY), "parameters", data::ElementPtr()});
    EXPECT_TRUE(HooksManager::loadLibraries(library_names));
...
TEST_F(HooksManagerTest, LibraryParameters) {
    ...
    HookLibsCollection library_names;
    library_names.push_back({std::string(BASIC_CALLOUT_LIBRARY), "parameters", data::ElementPtr()});
    library_names.push_back({std::string(CALLOUT_PARAMS_LIBRARY), "parameters", params});
...
TEST_F(HooksManagerTest, Parking) {
    ...
    HookLibsCollection library_names;
    library_names.push_back({std::string(ASYNC_CALLOUT_LIBRARY), "parameters", data::ElementPtr()});
    library_names.push_back({std::string(ASYNC_CALLOUT_LIBRARY), "parameters", data::ElementPtr()});
    ...
    // got unparked.
    ASSERT_NO_THROW(
        HooksManager::park<std::string>("hookpt_one", "foo", [&unparked] { unparked = true; }));
