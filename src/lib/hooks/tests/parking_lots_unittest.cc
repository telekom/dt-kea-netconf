...
TEST(ParkingLotTest, unpark) {
    ParkingLotPtr parking_lot = std::make_shared<ParkingLot>();
    ParkingLotHandlePtr parking_lot_handle =
        std::make_shared<ParkingLotHandle>(parking_lot);
...
TEST(ParkingLotTest, clear) {
    ParkingLotsPtr parking_lots = std::make_shared<ParkingLots>();
    ParkingLotPtr parking_lot = parking_lots->getParkingLotPtr(1234);
    ASSERT_TRUE(parking_lot);
    ParkingLotHandlePtr parking_lot_handle =
        std::make_shared<ParkingLotHandle>(parking_lot);

    std::shared_ptr<std::string> parked_object =
        std::make_shared<std::string>("foo");
    std::weak_ptr<std::string> weak_parked_object(parked_object);
