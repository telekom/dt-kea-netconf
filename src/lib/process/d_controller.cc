...
DControllerBase::launch(int argc, char* argv[], const bool test_mode) {
    ...
    int rcode;
    ElementPtr comment = parseAnswer(rcode, configFromFile());
    if (rcode != 0) {
        LOG_FATAL(dctl_logger, DCTL_CONFIG_FILE_LOAD_FAIL)
...
DControllerBase::checkConfigOnly() {
    try {
        ...
        ElementPtr whole_config = parseFile(config_file);
        if (!whole_config) {
        ...
        ElementPtr answer = checkConfig(module_config);
        int rcode = 0;
...
DControllerBase::configFromFile() {
    ...
    try {
        ...
        ElementPtr error = createAnswer(COMMAND_ERROR,
...
ElementPtr
DControllerBase::updateConfig(ElementPtr new_config) {
    return (process_->configure(new_config, false));
}

// Instance method for checking new config
ElementPtr
DControllerBase::checkConfig(ElementPtr new_config) {
...
std::string
DControllerBase::handleOtherObjects(ElementPtr args) {
...
ElementPtr
DControllerBase::configTestHandler(const std::string&, ElementPtr args) {
...
ElementPtr
DControllerBase::configReloadHandler(const std::string&, ElementPtr) {
    // Add reload in message?
    return (configFromFile());
}

ElementPtr
DControllerBase::configSetHandler(const std::string&, ElementPtr args) {
    const int status_code = COMMAND_ERROR; // 1 indicates an error
    ElementPtr module_config;
    std::string app_name = getAppName();
    std::string message;
    ...
    try {
        ...
        ElementPtr answer = updateConfig(module_config);
        int rcode = 0;
...
ElementPtr
DControllerBase::serverTagGetHandler(const std::string&, ElementPtr) {
...
ElementPtr
DControllerBase::statusGetHandler(const std::string&, ElementPtr) {
...
ElementPtr
DControllerBase::versionGetHandler(const std::string&, ElementPtr) {
    ElementPtr answer;
...
ElementPtr
DControllerBase::buildReportHandler(const std::string&, ElementPtr) {
    return (createAnswer(COMMAND_SUCCESS, isc::detail::getConfigReport()));
}

ElementPtr
DControllerBase::shutdownHandler(const std::string&, ElementPtr args) {
...
ElementPtr
DControllerBase::shutdownProcess(ElementPtr args) {
    if (process_) {
