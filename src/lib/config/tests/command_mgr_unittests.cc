...
public:
    ...
    static ElementPtr my_handler(const std::string& name,
    ...
    static ElementPtr handler_params_;
...
ElementPtr CommandMgrTest::handler_params_;
...
TEST_F(CommandMgrTest, listCommandsEmpty) {
    ...
    ElementPtr command = createCommand("list-commands");
...
TEST_F(CommandMgrTest, bogusCommand) {
    ...
    ElementPtr command = createCommand("no-such-command");
...
TEST_F(CommandMgrTest, listCommands) {
    ...
    ElementPtr list_all = createCommand("list-commands");
    ElementPtr answer;
...
TEST_F(CommandMgrTest, processCommand) {
    ...
    ElementPtr command = createCommand("my-command", my_params);
    ElementPtr answer;
    EXPECT_NO_THROW(answer = CommandMgr::instance().processCommand(command));
...
TEST_F(CommandMgrTest, unixCreate) {
    ...
    EXPECT_THROW(CommandMgr::instance().openCommandSocket(ElementPtr()),
...
TEST_F(CommandMgrTest, commandProcessedHookReplaceResponse) {
    ...
    // specified parameter.
    ElementPtr my_params = Element::fromJSON("[ \"just\", \"some\", \"data\" ]");
    ElementPtr command = createCommand("change-response", my_params);
    ElementPtr answer;
    ASSERT_NO_THROW(answer = CommandMgr::instance().processCommand(command));
