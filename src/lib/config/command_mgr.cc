...
public:
    ...
    Connection(const IOServicePtr& io_service,
               const std::shared_ptr<UnixDomainSocket>& socket,
               ConnectionPool& connection_pool,
               const long timeout)
        : socket_(socket), timeout_timer_(*io_service), timeout_(timeout), buf_(), response_(),
          connection_pool_(connection_pool), feed_(), response_in_progress_(false),
    ...
    void stop() {
        if (!response_in_progress_) {
            ...
            if (!watch_socket_->closeSocket(watch_error)) {
                LOG_ERROR(command_logger, COMMAND_WATCH_SOCKET_CLOSE_ERROR).arg(watch_error);
            }
    ...
    void doSend() {
        ...
        try {
            ...
            LOG_ERROR(command_logger, COMMAND_WATCH_SOCKET_MARK_READY_ERROR).arg(ex.what());
        }
    }
...
private:
    ...
    std::shared_ptr<UnixDomainSocket> socket_;
...
public:
    ...
    void stop(const ConnectionPtr& connection) {
        try {
            ...
            LOG_ERROR(command_logger, COMMAND_SOCKET_CONNECTION_CLOSE_FAIL).arg(ex.what());
        }
    }
...
void Connection::terminate() {
    try {
        ...
        LOG_ERROR(command_logger, COMMAND_SOCKET_CONNECTION_SHUTDOWN_FAIL).arg(ex.what());
    }
}

void Connection::receiveHandler(const boost::system::error_code& ec, size_t bytes_transferred) {
    if (ec) {
        if (ec.value() == boost::asio::error::eof) {
            ...
            if (feed_.getProcessedText().empty()) {
                ...
                os << "discarding partial command of " << feed_.getProcessedText().size()
                   << " bytes";
            }
    ...
    try {
        ...
        if (feed_.feedOk()) {
            ElementPtr cmd = feed_.toElement();
    ...
    if (rsp) {
        ...
        LOG_WARN(command_logger, COMMAND_RESPONSE_ERROR)
            .arg("internal server error: no response generated");
    }
...
void Connection::sendHandler(const boost::system::error_code& ec, size_t bytes_transferred) {
    ...
    try {
        ...
        LOG_ERROR(command_logger, COMMAND_WATCH_SOCKET_CLEAR_ERROR).arg(ex.what());
    }
...
void Connection::timeoutHandler() {
    LOG_INFO(command_logger, COMMAND_SOCKET_CONNECTION_TIMEOUT).arg(socket_->getNative());
    ...
    if (!feed_.getProcessedText().empty()) {
        os << ", discarded partial command of " << feed_.getProcessedText().size() << " bytes";
    }

    ElementPtr rsp = createAnswer(CONTROL_RESULT_ERROR, os.str());
    response_ = rsp->str();
    doSend();
}
...
public:
    /// @brief Constructor.
    CommandMgrImpl()
        : io_service_(), acceptor_(), socket_(), socket_name_(), connection_pool_(),
          timeout_(TIMEOUT_DHCP_SERVER_RECEIVE_COMMAND) {
    }
    ...
    void openCommandSocket(const isc::data::ElementPtr& socket_info);
    ...
    std::shared_ptr<UnixDomainSocketAcceptor> acceptor_;
...
void CommandMgrImpl::openCommandSocket(const isc::data::ElementPtr& socket_info) {
    socket_name_.clear();
    ...
    ElementPtr type = socket_info->get("socket-type");
    if (!type) {
    ...
    if (type->stringValue() != "unix") {
        isc_throw(BadSocketInfo, "Invalid 'socket-type' parameter value " << type->stringValue());
    }

    // UNIX socket is requested. It takes one parameter: socket-name that
    // specifies UNIX path of the socket.
    ElementPtr name = socket_info->get("socket-name");
    if (!name) {
    ...
    if (lock_fd == -1) {
        ...
        isc_throw(SocketError, "cannot create socket lockfile, " << lock_name << ", : " << errmsg);
    }
    ...
    if (ret != 0) {
        std::string errmsg = strerror(errno);
        isc_throw(SocketError, "cannot lock socket lockfile, " << lock_name << ", : " << errmsg);
    }
    ...
    LOG_INFO(command_logger, COMMAND_ACCEPTOR_START).arg(socket_name_);
...
CommandMgr::CommandMgr() : HookedCommandMgr(), impl_(new CommandMgrImpl()) {
}
...
void CommandMgr::openCommandSocket(const isc::data::ElementPtr& socket_info) {
...
int CommandMgr::getControlSocketFD() {
...
void CommandMgr::setIOService(const IOServicePtr& io_service) {
    impl_->io_service_ = io_service;
}

void CommandMgr::setConnectionTimeout(const long timeout) {
    impl_->timeout_ = timeout;
}

ElementPtr CommandMgr::sendCommand(std::string const& socket_path, ElementPtr const& command) {
    // We are using our own IO service because this method is synchronous.
    isc::asiolink::IOServicePtr const& io_service(std::make_shared<isc::asiolink::IOService>());
    ClientConnection conn(*io_service);
    boost::system::error_code received_ec;
    ConstJSONFeedPtr received_feed;

    conn.start(
        ClientConnection::SocketPath(socket_path),
        ClientConnection::ControlCommand(command->toWire()),
        [&io_service, &received_ec, &received_feed](boost::system::error_code const& ec,
                                                    ConstJSONFeedPtr const& feed) {
    ...
    if (received_ec) {
        // Got an error.
        isc_throw(ControlSocketError, PRETTY_FUNCTION_NAME()
                                          << ": communication error: " << received_ec.message());
    }

    if (!received_feed) {
        // Failed to get the answer.
        isc_throw(ControlSocketError, PRETTY_FUNCTION_NAME() << ": empty response");
    }
