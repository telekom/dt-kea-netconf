...
public:
    ...
    void checkIntegerValue(const ElementPtr& map, const std::string& param_name,
                           int64_t exp_value) {
        ...
        ElementPtr elem = map->get(param_name);
        ASSERT_TRUE(elem);
...
public:
    ...
    /// @return a bool value
    bool getAsBool(ElementPtr scope, const std::string& name) {
...
TEST_F(SimpleParserTest, checkRequired) {
    ...
    EXPECT_THROW(SimpleParser::checkRequired(REQUIRED_KEYWORDS, empty),
                 DhcpConfigError);
    ElementPtr other = Element::fromJSON("{ \"foo\": 1, \"bar\": 2 }");
    EXPECT_THROW(SimpleParser::checkRequired(REQUIRED_KEYWORDS, other),
                 DhcpConfigError);
    ElementPtr good = Element::fromJSON("{ \"foobar\": 2 }");
...
TEST_F(SimpleParserTest, setListDefaults) {
    ...
    ElementPtr first = empty->get(0);
    ElementPtr second = empty->get(1);
    ElementPtr third = empty->get(2);
