...
TEST(Element, TypeNameConversion) {
    ...
    EXPECT_EQ("unknown", Element::typeToName((Element::type_t)123));
}

TEST(Element, RemoveEmptyRecursively) {
    ElementPtr const& tested(Element::fromJSON(R"({
  "1": [],
  "2": {},
  "3": {
    "4": {}
  ...
  "14": [
    ...
    tested->removeEmptyRecursively();

    EXPECT_EQ(tested->str(), expected->str());
}
...
TEST(Element, MapElement) {
    ...
    EXPECT_FALSE(el->find("value2/nothing/"));
...
TEST(Element, copy) {
    ...
    ASSERT_NO_THROW(item = elem->get(0));
    ...
    ASSERT_NO_THROW(item = deep->get("list")->get(1));
    ASSERT_NO_THROW(item->setValue(3));
    EXPECT_FALSE(deep->equals(*complex));
    EXPECT_TRUE(shallow->equals(*complex));
    ASSERT_NO_THROW(item = shallow->get("string"));
...
TEST(Element, getPosition) {
    ...
    // Element "a"
    ElementPtr level1_el = top->get("a");
    ...
    ElementPtr level2_el = level1_el->get("e");
