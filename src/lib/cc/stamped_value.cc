...
StampedValue::getValue() const {
    ...
    try {
        switch (static_cast<Element::type_t>(value_->getType())) {
        case Element::string:
...
StampedValue::validateConstruct() const {
    ...
    if ((value_->getType() != Element::string) &&
        ...
        isc_throw(TypeError, "StampedValue: provided value of the '"
                  ...
                  << Element::typeToName(static_cast<Element::type_t>(value_->getType())));
    }
}

void
StampedValue::validateAccess(Element::type_t type) const {
    if (!value_) {
