...
/// @tparam EP ElementPtr or ElementPtr (compiler can't infer which one)
...
///
/// @tparam EP ElementPtr or ElementPtr (compiler will infer which one)
...
Value<EP> moveComments1(EP element) {
    ...
    typedef std::map<std::string, ElementPtr> map_type;
    ...
    for (map_type::const_iterator it = map.cbegin(); it != map.cend(); ++it) {
        if (it->first == "comment") {
            ...
            Value<ElementPtr> item = moveComments1(it->second);
    ...
    if (has_comment) {
        ...
        ElementPtr comment = element->get("comment");
        ElementPtr moved = Element::createMap();
        moved->set("comment", comment);
        ElementPtr previous = element->get("user-context");
