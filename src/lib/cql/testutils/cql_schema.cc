...
char const* CQL_VALID_TYPE = "type=cql";
...
void destroyCqlSchema(bool show_err, bool force) {
    ...
    if (force || (!softWipeEnabled()) || wipeCqlData(show_err)) {
        ...
        runCqlScript(DATABASE_SCRIPTS_DIR, "cql/configdb_drop.cql", show_err);
    }
}

void createCqlSchema(bool show_err, bool force) {
...
void runCqlScript(std::string const& path, std::string const& script_name, bool show_err) {
...
bool wipeCqlData(bool show_err) {
    ...
    version << std::get<isc::db::MAJOR>(CQL_SCHEMA_VERSION) << "."
            << std::get<isc::db::MINOR>(CQL_SCHEMA_VERSION);
