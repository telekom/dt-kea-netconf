...
/// @brief Define PostgreSQL backend versions.
/// @{
uint32_t constexpr PG_CONFIG_SCHEMA_VERSION_MAJOR = 1;
uint32_t constexpr PG_CONFIG_SCHEMA_VERSION_MINOR = 0;
/// @}

/// @brief Define PostgreSQL backend version: 6.1
/// @{
uint32_t constexpr PG_SCHEMA_VERSION_MAJOR = 6;
uint32_t constexpr PG_SCHEMA_VERSION_MINOR = 1;
/// @}
...
// statement.
size_t constexpr PGSQL_MAX_PARAMETERS_IN_QUERY = 32;
...
public:
    ...
    PgSqlConnection(ParameterMap const& parameters)
    ...
    void prepareStatements(const std::unordered_map<std::string, PgSqlTaggedStatement> map);
