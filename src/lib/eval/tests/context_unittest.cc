...
public:
    ...
    void checkTokenString(const TokenPtr& token, const std::string& expected) {
        ASSERT_TRUE(token);
        std::shared_ptr<TokenString> str =
            std::dynamic_pointer_cast<TokenString>(token);
        ASSERT_TRUE(str);
    ...
    void checkTokenHexString(const TokenPtr& token,
        ...
        ASSERT_TRUE(token);
        std::shared_ptr<TokenHexString> hex =
            std::dynamic_pointer_cast<TokenHexString>(token);
        ASSERT_TRUE(hex);
    ...
    void checkTokenIpAddress(const TokenPtr& token,
                             const std::string& expected) {
        ASSERT_TRUE(token);
        std::shared_ptr<TokenIpAddress> ipaddr =
            std::dynamic_pointer_cast<TokenIpAddress>(token);
        ASSERT_TRUE(ipaddr);
        ...
        std::unique_ptr<IOAddress> exp_ip;
    ...
    void checkTokenEq(const TokenPtr& token) {
        ASSERT_TRUE(token);
        std::shared_ptr<TokenEqual> eq =
            std::dynamic_pointer_cast<TokenEqual>(token);
        EXPECT_TRUE(eq);
    }
    ...
    void checkTokenInteger(const TokenPtr& token, uint32_t exp_value) {
        ASSERT_TRUE(token);
        std::shared_ptr<TokenInteger> integer =
            std::dynamic_pointer_cast<TokenInteger>(token);
        ASSERT_TRUE(integer);
    ...
    void checkTokenOption(const TokenPtr& token,
        ...
        ASSERT_TRUE(token);
        std::shared_ptr<TokenOption> opt =
            std::dynamic_pointer_cast<TokenOption>(token);
        ASSERT_TRUE(opt);
    ...
    void checkTokenRelay4(const TokenPtr& token,
                          ...
                          TokenOption::RepresentationType expected_repr) {
        ASSERT_TRUE(token);
        std::shared_ptr<TokenRelay4Option> relay4 =
            std::dynamic_pointer_cast<TokenRelay4Option>(token);
        EXPECT_TRUE(relay4);
    ...
    void checkTokenRelay6Option(const TokenPtr& token,
                                ...
                                TokenOption::RepresentationType expected_repr) {
        ASSERT_TRUE(token);
        std::shared_ptr<TokenRelay6Option> opt =
            std::dynamic_pointer_cast<TokenRelay6Option>(token);
        ASSERT_TRUE(opt);
    ...
    void checkTokenPkt(const TokenPtr& token, TokenPkt::MetadataType type) {
        ASSERT_TRUE(token);
        std::shared_ptr<TokenPkt> pkt =
            std::dynamic_pointer_cast<TokenPkt>(token);
        ASSERT_TRUE(pkt);
    ...
    void checkTokenPkt4(const TokenPtr& token, TokenPkt4::FieldType type) {
        ASSERT_TRUE(token);
        std::shared_ptr<TokenPkt4> pkt =
            std::dynamic_pointer_cast<TokenPkt4>(token);
        ASSERT_TRUE(pkt);
    ...
    void checkTokenPkt6(const TokenPtr& token,
        ...
        std::shared_ptr<TokenPkt6> pkt =
            std::dynamic_pointer_cast<TokenPkt6>(token);
    ...
    void checkTokenRelay6Field(const TokenPtr& token,
        ...
        ASSERT_TRUE(token);
        std::shared_ptr<TokenRelay6Field> opt =
            std::dynamic_pointer_cast<TokenRelay6Field>(token);
        ASSERT_TRUE(opt);
    ...
    void checkTokenMember(const TokenPtr& token,
        ...
        ASSERT_TRUE(token);
        std::shared_ptr<TokenMember> member =
            std::dynamic_pointer_cast<TokenMember>(token);
        ASSERT_TRUE(member);
    ...
    void checkTokenSubstring(const TokenPtr& token) {
        ASSERT_TRUE(token);
        std::shared_ptr<TokenSubstring> sub =
            std::dynamic_pointer_cast<TokenSubstring>(token);
        EXPECT_TRUE(sub);
    }
    ...
    void checkTokenConcat(const TokenPtr& token) {
        ASSERT_TRUE(token);
        std::shared_ptr<TokenConcat> conc =
            std::dynamic_pointer_cast<TokenConcat>(token);
        EXPECT_TRUE(conc);
    }
    ...
    void checkTokenIfElse(const TokenPtr& token) {
        ASSERT_TRUE(token);
        std::shared_ptr<TokenIfElse> alt =
            std::dynamic_pointer_cast<TokenIfElse>(token);
        EXPECT_TRUE(alt);
    }
    ...
    void checkTokenToHexString(const TokenPtr& token) {
        ASSERT_TRUE(token);
        std::shared_ptr<TokenToHexString> tohex =
            std::dynamic_pointer_cast<TokenToHexString>(token);
        EXPECT_TRUE(tohex);
    }
    ...
    void checkTokenVendor(const TokenPtr& token, uint32_t exp_vendor_id,
        ...
        std::shared_ptr<TokenVendor> vendor =
            std::dynamic_pointer_cast<TokenVendor>(token);
    ...
    void testVendorEnterprise(const std::string& expr,
        ...
        std::shared_ptr<TokenVendor> vendor =
            std::dynamic_pointer_cast<TokenVendor>(eval.expression.at(0));
    ...
    void checkTokenVendorClass(const TokenPtr& token, uint32_t vendor_id,
        ...
        std::shared_ptr<TokenVendorClass> vendor =
            std::dynamic_pointer_cast<TokenVendorClass>(token);
    ...
    void checkTokenSubOption(const TokenPtr& token,
                             ...
                             TokenOption::RepresentationType expected_repr) {
        ASSERT_TRUE(token);
        std::shared_ptr<TokenSubOption> sub =
            std::dynamic_pointer_cast<TokenSubOption>(token);
        ASSERT_TRUE(sub);
...
TEST_F(EvalContextTest, logicalOps) {
    ...
    ASSERT_TRUE(token);
    std::shared_ptr<TokenNot> tnot =
        std::dynamic_pointer_cast<TokenNot>(token);
    EXPECT_TRUE(tnot);
    ...
    ASSERT_TRUE(token);
    std::shared_ptr<TokenAnd> tand =
        std::dynamic_pointer_cast<TokenAnd>(token);
    EXPECT_TRUE(tand);
    ...
    ASSERT_TRUE(token);
    std::shared_ptr<TokenOr> tor =
        std::dynamic_pointer_cast<TokenOr>(token);
    EXPECT_TRUE(tor);
}
