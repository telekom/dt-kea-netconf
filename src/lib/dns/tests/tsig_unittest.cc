...
protected:
    TSIGTest() :
        tsig_ctx(nullptr), qid(0x2d65), test_name("www.example.com"),
        badkey_name("badkey.example.com"), test_class(RRClass::IN()),
        test_ttl(86400), message_(Message::RENDER),
    ...
    std::unique_ptr<TestTSIGContext> tsig_ctx;
    std::unique_ptr<TSIGContext> tsig_verify_ctx;
    TSIGKeyRing keyring;
    const uint16_t qid;
                               ...
                               Rcode rcode)
{
    ...
    if ((message_flags & AA_FLAG) != 0) {
        message_.setHeaderFlag(Message::HEADERFLAG_AA);
    }
    if ((message_flags & RD_FLAG) != 0) {
        message_.setHeaderFlag(Message::HEADERFLAG_RD);
    }
    if (add_question) {
        message_.addQuestion(Question(qname, test_class, qtype));
    }
    if (answer_data != NULL) {
        if (answer_type == NULL) {
            answer_type = &qtype;
    ...
    return (ConstTSIGRecordPtr(new TSIGRecord(*message_.getTSIGRecord())));
}
...
TEST_F(TSIGTest, verifyThenSignResponse) {
    ...
    createMessageFromFile("message_toWire2.wire");
    {
        ...
        commonVerifyChecks(*tsig_verify_ctx, message_.getTSIGRecord(),
...
TEST_F(TSIGTest, badtimeResponse) {
    ...
    };
    {
        ...
        commonSignChecks(tsig, message_.getQid(), 0x4da8b9d6,
...
TEST_F(TSIGTest, badsigResponse) {
                                ...
                                &dummy_data[0], dummy_data.size()));
    {
        ...
        commonVerifyChecks(bad_ctx, message_.getTSIGRecord(),
                           &received_data[0], received_data.size(),
    ...
    // context of "checked state".
    {
        ...
        commonSignChecks(createMessageAndSign(qid, test_name, &bad_ctx),
                         message_.getQid(), 0x4da8877a, NULL, 0,
                         16);   // 16: BADSIG
    }
}
...
TEST_F(TSIGTest, badsigThenValidate) {
    ...
    createMessageFromFile("tsig_verify4.wire");
    {
        ...
        commonVerifyChecks(*tsig_ctx, message_.getTSIGRecord(),
                           &received_data[0], received_data.size(),
...
TEST_F(TSIGTest, verifyAfterSendResponse) {
    ...
    isc::util::detail::gettimeFunction = testGetTime<0x4da8877a>;
    createMessageFromFile("message_toWire2.wire");
    tsig_verify_ctx->verify(message_.getTSIGRecord(), &received_data[0],
                            received_data.size());
    EXPECT_EQ(TSIGContext::RECEIVED_REQUEST, tsig_verify_ctx->getState());
    ...
    createMessageFromFile("message_toWire2.wire");
    EXPECT_THROW(tsig_verify_ctx->verify(message_.getTSIGRecord(),
                                         &received_data[0],
                                         received_data.size()),
                 TSIGContextError);
}
...
TEST_F(TSIGTest, signAfterVerified) {
    ...
    createMessageAndSign(qid, test_name, tsig_ctx.get());
    createMessageFromFile("tsig_verify5.wire");
    tsig_ctx->verify(message_.getTSIGRecord(), &received_data[0],
                     received_data.size());
...
TEST_F(TSIGTest, truncatedMAC) {
    ...
    {
        SCOPED_TRACE("Verify test for request");
        commonVerifyChecks(sha_ctx, message_.getTSIGRecord(),
                           &received_data[0], received_data.size(),
                           TSIGError::NOERROR(), TSIGContext::RECEIVED_REQUEST);
    }
    ...
    {
        SCOPED_TRACE("Verify test for request");
        commonVerifyChecks(bad_sha_ctx, message_.getTSIGRecord(),
                           &received_data[0], received_data.size(),
