...
private:
    ...
    RRType parseRRParams(bool& explicit_ttl, MasterToken rrparam_token) {
        ...
        std::unique_ptr<RRClass> rrclass
    ...
    /// doesn't exist in the unique_ptr, make a new RRTTL copy of the
    /// right argument.
    static void assignTTL(std::unique_ptr<RRTTL>& left, const RRTTL& right) {
        if (!left) {
...
private:
    ...
    std::unique_ptr<RRTTL> default_ttl_; // Default TTL of RRs used when
                                           // unspecified.  If NULL no default
                                           // is known.
    std::unique_ptr<RRTTL> current_ttl_; // The TTL used most recently.
