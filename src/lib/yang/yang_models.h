...
static const std::string KEATEST_MODULE = "keatest-module";

struct Is {
    static bool Ietf(std::string const& model) {
        return model == IETF_DHCPV6_SERVER;
    }
};

struct Has {
    static bool OperationalDataNodes(std::string const& model) {
        return model == IETF_DHCPV6_SERVER || model == KEA_DHCP4_SERVER;
    }
};
