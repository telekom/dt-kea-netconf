...
struct TranslatorOptionDef : virtual public TranslatorBasic {
    ...
    void setOptionDef(const std::string& xpath, isc::data::ElementPtr elem);
...
protected:
    ...
    /// @param elem The JSON element.
    /// @throw BadValue on option definition without name or type.
    void setOptionDefKea(const std::string& xpath, isc::data::ElementPtr elem);
};
...
/// Currently supports kea-dhcp[46]-server models.
/// @todo: Support for ietf-dhcpv6-server model.
struct TranslatorOptionDefList : virtual public TranslatorOptionDef {
    using S_Session = sysrepo::S_Session;
    ...
    /// @throw SysrepoError when sysrepo raises an error.
    isc::data::ElementPtr getOptionDefList(const std::string& xpath);

    /// @brief Translate and set option definition list from JSON to YANG.
    ///
    /// @param xpath The xpath of the option definition list.
    /// @param elem The JSON element.
    void setOptionDefList(const std::string& xpath, isc::data::ElementPtr elem);
...
protected:
    ...
    /// @param xpath The xpath of the option definition list.
    /// @throw SysrepoError when sysrepo raises an error.
    isc::data::ElementPtr getOptionDefListKea(const std::string& xpath);
    ...
    void setOptionDefListKea(const std::string& xpath, isc::data::ElementPtr elem);
};
