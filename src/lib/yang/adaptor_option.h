...
struct AdaptorOption {
    ...
    static void checkType(isc::data::ElementPtr option);

    /// @brief Check if code is specified in option defintion.
    ///
    /// @param option The option.
    /// @throw MissingKey if the code is not present.
    static void checkCode(isc::data::ElementPtr option);
    ...
    static void collect(isc::data::ElementPtr option, OptionCodes& codes);
...
protected:
    ...
    static void initCodesInternal(OptionCodes& codes,
