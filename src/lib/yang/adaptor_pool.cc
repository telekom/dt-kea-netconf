...
void AdaptorPool::canonizePool(ElementPtr pool) {
...
void AdaptorPool::fromSubnet(const string& model, ElementPtr subnet, ElementPtr pools) {
    if (model == IETF_DHCPV6_SERVER) {
        fromSubnetIetf6(subnet, pools);
    } else if ((model != KEA_DHCP4_SERVER) && (model != KEA_DHCP6_SERVER)) {
        isc_throw(NotImplemented, "fromSubnet not implemented for the model: " << model);
    }
}

void AdaptorPool::fromSubnetIetf6(ElementPtr subnet, ElementPtr pools) {
...
void AdaptorPool::toSubnet(const string& model, ElementPtr subnet, ElementPtr pools) {
    if (model == IETF_DHCPV6_SERVER) {
        toSubnetIetf6(subnet, pools);
    } else if ((model != KEA_DHCP4_SERVER) && (model != KEA_DHCP6_SERVER)) {
        isc_throw(NotImplemented, "toSubnet not implemented for the model: " << model);
    }
}

void AdaptorPool::toSubnetIetf6(ElementPtr subnet, ElementPtr pools) {
