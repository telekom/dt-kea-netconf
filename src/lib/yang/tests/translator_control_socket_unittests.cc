...
struct TranslatorControlSocketTest : GenericTranslatorTest<control_socket, TranslatorControlSocket> {
...
TEST_F(TranslatorControlSocketTest, set) {
    ...
    ElementPtr name = got->get("socket-name");
    ASSERT_TRUE(name);
    ASSERT_EQ(Element::string, name->getType());
    EXPECT_EQ("/tmp/kea.sock", name->stringValue());
    ElementPtr type = got->get("socket-type");
    ASSERT_TRUE(type);
    ASSERT_EQ(Element::string, type->getType());
    EXPECT_EQ("unix", type->stringValue());
    ElementPtr context = got->get("user-context");
    ASSERT_TRUE(context);
...
TEST_F(TranslatorControlSocketTest, setEmpty) {
    ...
    // Reset to empty.
    ASSERT_NO_THROW(t_obj_->setControlSocket(xpath, ElementPtr()));
