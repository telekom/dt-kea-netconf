...
const string AdaptorHost::STD_CHARACTERS =
    "0123456789@ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz-.@_";

void AdaptorHost::quoteIdentifier(ElementPtr host) {
    ElementPtr flex_id = host->get("flex-id");
    if (!flex_id) {
        return;
    }
    ...
    tmp << hex;
    bool delim = false;
    for (vector<uint8_t>::const_iterator it = binary.begin(); it != binary.end(); ++it) {
        if (delim) {
            tmp << ":";
        }
