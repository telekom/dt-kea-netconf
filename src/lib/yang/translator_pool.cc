...
ElementPtr TranslatorPool::getPool(const string& xpath) {
    try {
        ...
        isc_throw(SysrepoError, "sysrepo error getting pool at '" << xpath << "': " << ex.what());
    }
    isc_throw(NotImplemented, "getPool not implemented for the model: " << model_);
}

ElementPtr TranslatorPool::getPoolIetf6(const string& xpath) {
    ElementPtr result = Element::createMap();
    // Skip pool-id which exists but is not used.
    ElementPtr pool = getItem(xpath + "/pool-prefix");
    if (!pool) {
...
ElementPtr TranslatorPool::getPoolKea(const string& xpath) {
    ElementPtr result = Element::createMap();
    ElementPtr prefix = getItem(xpath + "/prefix");
    if (prefix) {
        result->set("pool", prefix);
    } else {
        ElementPtr start_addr = getItem(xpath + "/start-address");
        ElementPtr end_addr = getItem(xpath + "/end-address");
        if (!start_addr || !end_addr) {
            isc_throw(BadValue, "getPoolKea requires either prefix or "
                                "both start and end addresses at "
                                    << xpath);
        }
        ostringstream range;
        range << start_addr->stringValue() << " - " << end_addr->stringValue();
...
void TranslatorPool::setPool(const string& xpath, ElementPtr elem) {
    try {
        if (model_ == IETF_DHCPV6_SERVER) {
            ...
            isc_throw(NotImplemented, "setPool not implemented for the model: " << model_);
        }
    } catch (const sysrepo_exception& ex) {
        isc_throw(SysrepoError, "sysrepo error setting pool '" << elem->str() << "' at '" << xpath
                                                               << "': " << ex.what());
    }
}

void TranslatorPool::setPoolIetf6(const string& xpath, ElementPtr elem) {
    ElementPtr pool = elem->get("pool");
    if (!pool) {
        isc_throw(BadValue, "setPoolIetf6 requires pool: " << elem->str());
    }
    string prefix = pool->stringValue();
    if (prefix.find("/") == string::npos) {
        isc_throw(BadValue, "setPoolIetf only supports pools in prefix (vs range) "
                            "format and was called with '"
                                << prefix << "'");
    }
    setItem(xpath + "/pool-prefix", pool, SR_STRING_T);
    string addr = prefix.substr(0, prefix.find_first_of(" /"));
    uint8_t plen =
        boost::lexical_cast<unsigned>(prefix.substr(prefix.find_last_of(" /") + 1, string::npos));
    const IOAddress& base(addr);
    setItem(xpath + "/start-address", Element::create(firstAddrInPrefix(base, plen).toText()),
            SR_STRING_T);
    setItem(xpath + "/end-address", Element::create(lastAddrInPrefix(base, plen).toText()),
            SR_STRING_T);
    ...
    setItem(xpath + "/max-address-count", Element::create(string("disabled")), SR_ENUM_T);
}

void TranslatorPool::setPoolKea(const string& xpath, ElementPtr elem) {
    ElementPtr pool = elem->get("pool");
    if (!pool) {
...
void TranslatorPool::getAddresses(const string& prefix,
    ...
    if (dash == string::npos) {
        isc_throw(BadValue, "getAddresses called with invalid prefix: " << prefix);
    }
...
ElementPtr TranslatorPools::getPools(const string& xpath) {
    try {
        if (model_ == IETF_DHCPV6_SERVER) {
        ...
        isc_throw(SysrepoError, "sysrepo error getting pools at '" << xpath << "': " << ex.what());
    }
    isc_throw(NotImplemented, "getPools not implemented for the model: " << model_);
}

ElementPtr TranslatorPools::getPoolsIetf(const string& xpath) {
    return getList<TranslatorPool>(xpath + "/address-pool", *this, &TranslatorPool::getPool);
}

ElementPtr TranslatorPools::getPoolsKea(const string& xpath) {
    return getList<TranslatorPool>(xpath + "/pools", *this, &TranslatorPool::getPool);
}

void TranslatorPools::setPools(const string& xpath, ElementPtr elem) {
    try {
        if (model_ == IETF_DHCPV6_SERVER) {
            ...
            isc_throw(NotImplemented, "setPools not implemented for the model: " << model_);
        }
    } catch (const sysrepo_exception& ex) {
...
void TranslatorPools::setPoolsById(const string& xpath, ElementPtr elem) {
...
void TranslatorPools::setPoolsByAddresses(const string& xpath, ElementPtr elem) {
    for (size_t i = 0; i < elem->size(); ++i) {
        ElementPtr pool = elem->get(i);
        if (!pool->contains("pool")) {
            isc_throw(BadValue,
                      "setPoolsByAddresses(): missing \"pool\" element in: " << pool->str());
        }
        ...
        ostringstream prefix;
        prefix << xpath << "/pools[start-address='" << start_addr << "'][end-address='" << end_addr
               << "']";
        setPool(prefix.str(), pool);
    }
}
