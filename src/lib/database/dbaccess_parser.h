...
public:
    ...
    void parse(std::string& access_string,
               isc::data::ElementPtr database_config);
    ...
    /// @brief wrapper over @ref DbAccess::parse()
    static std::string parse(isc::data::ElementPtr database_config) {
        DbAccessParser parser;
        std::string db_access;
        parser.parse(db_access, database_config);
        return db_access;
    }
