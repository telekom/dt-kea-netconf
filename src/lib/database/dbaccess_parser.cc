...
DbAccessParser::parse(std::string& access_string,
    ...
    // 2. Update the copy with the passed keywords.
    for (std::pair<std::string, ElementPtr> param : database_config->mapValue()) {
        try {
    ...
    if ((dbtype != "memfile") &&
        ...
        (dbtype != "postgresql") &&
        (dbtype != "cql")) {
        ElementPtr value = database_config->get("type");
    ...
    if ((lfc_interval < 0) ||
        ...
        ElementPtr value = database_config->get("lfc-interval");
    ...
    if ((timeout < 0) ||
        ...
        ElementPtr value = database_config->get("connect-timeout");
    ...
    if ((port < 0) ||
        ...
        ElementPtr value = database_config->get("port");
    ...
    if ((max_row_errors < 0) ||
        ...
        ElementPtr value = database_config->get("max-row-errors");
    ...
    if (max_reconnect_tries < 0) {
        ElementPtr value = database_config->get("max-reconnect-tries");
    ...
    if ((reconnect_wait_time < 0) ||
        ...
        ElementPtr value = database_config->get("reconnect-wait-time");
    ...
    if ((request_timeout < 0) ||
        ...
        ElementPtr value = database_config->get("request-timeout");
    ...
    if ((tcp_keepalive < 0) ||
        ...
        ElementPtr value = database_config->get("tcp-keepalive");
