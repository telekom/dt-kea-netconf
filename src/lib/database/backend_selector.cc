...
BackendSelector::BackendSelector(const data::ElementPtr& access_map)
    : backend_type_(BackendSelector::Type::UNSPEC),
      host_(), port_(0) {
    ...
    ElementPtr t = access_map->get("type");
    if (t) {
    ...
    ElementPtr h = access_map->get("host");
    if (h) {
    ...
    ElementPtr p = access_map->get("port");
    if (p) {
...
template <>
std::string BackendSelector::TypeToString<BackendSelector::Type::CQL>() {
    return "cql";
}

template <>
std::string BackendSelector::TypeToString<BackendSelector::Type::MYSQL>() {
    return "mysql";
}

template <>
std::string BackendSelector::TypeToString<BackendSelector::Type::PGSQL>() {
    return "pgsql";
}

template <>
uint16_t BackendSelector::port<BackendSelector::Type::CQL>() {
    return 9042;
}

template <>
uint16_t BackendSelector::port<BackendSelector::Type::MYSQL>() {
    return 3306;
}

template <>
uint16_t BackendSelector::port<BackendSelector::Type::PGSQL>() {
    return 5432;
}
