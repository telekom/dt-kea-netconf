...
protected:
    ...
    std::shared_ptr<HttpListenerImpl> impl_;
};

/// @brief Pointer to the @ref HttpListener.
typedef std::shared_ptr<HttpListener> HttpListenerPtr;

/// @brief Pointer to the const @ref HttpListener.
typedef std::shared_ptr<const HttpListener> ConstHttpListenerPtr;
