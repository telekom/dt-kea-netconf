...
HttpResponseJson::setGenericBody(const HttpStatusCode& status_code) {
    ...
    if (isClientError(status_code) || isServerError(status_code)) {
        std::map<std::string, ElementPtr> map_elements;
        map_elements["result"] =
            ElementPtr(new IntElement(statusCodeToNumber(status_code)));
        map_elements["text"] =
            ElementPtr(new StringElement(statusCodeToString(status_code)));
...
void
HttpResponseJson::setBodyAsJson(const ElementPtr& json_body) {
    if (json_body) {
...
HttpResponseJson::getJsonElement(const std::string& element_name) const {
    try {
        ElementPtr body = getBodyAsJson();
        if (body) {
            const std::map<std::string, ElementPtr>& map_value = body->mapValue();
