...
typedef std::shared_ptr<GenericResponse> GenericResponsePtr;
    ...
    createDynamicHttpResponse(const ConstHttpRequestPtr& request) {
        ...
        if (request_json) {
            ...
            if (body) {
                ...
                ElementPtr content_type = body->get("requested-content-type");
                ElementPtr partial_response = body->get("partial-response");
...
typedef std::shared_ptr<TestHttpClient> TestHttpClientPtr;
...
public:
    ...
    void testConsecutiveRequests(const HttpVersion& version) {
        ...
        ElementPtr sequence1 = response1->getJsonElement("sequence");
        ASSERT_TRUE(sequence1);

        ASSERT_TRUE(response2);
        ElementPtr sequence2 = response2->getJsonElement("sequence");
