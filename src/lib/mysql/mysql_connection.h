...
public:
    ...
    MySqlConnection(ParameterMap const& parameters) : DatabaseConnection(parameters) {
    }
    ...
    void prepareStatement(uint32_t const index, std::string const& text);
    ...
    static void
    convertToDatabaseTime(const time_t cltt, const uint32_t valid_lifetime, MYSQL_TIME& expire);
    ...
    ///        is put.
    static void
    convertFromDatabaseTime(const MYSQL_TIME& expire, uint32_t valid_lifetime, time_t& cltt);
    ///@}
    ...
    void selectQuery(StatementIndex const& index,
    ...
    /// in the query.
    template<typename StatementIndex>
    void insertQuery(StatementIndex const& index,
                     const MySqlBindingCollection& in_bindings) {
        checkUnusable();
        std::vector<MYSQL_BIND> in_bind_vec;
        for (MySqlBindingPtr in_binding : in_bindings) {
    ...
    template<typename StatementIndex>
    uint64_t updateDeleteQuery(StatementIndex const& index,
                               const MySqlBindingCollection& in_bindings) {
        checkUnusable();
        std::vector<MYSQL_BIND> in_bind_vec;
        for (MySqlBindingPtr in_binding : in_bindings) {
        ...
        if (status != 0) {
            ...
            default:
                ...
                isc_throw(db::DbOperationError,
                          what << " for <" << text_statements_[static_cast<int>(index)]
                               << ">, reason: " << mysql_error(mysql_) << " (error code "
                               << mysql_errno(mysql_) << ")");
            }
        }
    }
...
std::unordered_map<std::type_index, decltype(MYSQL_BIND::buffer_type)> bufferType{
    {typeid(int8_t), MYSQL_TYPE_TINY},        {typeid(uint8_t), MYSQL_TYPE_TINY},
    {typeid(int16_t), MYSQL_TYPE_SHORT},      {typeid(uint16_t), MYSQL_TYPE_SHORT},
    {typeid(int32_t), MYSQL_TYPE_LONG},       {typeid(uint32_t), MYSQL_TYPE_LONG},
    {typeid(int64_t), MYSQL_TYPE_LONGLONG},   {typeid(uint64_t), MYSQL_TYPE_LONGLONG},
    {typeid(std::string), MYSQL_TYPE_STRING}, {typeid(char), MYSQL_TYPE_STRING}};

template <typename T>
inline decltype(MYSQL_BIND::buffer_length) size_of(T*) {
    return sizeof(T);
}

template <>
inline decltype(MYSQL_BIND::buffer_length) size_of<std::string>(std::string* buffer) {
    return buffer->size();
}

template <>
inline decltype(MYSQL_BIND::buffer_length) size_of<char>(char* buffer) {
    return std::strlen(buffer);
}

template <typename T>
inline decltype(MYSQL_BIND::buffer) raw_data(T* buffer) {
    return reinterpret_cast<decltype(MYSQL_BIND::buffer)>(buffer);
}

template <>
inline decltype(MYSQL_BIND::buffer) raw_data<std::string>(std::string* buffer) {
    return reinterpret_cast<decltype(MYSQL_BIND::buffer)>(const_cast<char*>(buffer->c_str()));
}

template <typename T>
inline void precondition(T*, size_t) {
}

template <>
inline void precondition<char>(char* buffer, size_t size) {
    std::memset(buffer, '\0', size);
}

template <typename T>
static MYSQL_BIND mySqlBind(T* buffer) {
    MYSQL_BIND b;
    std::memset(&b, 0, sizeof(MYSQL_BIND));
    b.buffer = raw_data(buffer);
    b.buffer_length = size_of(buffer);
    auto buffer_type = bufferType.find(typeid(T));
    b.buffer_type = buffer_type == bufferType.end() ? MYSQL_TYPE_NULL : buffer_type->second;
    b.is_unsigned = std::is_unsigned<T>::value ? isc::db::MLM_TRUE : isc::db::MLM_FALSE;
    return b;
}

template <typename T>
static MYSQL_BIND mySqlBind(T* buffer, size_t& size) {
    size = 1024;
...
struct MySqlBind {
    MySqlBind() {
        // 32: the freezing point of water at standard atmospheric pressure in degrees Fahrenheit
        data_.reserve(32);
    }

    template <typename T>
    void add(T* buffer) {
        data_.push_back(mySqlBind(buffer));
    }

    template <typename T>
    void add(T* buffer, size_t& size) {
        data_.push_back(mySqlBind(buffer, size));
    }
