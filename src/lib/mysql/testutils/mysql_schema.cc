...
void destroyMySQLSchema(bool show_err, bool force) {
    ...
    if (force || (!softWipeEnabled()) || wipeMySQLData(show_err)) {
        ...
        runMySQLScript(DATABASE_SCRIPTS_DIR, "mysql/configdb_drop.mysql", show_err);
    }
}
...
bool wipeMySQLData(bool show_err) {
    ...
    cmd << "mysql/wipe_data.sh" << " " << version.str()
        << " -N -B -h --host=localhost --user=keatest --password=keatest keatest";
    if (!show_err) {
        cmd << " 2>/dev/null ";
    }
...
void runMySQLScript(const std::string& path, const std::string& script_name,
                    bool show_err) {
    std::ostringstream cmd;
    cmd << "mysql -N -B --host=localhost --user=keatest --password=keatest --protocol=tcp keatest";
    if (!show_err) {
        cmd << " 2>/dev/null ";
    }
