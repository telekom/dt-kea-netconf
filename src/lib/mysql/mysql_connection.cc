...
#ifdef HAVE_GTEST
    // Force TCP when running unit tests.
    sprotocol = "tcp";
#endif
...
void
MySqlConnection::prepareStatement(uint32_t const index, std::string const& text) {
    ...
    int status = mysql_stmt_prepare(statements_[index], text.c_str(), text.size());
    if (status != 0) {
        isc_throw(DbOperationError, "unable to prepare MySQL statement <" <<
...
void MySqlConnection::convertToDatabaseTime(const time_t cltt,
