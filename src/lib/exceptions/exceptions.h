...
private:
    ...
    void operator=(Exception const& src) = delete;
...
struct RunTimeFail : public Exception {
    RunTimeFail(const char* file, size_t line, const char* what)
        : isc::Exception(file, line, what) {
    }
};

/// @brief An alternative to googletest's ASSERT_NO_THROW that also displays what exception was
/// thrown
...
/// @brief An alternative to googletest's EXPECT_NO_THROW that also displays what exception was
/// thrown
