...
struct adapter_CfgSubnets {};

template <>
struct adapter_CfgSubnets<DHCP_SPACE_V4> {
    using type = CfgSubnets4;
};

template <>
struct adapter_CfgSubnets<DHCP_SPACE_V6> {
    using type = CfgSubnets6;
};

}  // namespace

template <DhcpSpaceType D>
using CfgSubnetsT = typename adapter_CfgSubnets<D>::type;
...
public:
    ...
    DdnsParams(const Subnet4Ptr& subnet, bool d2_client_enabled)
        : subnet_(std::dynamic_pointer_cast<Subnet>(subnet)),
...
typedef std::shared_ptr<DdnsParams> DdnsParamsPtr;
...
public:
    ...
    enum SrvConfigType {
        ...
        // provided configuration file. There is no need for a master database.
        CONFIG_DATABASE = 3
    };
    ...
    SrvConfigType& getConfigurationType() {
    ...
    SrvConfigType const& getConfigurationType() const {
        return cfg_configuration_type_;
    }
    ...
    const isc::data::ElementPtr getControlSocketInfo() const {
        return (control_socket_);
    }

    isc::data::ElementPtr const& getNotificationsSocketInfo() const {
        return notifications_socket_;
    }

    /// @brief Sets information about the control socket
    /// @param control_socket Element that holds control-socket map
    void setControlSocketInfo(const isc::data::ElementPtr& control_socket) {
        control_socket_ = control_socket;
    }

    void setNotificationsSocketInfo(isc::data::ElementPtr const& socket) {
        notifications_socket_ = socket;
    }

    /// @brief Returns DHCP queue control information
    /// @return pointer to the DHCP queue control information
    const isc::data::ElementPtr getDHCPQueueControl() const {
    ...
    void setDHCPQueueControl(const isc::data::ElementPtr dhcp_queue_control) {
    ...
    const isc::data::ElementPtr getDHCPMultiThreading() const {
    ...
    void setDHCPMultiThreading(const isc::data::ElementPtr dhcp_multi_threading) {
    ...
    void setMasterCfgTimestamp(isc::db::timestamp_t const& timestamp) {
    ...
    isc::db::timestamp_t getMasterCfgTimestamp() const {
    ...
    void setServerCfgTimestamp(isc::db::timestamp_t const& timestamp) {
    ...
    isc::db::timestamp_t getServerCfgTimestamp() const {
        return server_cfg_timestamp_;
    }

#ifdef TERASTREAM
    /// @brief Sets the history size for private allocations i.e. the maximum
    ///     allowed number of addresses that can be cycled through.
    ///
    /// @param history_size maximum number of addresses
    void setPrivacyHistorySize(uint32_t history_size) {
        history_size_ = history_size;
    }

    /// @brief Retrieves the history size for private allocations i.e. the
    ///     maximum allowed number of addresses that can be cycled through.
    ///
    /// @return maximum number of addresses
    uint32_t getPrivacyHistorySize() const {
        return history_size_;
    }

    /// @brief Sets the valid lifetime for private allocations i.e. the valid
    ///     lifetime allowed for a single address to be reused before
    ///     cycling through the history
    ///
    /// @param valid_lifetime for the address
    void setPrivacyValidLifetime(uint32_t valid_lifetime) {
        valid_lft_ = valid_lifetime;
    }

    /// @brief Retrieves the valid lifetime for private allocations i.e. the
    ///     valid lifetime allowed for a single address to be reused before
    ///     cycling through the history.
    ///
    /// @return valid lifetime address can be reused
    uint32_t getPrivacyValidLifetime() const {
        return valid_lft_;
    }
#endif  // TERASTREAM
    ...
    /// empty pointer.
    isc::data::ElementPtr getConfiguredGlobal(std::string name) const;
    ...
    void extractConfiguredGlobals(isc::data::ElementPtr config);
    ...
    void addConfiguredGlobal(const std::string& name, isc::data::ElementPtr value) {
    ...
    /// @{
    template <DhcpSpaceType D>
    std::shared_ptr<CfgSubnetsT<D>> getCfgSubnets();

    template <DhcpSpaceType D>
    std::shared_ptr<CfgSubnetsT<D> const> getCfgSubnets() const;
    /// @}
...
private:
    ...
    SrvConfigType cfg_configuration_type_;
    ...
    isc::data::ElementPtr control_socket_;

    isc::data::ElementPtr notifications_socket_;

    /// @brief Pointer to the dhcp-queue-control information
    isc::data::ElementPtr dhcp_queue_control_;

    /// @brief Pointer to the multi-threading information
    isc::data::ElementPtr dhcp_multi_threading_;
    ...
    isc::db::timestamp_t master_server_cfg_timestamp_;

    /// @brief The configuration timestamp of the server configuration.
    isc::db::timestamp_t server_cfg_timestamp_;

#ifdef TERASTREAM
    /// @brief The maximum allowed number of addresses that can be cycled
    ///     through, used in private allocations
    uint32_t history_size_;

    /// @brief The valid lifetime allowed for a single address to be reused
    ///     before cycling through the history, used in private allocations
    uint32_t valid_lft_;
#endif  // TERASTREAM
...
typedef std::shared_ptr<SrvConfig> SrvConfigPtr;

/// @brief Const pointer to the @c SrvConfig.
typedef std::shared_ptr<const SrvConfig> ConstSrvConfigPtr;
//@}

using SrvConfigType = SrvConfig::SrvConfigType;
