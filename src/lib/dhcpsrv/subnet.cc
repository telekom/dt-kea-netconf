...
SubnetID Subnet::static_id_ = 0;

Subnet::Subnet(const isc::asiolink::IOAddress& prefix, uint8_t len,
               const SubnetID id)
    : generated_(id == 0), id_(generated_ ? generateNextID() : id), prefix_(prefix),
      prefix_len_(len),
      ...
      allocation_type_(ALLOCATION_TYPE_DEFAULT),
...
Subnet4::create(const IOAddress& prefix, uint8_t length,
                ...
                const Triplet<uint32_t>& valid_lifetime,
                const SubnetID id) {
    Subnet4Ptr subnet = std::make_shared<Subnet4>
...
Subnet6::create(const IOAddress& prefix, uint8_t length,
                ...
                const Triplet<uint32_t>& valid_lifetime,
                const SubnetID id) {
    Subnet6Ptr subnet = std::make_shared<Subnet6>
...
Subnet::toElement() const {
    ...
    // Set allocation-type
    if (getAllocationType() != ALLOCATION_TYPE_DEFAULT) {
        map->set("allocation-type", Element::create(Subnet::allocationTypeToString(getAllocationType())));
    }
...
Subnet4::toElement() const {
    ...
    if (getExcludedPSIDs().size()) {
        ElementPtr excluded_list = Element::createList();
        for (auto value: getExcludedPSIDs()) {
            excluded_list->add(Element::create(value));
        }
        map->set("v4-excluded-psids", excluded_list);
    }
...
std::string
Subnet::allocationTypeToString(Subnet::AllocationType value) {
    static std::unordered_map<Subnet::AllocationType, std::string> map{
        {Subnet::ALLOCATION_TYPE_DEFAULT, "default"},
        {Subnet::ALLOCATION_TYPE_PRIVACY, "privacy"}
    };
    return map.at(value);
}

Subnet::AllocationType
Subnet::stringToAllocationType(std::string value) {
    static std::unordered_map<std::string, Subnet::AllocationType> map{
        {"default", Subnet::ALLOCATION_TYPE_DEFAULT},
        {"privacy", Subnet::ALLOCATION_TYPE_PRIVACY}
    };
    return map.at(value);
}
...
bool Subnet4::isExcludedAddress(const asiolink::IOAddress& address) {
    if (address.getPsidLen()) {
        return excludedPSIDs_.find(address.getPsid()) != excludedPSIDs_.end();
    }
    return false;
}
