...
private:
    ...
    static std::unique_ptr<LeaseMgr>& getLeaseMgrPtr();
};

#ifdef TERASTREAM_LOCK
template <DhcpSpaceType D>
struct PacketLock {
    PacketLock(std::vector<uint8_t>& txn_id) : txn_id_(txn_id) {
        locked_ = LeaseMgrFactory::instance().lock<D>(txn_id_);
    }
    ~PacketLock() {
        if (locked_) {
            LeaseMgrFactory::instance().unlock<D>(txn_id_);
        }
    }
...
private:
    PacketLock() = delete;
    PacketLock(PacketLock const&) = delete;
    PacketLock& operator=(PacketLock const&) = delete;
