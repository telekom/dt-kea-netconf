...
Lease::Lease(const isc::asiolink::IOAddress& addr,
    ...
    : addr_(addr), valid_lft_(valid_lft), old_valid_lft_(valid_lft),
      cltt_(cltt), old_cltt_(cltt),
#ifdef TERASTREAM
      privacy_expire_(0), privacy_hash_(new PrivacyHash(0)),
#endif  // TERASTREAM
...
void
Lease::fromElementCommon(const LeasePtr& lease, const data::ElementPtr& element) {
    if (!element) {
    ...
    // Subnet identifier.
    ElementPtr subnet_id = element->get("subnet-id");
    ...
    // Hardware address.
    ElementPtr hw_address = element->get("hw-address");
    if (hw_address) {
    ...
    // cltt
    ElementPtr cltt = element->get("cltt");
    ...
    // valid lifetime
    ElementPtr valid_lifetime = element->get("valid-lft");
    ...
    // fqdn-fwd
    ElementPtr fqdn_fwd = element->get("fqdn-fwd");
    ...
    // fqdn-fwd
    ElementPtr fqdn_rev = element->get("fqdn-rev");
    ...
    // hostname
    ElementPtr hostname = element->get("hostname");
    ...
    // state
    ElementPtr state = element->get("state");
    ...
    // user context
    ElementPtr ctx = element->get("user-context");
    if (ctx) {
...
#ifdef TERASTREAM
        sw_4o6_src_address_ = other.sw_4o6_src_address_;
        privacy_expire_ = other.privacy_expire_;
        privacy_hash_.reset(new PrivacyHash(*other.privacy_hash_));
        privacy_history_ = other.privacy_history_;
#endif  // TERASTREAM
...
Lease4Ptr
Lease4::fromElement(const ElementPtr& element) {
    Lease4Ptr lease(new Lease4());

    // Extract common lease properties into the lease.
    fromElementCommon(std::dynamic_pointer_cast<Lease>(lease), element);
    ...
    ElementPtr client_id = element->get("client-id");
    if (client_id) {
...
Lease6::Lease6(Lease::Type type, const isc::asiolink::IOAddress& addr,
               ...
               const std::string& hostname, const HWAddrPtr& hwaddr,
               uint8_t prefixlen, time_t alloc_time)
...
Lease6::Lease6()
    : Lease(isc::asiolink::IOAddress("::"), 0, 0, 0, false, false, "",
            ...
            duid_(DuidPtr()), preferred_lft_(0), alloc_time_(time(nullptr)),
            pool_id_(-1) {
}
...
Lease6::toText() const {
    ...
    stream << "Type:          " << typeToText(type_) << "("
           ...
           << "Alloc time:    " << alloc_time_ << "\n"
           << "Pool id:       " << (pool_id_ != -1) ? to_string(pool_id_) + "\n" : "not found\n";
...
Lease4::operator==(const Lease4& other) const {
    return (nullOrEqualValues(hwaddr_, other.hwaddr_) &&
            ...
            sw_4o6_src_address_ == other.sw_4o6_src_address_ &&
            privacy_expire_ == other.privacy_expire_ &&
            nullOrEqualValues(privacy_hash_, other.privacy_hash_) &&
            privacy_history_ == other.privacy_history_ &&
#endif  // TERASTREAM
            hostname_ == other.hostname_ &&
            fqdn_fwd_ == other.fqdn_fwd_ &&
...
Lease6::toElement() const {
    ...
    map->set("fqdn-rev", Element::create(fqdn_rev_));
    map->set("hostname", Element::create(hostname_));
    map->set("allocation-time", Element::create(static_cast<long int>(alloc_time_)));
    map->set("pool-id", Element::create(pool_id_));
...
Lease6Ptr
Lease6::fromElement(const data::ElementPtr& element) {
    ...
    // lease type
    ElementPtr lease_type = element->get("type");
    ...
    // prefix length
    ElementPtr prefix_len = element->get("prefix-len");
    ...
    // IAID
    ElementPtr iaid = element->get("iaid");
    ...
    // DUID
    ElementPtr duid = element->get("duid");
    ...
    // preferred lifetime
    ElementPtr preferred_lft = element->get("preferred-lft");
    ...
    // allocation time
    ElementPtr alloc_time = element->get("allocation-time");
    if (!alloc_time || (alloc_time->getType() != Element::integer)) {
        isc_throw(BadValue, "allocation-time is not present in the parsed lease"
                  " or is not a number");
    }

    if (alloc_time->intValue() <= 0) {
        isc_throw(BadValue, "allocation_time " << alloc_time->intValue() << " is not a"
                  " positive integer in the parsed lease");
    }

    lease->alloc_time_ = static_cast<time_t>(alloc_time->intValue());

    // pool id
    ElementPtr pool_id = element->get("pool-id");
    if (!pool_id || (pool_id->getType() != Element::integer)) {
        isc_throw(BadValue, "pool-id not present in the parsed lease"
                  " or it is not a number");
    }
