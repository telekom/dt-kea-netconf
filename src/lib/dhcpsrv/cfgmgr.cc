...
Optional<std::string> CfgMgr::getDataDir() const {
    return (datadir_);
}

void CfgMgr::setDataDir(const std::string& datadir, bool unspecified) {
    datadir_ = Optional<std::string>(datadir, unspecified);
}

void CfgMgr::setD2ClientConfig(D2ClientConfigPtr& new_config) {
...
bool CfgMgr::ddnsEnabled() {
    return (d2_client_mgr_.ddnsEnabled());
}

const D2ClientConfigPtr& CfgMgr::getD2ClientConfig() const {
    return (d2_client_mgr_.getD2ClientConfig());
}

D2ClientMgr& CfgMgr::getD2ClientMgr() {
    return (d2_client_mgr_);
}

void CfgMgr::ensureCurrentAllocated() {
...
void CfgMgr::revert(const size_t index) {
    ensureCurrentAllocated();
    if (index == 0) {
...
SrvConfigPtr CfgMgr::getStagingCfg() {
    ensureCurrentAllocated();
    if (configuration_->sequenceEquals(*configs_.back())) {
...
SrvConfigPtr CfgMgr::createExternalCfg() {
    uint32_t seq = 0;
...
void CfgMgr::mergeIntoStagingCfg(const uint32_t seq) {
    mergeIntoCfg(getStagingCfg(), seq);
}

void CfgMgr::mergeIntoCurrentCfg(const uint32_t seq) {
    try {
...
void CfgMgr::mergeIntoCfg(const SrvConfigPtr& target_config, const uint32_t seq) {
...
CfgMgr::CfgMgr()
    : config_set_command_received_count_(0), datadir_(DHCP_DATA_DIR, true), d2_client_mgr_(),
      family_(AF_INET) {
