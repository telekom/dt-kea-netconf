...
void CfgHostsList::internalize(ElementPtr list) {
    if (!list) {
    ...
    for (size_t i = 0; i < list->size(); ++i) {
        ...
        ElementPtr resvs = item->get("reservations");
        if (!resvs) {
            isc_throw(BadValue, "internal error: CfgHostsList::internalize: "
...
ElementPtr CfgHostsList::get(SubnetID id) const {
