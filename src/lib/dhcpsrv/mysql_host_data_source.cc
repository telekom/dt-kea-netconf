...
public:
    ...
    virtual void processFetchedData(ConstHostCollection& hosts) {
        ...
        if (hosts.empty() || (hosts.back()->getHostId() != getHostId())) {
            ...
            // collection.
            hosts.push_back(HostPtr(retrieveHost()));
        }
    }
...
private:
    ...
    typedef std::shared_ptr<OptionProcessor> OptionProcessorPtr;
...
public:
    ...
    virtual void processFetchedData(ConstHostCollection& hosts) {
        ...
        if (!hosts.empty()) {
            ...
            most_recent_host = std::const_pointer_cast<Host>(hosts.back());
        }
...
public:
    ...
    virtual void processFetchedData(ConstHostCollection& hosts) {
        ...
        HostPtr host(std::const_pointer_cast<Host>(hosts.back()));
...
public:
    ...
    std::vector<MYSQL_BIND> createBindForSend(const OptionDescriptor& opt_desc,
        ...
        try {
            ...
            ElementPtr ctx = opt_desc.getContext();
            if (ctx) {
...
public:
    ...
    std::shared_ptr<MySqlHostWithOptionsExchange> host_ipv4_exchange_;

    /// @brief Pointer to an object representing an exchange which can
    /// be used to retrieve hosts, DHCPv6 options and IPv6 reservations.
    std::shared_ptr<MySqlHostIPv6Exchange> host_ipv6_exchange_;

    /// @brief Pointer to an object representing an exchange which can
    /// be used to retrieve hosts, DHCPv4 and DHCPv6 options, and
    /// IPv6 reservations using a single query.
    std::shared_ptr<MySqlHostIPv6Exchange> host_ipv46_exchange_;

    /// @brief Pointer to an object representing an exchange which can
    /// be used to insert new IPv6 reservation.
    std::shared_ptr<MySqlIPv6ReservationExchange> host_ipv6_reservation_exchange_;

    /// @brief Pointer to an object representing an exchange which can
    /// be used to insert DHCPv4 or DHCPv6 option into dhcp4_options
    /// or dhcp6_options table.
    std::shared_ptr<MySqlOptionExchange> host_option_exchange_;
...
typedef std::shared_ptr<MySqlHostContextPool> MySqlHostContextPoolPtr;
...
public:
    ...
    static StatementIndex const WRITE_STMTS_BEGIN = INSERT_HOST;
    ...
    /// @brief Truncates the host reservation table.
    void delAllHosts(MySqlHostContextPtr& ctx) const;
...
void
MySqlHostDataSourceImpl::delAllHosts(MySqlHostContextPtr& ctx) const {
    // Allocate a new statement.
    MYSQL_STMT *stmt = mysql_stmt_init(ctx->conn_.mysql_);
    if (stmt == NULL) {
        isc_throw(DbOperationError, "unable to allocate MySQL prepared "
                  "statement structure, reason: " << mysql_error(ctx->conn_.mysql_));
    }

    // Prepare the statement from SQL text.
    const char* truncate_sql = "TRUNCATE hosts";
    int status = mysql_stmt_prepare(stmt, truncate_sql, strlen(truncate_sql));
    if (status != 0) {
        mysql_stmt_close(stmt);
        isc_throw(DbOperationError, "unable to prepare MySQL statement <"
                  << truncate_sql << ">, reason: " << mysql_errno(ctx->conn_.mysql_));
    }
...
MySqlHostDataSource::getName() const {
    ...
    try {
        return ctx->conn_.getParameter("name");
    } catch (...) {
...
VersionTuple MySqlHostDataSource::getVersion() const {
...
MySqlHostDataSource::syncReservations() {
    // Get a context
    MySqlHostContextAlloc const get_context(*impl_);
    MySqlHostContextPtr ctx(get_context.ctx_);

    impl_->delAllHosts(ctx);
