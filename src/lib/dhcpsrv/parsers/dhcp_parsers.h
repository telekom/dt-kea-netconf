...
typedef std::shared_ptr<OptionStorage> OptionStoragePtr;
...
public:
    ...
    getPosition(const std::string& name, const data::ElementPtr parent =
                data::ElementPtr()) const {
...
typedef std::pair<std::string, isc::data::ElementPtr> ConfigPair;

/// @brief a collection of elements that store uint32 values
typedef ValueStorage<uint32_t> Uint32Storage;
typedef std::shared_ptr<Uint32Storage> Uint32StoragePtr;

/// @brief a collection of elements that store string values
typedef ValueStorage<std::string> StringStorage;
typedef std::shared_ptr<StringStorage> StringStoragePtr;

/// @brief Storage for parsed boolean values.
typedef ValueStorage<bool> BooleanStorage;
typedef std::shared_ptr<BooleanStorage> BooleanStoragePtr;
...
public:
    ...
    void parse(CfgMACSource& mac_sources, isc::data::ElementPtr value);
};
...
public:
    void sanityChecks(isc::data::ElementPtr const& value);

    /// @brief "Parses" control-socket structure
    ///
    /// Since the SrvConfig structure takes the socket definition
    /// as ElementPtr, there's really nothing to parse here.
    ...
    /// @param value pointer to the content of parsed values
    void parse(SrvConfig& srv_cfg, isc::data::ElementPtr value);

    void parseForNotifications(SrvConfig& srv_cfg, isc::data::ElementPtr const& value);
};
...
public:
    ...
    parse(isc::data::ElementPtr option_def);
...
public:
    ...
    void parse(CfgOptionDefPtr cfg, isc::data::ElementPtr def_list);
...
typedef std::shared_ptr<PoolStorage> PoolStoragePtr;
...
public:
    ...
    virtual void parse(PoolStoragePtr pools,
                       isc::data::ElementPtr pool_structure,
...
protected:
    ...
    virtual PoolPtr poolMaker(isc::asiolink::IOAddress &addr,
                              uint32_t len,
                              int32_t ptype = 0,
                              int8_t const threshold = 100) = 0;
...
protected:
    ...
    PoolPtr poolMaker(asiolink::IOAddress& addr,
                      uint32_t len,
                      int32_t ignored,
    ...
    /// PoolParser derivations. The V6 derivation requires a third value.
    /// @return returns a PoolPtr to the new Pool4 object.
    PoolPtr poolMaker(asiolink::IOAddress& min,
                      asiolink::IOAddress& max,
                      int32_t ignored,
                      int8_t const threshold = 100);
};
...
public:
    ...
    /// @throw isc::dhcp::DhcpConfigError when pool parsing fails
    virtual void parse(PoolStoragePtr pools,
                       isc::data::ElementPtr pools_list) = 0;
};
...
public:
    ...
    /// @param pools_list a list of pool structures
    /// @throw isc::dhcp::DhcpConfigError when pool parsing fails
    void parse(PoolStoragePtr pools, data::ElementPtr pools_list);
};
...
public:
    ...
    void parse(const isc::dhcp::Network::RelayInfoPtr& relay_info,
               isc::data::ElementPtr relay_elem);
...
protected:
    ...
    SubnetPtr parse(isc::data::ElementPtr subnet);
    ...
    /// @param len is the prefix length
    virtual void initSubnet(isc::data::ElementPtr params,
                            isc::asiolink::IOAddress addr, uint8_t len) = 0;

    /// @brief Attempts to convert text representation to AllocationType enum.
    ///
    /// Allowed values are "default", "privacy".
    /// See Subnet::AllocationType for their exact meaning.
    ///
    /// @param txt Allocation Type in the textual form.
    ///
    /// @throw BadValue if the text cannot be converted.
    ///
    /// @return one of allowed AllocationType values
    static Subnet::AllocationType allocationTypeFromText(const std::string& txt);
...
private:
    ...
    /// failed.
    void createSubnet(isc::data::ElementPtr data);
...
public:
    ...
    Subnet4Ptr parse(data::ElementPtr subnet);
...
protected:
    ...
    /// @param len is the prefix length
    void initSubnet(data::ElementPtr params,
...
public:
    ...
    size_t parse(SrvConfigPtr cfg, data::ElementPtr subnets_list);
...
protected:
    ...
    /// @return returns a PoolPtr to the new Pool4 object.
    PoolPtr
    poolMaker(asiolink::IOAddress& addr, uint32_t len, int32_t ptype, int8_t const threshold = 100);
...
public:
    ...
    void parse(PoolStoragePtr pools, data::ElementPtr pd_pool_);
...
public:
    ...
    ///
    /// @throw DhcpConfigError if configuration parsing fails.
    void parse(PoolStoragePtr pools, data::ElementPtr pd_pool_list);
};
...
public:
    ...
    Subnet6Ptr parse(data::ElementPtr subnet);
...
protected:
    ...
    /// @param len is the prefix length
    void initSubnet(isc::data::ElementPtr params,
...
public:
    ...
    D2ClientConfigPtr parse(isc::data::ElementPtr d2_client_cfg);
    ...
    static size_t setAllDefaults(isc::data::ElementPtr d2_config);
...
private:
    ...
    getProtocol(isc::data::ElementPtr scope, const std::string& name);
    ...
    getFormat(isc::data::ElementPtr scope, const std::string& name);
    ...
    getMode(isc::data::ElementPtr scope, const std::string& name);
};
