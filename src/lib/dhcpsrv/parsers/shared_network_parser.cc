...
SharedNetwork4Ptr
SharedNetwork4Parser::parse(const data::ElementPtr& shared_network_data) {
    ...
    try {
        ...
        NetworkPtr network = std::dynamic_pointer_cast<Network>(shared_network);
        ...
        if (shared_network_data->contains("next-server")) {
            ...
            try {
                ...
                ElementPtr next = shared_network_data->get("next-server");
                std::string pos;
                if (next) {
        ...
        if (shared_network_data->contains("server-hostname")) {
            ...
            if (!sname.empty()) {
                if (sname.length() >= Pkt4::MAX_SNAME_LEN) {
                    ElementPtr error = shared_network_data->get("server-hostname");
        ...
        if (shared_network_data->contains("boot-file-name")) {
            ...
            if (!filename.empty()) {
                if (filename.length() > Pkt4::MAX_FILE_LEN) {
                    ElementPtr error = shared_network_data->get("boot-file-name");
        ...
        ElementPtr user_context = shared_network_data->get("user-context");
        if (user_context) {
    ...
    // for each shared network to allow for fetching global parameters.
     shared_network->setFetchGlobalsFn([]() -> ElementPtr {
...
SharedNetwork6Ptr
SharedNetwork6Parser::parse(const data::ElementPtr& shared_network_data) {
    ...
    try {
        ...
        if (!iface.unspecified() && !iface.empty()) {
            if (check_iface_ && !IfaceMgr::instance().getIface(iface)) {
                ElementPtr error = shared_network_data->get("interface");
                isc_throw(DhcpConfigError,
                          "Specified network interface name " << iface
                          << " for shared network " << name
                          << " is not present in the system ("
