...
public:
    ...
    /// @param lock mutex used to synchronize writing and reading from the packet queue with a
    ///     reverse lock paradigm
    ///
    /// @throw DhcpConfigError if the interface names and/or addresses
    /// are invalid.
    void parse(const CfgIfacePtr& config, const isc::data::ElementPtr& values);
...
private:
    ...
    void parseInterfacesList(const CfgIfacePtr& cfg_iface,
                             isc::data::ElementPtr ifaces_list);
