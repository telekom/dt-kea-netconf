...
OptionDataParser::parse(isc::data::ElementPtr single_option) {
...
Optional<uint32_t>
OptionDataParser::extractCode(ElementPtr parent) const {
    uint32_t code;
    try {
...
Optional<std::string>
OptionDataParser::extractName(ElementPtr parent) const {
    std::string name;
    try {
...
std::string
OptionDataParser::extractData(ElementPtr parent) const {
    std::string data;
    try {
...
Optional<bool>
OptionDataParser::extractCSVFormat(ElementPtr parent) const {
...
std::string
OptionDataParser::extractSpace(ElementPtr parent) const {
...
Optional<bool>
OptionDataParser::extractPersistent(ElementPtr parent) const {
    bool persist = false;
    try {
...
std::pair<OptionDescriptor, std::string>
OptionDataParser::createOption(ElementPtr option_data) {
    ...
    ElementPtr user_context = option_data->get("user-context");
...
void OptionDataListParser::parse(const CfgOptionPtr& cfg,
                                 isc::data::ElementPtr option_data_list) {
    OptionDataParser option_parser(address_family_, cfg_option_def_);
    BOOST_FOREACH(ElementPtr data, option_data_list->listValue()) {
