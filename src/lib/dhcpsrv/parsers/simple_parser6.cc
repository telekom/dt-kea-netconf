...
const SimpleKeywords SimpleParser6::SUBNET6_PARAMETERS = {
    ...
    { "metadata",                       Element::map },
    // DT-specific
    { "network-description",            Element::string },
};
...
const SimpleKeywords SimpleParser6::PD_POOL6_PARAMETERS = {
    ...
    { "metadata",                   Element::map },
    { "id",                         Element::integer },
    { "max-pd-space-utilization",   Element::integer },
};
...
size_t SimpleParser6::setAllDefaults(ElementPtr global) {
    ...
    ElementPtr subnets = global->get("subnet6");
    if (subnets) {
    ...
    if (ifaces_cfg) {
        cnt += setDefaults(ifaces_cfg, IFACE6_DEFAULTS);
    }
    ...
    if (shared) {
        BOOST_FOREACH(ElementPtr net, shared->listValue()) {
            ...
            ElementPtr subs = net->get("subnet6");
            if (subs) {
