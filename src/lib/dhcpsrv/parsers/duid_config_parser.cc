...
DUIDConfigParser::parse(const CfgDUIDPtr& cfg,
                        isc::data::ElementPtr duid_configuration) {
    if (!cfg) {
        // Sanity check
    ...
    try {
        ...
        } else if (duid_type == "UUID") {
            numeric_type = DUID::DUID_UUID;
        } else {
            isc_throw(BadValue, "unsupported DUID type '"
                      << duid_type << "'. Expected: LLT, EN, LL or UUID");
        }
        ...
        param = "user-context";
        ElementPtr user_context = duid_configuration->get("user-context");
        if (user_context) {
