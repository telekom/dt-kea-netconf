...
Pkt6Ptr Dhcp4o6IpcBase::receive() {
    ...
    for (OptionCollection::const_iterator opt = vendor_options.begin();
        ...
        option_vendor = std::dynamic_pointer_cast<OptionVendor>(opt->second);
        if (option_vendor) {
    ...
    OptionStringPtr ifname = std::dynamic_pointer_cast<
    ...
    OptionCustomPtr srcs = std::dynamic_pointer_cast<
    ...
    OptionUint16Ptr sport = std::dynamic_pointer_cast<
...
void Dhcp4o6IpcBase::send(const Pkt6Ptr& pkt) {
    ...
    OptionVendorPtr option_vendor = std::dynamic_pointer_cast<
