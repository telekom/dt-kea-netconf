...
std::string Network::hostReservationModeToString(HRMode value) {
    static std::unordered_map<HRMode, std::string> map{{HR_DISABLED, "disabled"},
...
Network::HRMode Network::stringToHostReservationMode(std::string const& value) {
    static std::unordered_map<std::string, HRMode> map{{"disabled", HR_DISABLED},
                                                       {"out-of-pool", HR_OUT_OF_POOL},
                                                       {"all", HR_ALL},
                                                       {"global", HR_GLOBAL}};
    return map.at(value);
}
...
Network::getGlobalProperty(Optional<IOAddress> property,
    ...
    if (!global_name.empty() && fetch_globals_fn_) {
        ElementPtr globals = fetch_globals_fn_();
        if (globals && (globals->getType() == Element::map)) {
            ElementPtr global_param = globals->get(global_name);
            if (global_param) {
...
Network::toElement() const {
    ...
    if (!hrmode.unspecified()) {
        std::string mode = Network::hostReservationModeToString(hrmode.get());
