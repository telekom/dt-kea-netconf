...
private:
    ...
    bool loadLeasesFromFiles(const std::string& filename,
                             std::shared_ptr<LeaseFileType>& lease_file,
                             StorageType& storage);
    ...
    std::shared_ptr<CSVLeaseFile4> lease_file4_;

    /// @brief Holds the pointer to the DHCPv6 lease file IO.
    std::shared_ptr<CSVLeaseFile6> lease_file6_;
...
private:
    ...
    void lfcExecute(std::shared_ptr<LeaseFileType>& lease_file);

    /// @brief A pointer to the Lease File Cleanup configuration.
    std::unique_ptr<LFCSetup> lfc_setup_;
