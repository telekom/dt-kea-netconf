...
#ifndef SHARD_CONFIG_MGR_FACTORY_H
#define SHARD_CONFIG_MGR_FACTORY_H
...
#endif
#ifdef HAVE_MYSQL
#include <dhcpsrv/mysql_shard_config_mgr.h>
#endif
#ifdef HAVE_PGSQL
#include <dhcpsrv/pgsql_shard_config_mgr.h>
#endif
...
struct NoShardConfigManager : public Exception {
    NoShardConfigManager(const char* file, size_t line, const char* what)
        : isc::Exception(file, line, what) {
    }
};

/// @brief shard configuration manager factory
///
...
struct ShardConfigMgrFactory {
    ...
    static void create(const std::string& dbaccess) {
        ...
        std::string redacted(isc::db::DatabaseConnection::redactedAccessString(parameters));
...
// Yes, check what it is.
        ...
        if (parameters[type] == "cql") {
            ...
            getConfigurationMgrPtr() =
                std::make_shared<CqlShardConfigMgr<ShardConfigT<D>>>(parameters);
            return;
        }
#endif
#ifdef HAVE_MYSQL
        if (parameters[type] == "mysql") {
            ...
            getConfigurationMgrPtr() = std::make_shared<MySqlShardConfigMgr<D>>(parameters);
            return;
        }
#endif
#ifdef HAVE_PGSQL
        if (parameters[type] == "postgresql") {
            ...
            getConfigurationMgrPtr() = std::make_shared<PgSqlShardConfigMgr<D>>(parameters);
            return;
        }
#endif
        ...
        isc_throw(isc::db::InvalidType,
                  "Database access parameter 'type' does not specify a supported database backend: "
                      << parameters[type]);
    }
    ...
    static ShardConfigMgr& instance() {
        std::shared_ptr<ShardConfigMgr> cfg_mgr_ptr(getConfigurationMgrPtr());
        if (cfg_mgr_ptr.get() == nullptr) {
            isc_throw(NoShardConfigManager, "no current server configuration manager is available");
        }
        return *cfg_mgr_ptr;
    }
...
private:
    ...
    static std::shared_ptr<ShardConfigMgr>& getConfigurationMgrPtr() {
        static std::shared_ptr<ShardConfigMgr> configuration_mgr_ptr;
...
#endif  // SHARD_CONFIG_MGR_FACTORY_H
