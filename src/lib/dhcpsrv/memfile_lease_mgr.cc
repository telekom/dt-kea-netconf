...
public:
    ...
    void setup(const uint32_t lfc_interval,
               const std::shared_ptr<CSVLeaseFile4>& lease_file4,
               const std::shared_ptr<CSVLeaseFile6>& lease_file6,
...
private:
    ...
    /// the LFC.
    std::unique_ptr<util::ProcessSpawn> process_;
...
Memfile_LeaseMgr::updateLease6Internal(const Lease6Ptr& lease) {
    ...
    // remain consistent.
    if (persistLeases(V6)) {
        lease_file6_->append(*lease, (*lease_it)->alloc_time_);
    }

    lease->alloc_time_ = (*lease_it)->alloc_time_;
...
Memfile_LeaseMgr::deleteLeaseInternal(const Lease6Ptr& lease) {
    ...
    if (l == storage6_.end()) {
        ...
        if (persistLeases(V6)) {
            ...
            lease_file6_->append(lease_copy, (*l)->alloc_time_);
        }
        storage6_.erase(l);
        return (true);
    }
...
void Memfile_LeaseMgr::lfcExecute(std::shared_ptr<LeaseFileType>& lease_file) {
    bool do_lfc = true;
...
bool
Memfile_LeaseMgr::lock4(std::vector<uint8_t>&) {
    return true;
}

bool
Memfile_LeaseMgr::unlock4(std::vector<uint8_t>&) {
    return true;
}

bool
Memfile_LeaseMgr::lock6(std::vector<uint8_t>&) {
    return true;
}

bool
Memfile_LeaseMgr::unlock6(std::vector<uint8_t>&) {
    return true;
}
