...
public:
    ...
    virtual void processRowData(ConstHostCollection& hosts,
        ...
        if (hosts.empty()) {
            ...
            HostID row_host_id = getHostId(r, row);
            current_host = std::const_pointer_cast<Host>(hosts.back());
...
public:
    ...
    std::shared_ptr<PgSqlHostWithOptionsExchange> host_ipv4_exchange_;
    ...
    std::shared_ptr<PgSqlHostIPv6Exchange> host_ipv6_exchange_;

    /// @brief Pointer to an object representing an exchange which can
    /// be used to retrieve hosts, DHCPv4 and DHCPv6 options, and
    /// IPv6 reservations using a single query.
    std::shared_ptr<PgSqlHostIPv6Exchange> host_ipv46_exchange_;

    /// @brief Pointer to an object representing an exchange which can
    /// be used to insert new IPv6 reservation.
    std::shared_ptr<PgSqlIPv6ReservationExchange> host_ipv6_reservation_exchange_;

    /// @brief Pointer to an object representing an exchange which can
    /// be used to insert DHCPv4 or DHCPv6 option into dhcp4_options
    /// or dhcp6_options table.
    std::shared_ptr<PgSqlOptionExchange> host_option_exchange_;
...
typedef std::shared_ptr<PgSqlHostContextPool> PgSqlHostContextPoolPtr;
...
public:
    ...
    void delAllHosts(PgSqlHostContextPtr& ctx) const;
    ...
    ///         minor number.
    ///
    /// @throw isc::DbOperationError An operation on the open database
    ///        has failed.
...
void
PgSqlHostDataSourceImpl::delAllHosts(PgSqlHostContextPtr& ctx) const {
    const char* truncate_sql = "TRUNCATE hosts;";
    PgSqlResult r(PQexec(ctx->conn_, truncate_sql));
    if(PQresultStatus(r) != PGRES_TUPLES_OK) {
        isc_throw(DbOperationError, "unable to execute PostgreSQL statement <"
                  << truncate_sql << ">, reason: " << PQerrorMessage(ctx->conn_));
    }
}
...
PgSqlHostDataSource::syncReservations() {
    // Get a context
    PgSqlHostContextAlloc const& get_context(*impl_);
    PgSqlHostContextPtr ctx(get_context.ctx_);
