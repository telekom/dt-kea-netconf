...
std::unique_ptr<HostMgr>&
HostMgr::getHostMgrPtr() {
    static std::unique_ptr<HostMgr> host_mgr_ptr;
    return (host_mgr_ptr);
}
...
HostMgr::checkCacheBackend(bool logging) {
    ...
    CacheHostDataSourcePtr cache_ptr =
        std::dynamic_pointer_cast<CacheHostDataSource>(sources[0]);
    if (cache_ptr) {
...
HostMgr&
HostMgr::instance() {
    std::unique_ptr<HostMgr>& host_mgr_ptr = getHostMgrPtr();
    if (!host_mgr_ptr) {
        create();
    }
    return (*host_mgr_ptr);
