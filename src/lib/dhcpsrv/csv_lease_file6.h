...
public:
    ...
    /// @param alloc_time Lease allocation time, 0 if lease is appended for
    /// the 1st time
    /// @throw BadValue if the lease to be written has an empty DUID and is
    /// whose state is not STATE_DECLINED.
    void append(const Lease6& lease, time_t alloc_time = 0);
...
private:
    ...
    ///
    /// @param row CSV file row holding lease information.
    data::ElementPtr readContext(const util::CSVRow& row);

    /// @brief Reads allocation value from the CSV file row.
    ///
    /// @param row CSV file row holding lease information.
    time_t readAllocation(const util::CSVRow& row);

    /// @brief Reads pool-id value from the CSV file row.
    ///
    /// @param row CSV file row holding lease information.
    int64_t readPoolID(const util::CSVRow& row);
    //@}
