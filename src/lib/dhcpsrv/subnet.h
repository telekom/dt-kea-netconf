...
using PSIDContainer = std::set<uint16_t>;

struct Subnet : public virtual Network {
    /// @brief Specifies Address Allocation type.
    ///
    typedef enum  {
        /// default address allocation
        ALLOCATION_TYPE_DEFAULT,
        /// address allocation privacy: change IP according to privacy settings
        ALLOCATION_TYPE_PRIVACY,
    ...
    /// @brief Resets subnet-id counter to its initial value (0).
    ///
    ...
    void getSharedNetwork(SharedNetworkPtrType& shared_network) const {
        shared_network = std::dynamic_pointer_cast<
    ...
    /// @brief Attempts to convert AllocationType enum to text representation.
    ///
    /// Allowed values are "default", "privacy".
    /// See Subnet::AllocationType for their exact meaning.
    ///
    /// @param value Allocation Type enum.
    ///
    /// @throw BadValue if the value cannot be converted.
    ///
    /// @return one of allowed AllocationType values in text representation
    static std::string allocationTypeToString(Subnet::AllocationType value);

    static Subnet::AllocationType stringToAllocationType(std::string);

    /// @brief Specifies what type of Address Allocation is used.
    ///
    /// @return the type of address allocation type is used.
    AllocationType getAllocationType() const {
        return (allocation_type_);
    }

    /// @brief Sets Address Allocation type.
    ///
    /// @param type allocation type to be set
    void setAllocationType(AllocationType type) {
        allocation_type_ = type;
    }

    /// @brief Sets subnet-id counter to specific value (id)
    ///
    /// This should be called during reconfiguration, before any new
    /// subnet objects are created. It will ensure that the subnet_id will
    /// be consistent between reconfigures.
    ///
    /// @param id subnet-id value to be set
    static void setSubnetID(SubnetID id) {
        static_id_ = id;
    }

    /// @brief Returns a flag indicating if the subnet-id has automatically
    /// been generated
    ///
    /// @return true if the subnet id is generated, false otherwise
...
private:
    /// @brief Specifies if the subnet-id has automatically been generated
    bool generated_;
    ...
    /// @brief Specifies address allocation type
    ///
    /// See @ref AllocationType type for details.
...
typedef std::shared_ptr<const Subnet4> ConstSubnet4Ptr;
...
public:
    ...
    PSIDContainer const& getExcludedPSIDs() const {
        return (excludedPSIDs_);
    }
    ...
    /// @brief Sets container with excluded PSIDs.
    ///
    /// Will be used for v4-excluded-psids field (may be empty if not defined)
    void setExcludedPSIDs(PSIDContainer& excludedPSIDs) {
        excludedPSIDs_ = excludedPSIDs;
    }

    static std::pair<uint8_t, uint8_t> getPSIDOffsetAndLength(SubnetPtr subnet) {
        uint8_t psid_offset = 0;
        uint8_t psid_length = 0;
        if (subnet->get().first.isV4()) {
            Subnet4Ptr const& subnet4(std::dynamic_pointer_cast<Subnet4>(subnet));
            if (subnet4->get4o6().enabled()) {
                psid_offset = subnet4->get4o6().getPsidOffset();
                psid_length = subnet4->get4o6().getPsidLen();
            }
        }

        return std::make_pair(psid_offset, psid_length);
    }
...
private:
    ...
    PSIDContainer excludedPSIDs_;
};

class Subnet6;

/// @brief A const pointer to a @c Subnet6 object.
typedef std::shared_ptr<const Subnet6> ConstSubnet6Ptr;

/// @brief A pointer to a Subnet6 object
typedef std::shared_ptr<Subnet6> Subnet6Ptr;
...
struct adapter_SubnetCollection {};

template <>
struct adapter_SubnetCollection<DHCP_SPACE_V4> {
    using type = Subnet4Collection;
};

template <>
struct adapter_SubnetCollection<DHCP_SPACE_V6> {
    using type = Subnet6Collection;
};

}  // namespace

template <DhcpSpaceType D>
using SubnetCollection = typename adapter_SubnetCollection<D>::type;

template <DhcpSpaceType D>
using SubnetCollectionPtr = std::shared_ptr<SubnetCollection<D>>;
...
template <>
struct adapter_Subnet<DHCP_SPACE_V4> {
    using type = Subnet4;
};

template <>
struct adapter_Subnet<DHCP_SPACE_V6> {
    using type = Subnet6;
};
...
template <DhcpSpaceType D>
using SubnetTPtr = std::shared_ptr<SubnetT<D>>;
//@}
