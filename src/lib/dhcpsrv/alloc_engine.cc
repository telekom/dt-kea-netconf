...
AllocEngine::IterativeAllocator::increaseAddress(const isc::asiolink::IOAddress& address,
                                                 bool prefix,
                                                 const uint8_t prefix_len,
                                                 uint8_t const psid_offset,
                                                 uint8_t const psid_len) {
    if (!prefix) {
...
AllocEngine::IterativeAllocator::pickAddressInternal(const SubnetPtr& subnet,
    ...
    if (first == pools.end()) {
        isc_throw(std::bind(&NoPools::create, std::placeholders::_1, std::placeholders::_2,
                            std::placeholders::_3, not_in_range),
    ...
    for (;;) {
        ...
        if (valid) {
            ...
            if (prefix) {
                Pool6Ptr pool6 = std::dynamic_pointer_cast<Pool6>(*it);
...
AllocEngine::HashedAllocator::pickAddressInternal(const SubnetPtr&,
                                                  ...
                                                  const DuidPtr&,
                                                  const IOAddress&,
                                                  [[maybe_unused]] uint8_t const psid_offset,
                                                  [[maybe_unused]] uint8_t const psid_len) {
    isc_throw(NotImplemented, "Hashed allocator is not implemented");
}
...
void AllocEngine::findReservation(ClientContext6& ctx) {
    ...
    if (use_single_query) {
        for (auto id_pair : ctx.host_identifiers_) {
            ...
            // to eliminate them.
            for (ConstHostPtr const& host : hosts) {
...
#ifdef TERASTREAM
        removeNonmatchingPoolLeases6(ctx, leases);
        applyPrivacy(ctx, leases, ctx.private_lease_);
#endif  // TERASTREAM
...
AllocEngine::allocateUnreservedLeases6(ClientContext6& ctx) {
    ...
    for (; subnet; subnet = subnet->getNextSubnet(original_subnet)) {
        ...
        pool = std::dynamic_pointer_cast<Pool6>
...
#ifdef TERASTREAM
            if (!isReservedPrivateAddress(ctx, hint)) {
#endif  // TERASTREAM
...
#ifdef TERASTREAM
            if (ctx.subnet_->getAllocationType() == Subnet::ALLOCATION_TYPE_DEFAULT) {
#endif  // TERASTREAM
...
#ifdef LEASE_LOOKUP_BY_ADDRESS_FIRST
        } else {
            leases.push_back(lease);
            return leases;
#endif  // LEASE_LOOKUP_BY_ADDRESS_FIRST
        }
    }
...
#ifdef TERASTREAM
            if (isReservedPrivateAddress(ctx, candidate)) {
                continue;
            }
#endif  // TERASTREAM
...
Lease6Ptr AllocEngine::createLease6(ClientContext6& ctx,
    ...
    Pool6Ptr pool = std::dynamic_pointer_cast<
        Pool6>(ctx.subnet_->getPool(lease->type_, lease->addr_, false));
    lease->pool_id_ = pool ? pool->getId() : -1;

#ifdef TERASTREAM
    lease->privacy_expire_ = lease->cltt_ + CfgMgr::instance().getCurrentCfg()->getPrivacyValidLifetime();
    if (ctx.subnet_->getAllocationType() != Subnet::ALLOCATION_TYPE_DEFAULT) {
        uint32_t count = CfgMgr::instance().getCurrentCfg()->getPrivacyHistorySize();
        if (count && ctx.private_lease_) {
            std::vector<std::string> history;
            lease->privacy_history_ = ctx.private_lease_->addr_.toText();
            boost::split(history, ctx.private_lease_->privacy_history_, boost::is_any_of(" "));
            count--;
            for (std::vector<std::string>::const_iterator it = history.begin(); it != history.end() && count; ++it) {
                lease->privacy_history_.append(" ");
                lease->privacy_history_.append(*it);
                count--;
            }
        }
    }
...
#ifdef TERASTREAM
            if (ctx.private_lease_) {
                LeaseMgrFactory::instance().deleteLease(ctx.private_lease_);
            }
#endif  // TERASTREAM
...
#ifdef LEASE_LOOKUP_BY_ADDRESS_FIRST
void findClientLeaseByAddress(AllocEngine::ClientContext4& ctx, Lease4Ptr& client_lease) {
    ...
    // Client identifier is optional. First check if we can try to lookup
    // by client-id.
    bool try_clientid_check = (ctx.clientid_ &&
        SharedNetwork4::subnetsIncludeMatchClientId(original_subnet,
                                                    ctx.query_->getClasses()));
...
#ifdef TERASTREAM
void findClientLeases(const AllocEngine::ClientContext4& ctx,  Lease4Collection& leases) {
    LeaseMgr& lease_mgr = LeaseMgrFactory::instance();
    ...
    if (ctx.clientid_) {
        leases = lease_mgr.getLease4(*ctx.clientid_);
    }

    // If no lease found using the client identifier, try the lookup using
    // the HW address.
    if (leases.empty() && ctx.hwaddr_) {
        leases = lease_mgr.getLease4(*ctx.hwaddr_);
        ...
        for (Lease4Collection::iterator lease = leases.begin();
             lease != leases.end(); ++lease) {
            Lease4Ptr client_lease = *lease;
            if (!client_lease->belongsToClient(ctx.hwaddr_, ctx.clientid_)) {
                client_lease.reset();
            }
        }
    }
    leases.erase(std::remove(leases.begin(), leases.end(), Lease4Ptr()), leases.end());
}
#endif  // TERASTREAM
...
AllocEngine::requestLease4(AllocEngine::ClientContext4& ctx) {
    ...
    Lease4Ptr client_lease;
#ifdef LEASE_LOOKUP_BY_ADDRESS_FIRST
    findClientLeaseByAddress(ctx, client_lease);
    if (!client_lease) {
#endif  // LEASE_LOOKUP_BY_ADDRESS_FIRST
    findClientLease(ctx, client_lease);
#ifdef LEASE_LOOKUP_BY_ADDRESS_FIRST
    ...
    if (!ctx.requested_address_.isV4Zero()) {
#ifdef LEASE_LOOKUP_BY_ADDRESS_FIRST
        Lease4Ptr existing(client_lease);
#else  // LEASE_LOOKUP_BY_ADDRESS_FIRST
...
AllocEngine::createLease4(const ClientContext4& ctx, const IOAddress& addr,
    ...
    if (ctx.query_->inClass("BOOTP")) {
        ...
        if (opt) {
            opt_lft = std::dynamic_pointer_cast<OptionInt<uint32_t>>(opt);
        }
        if (opt_lft) {
    ...
    if (ctx.callout_handle_ &&
        ...
        Subnet4Ptr subnet4 = std::dynamic_pointer_cast<Subnet4>(ctx.subnet_);
...
AllocEngine::renewLease4(const Lease4Ptr& lease,
    ...
    /// @todo: remove this?
    Lease4Ptr old_values = std::make_shared<Lease4>(*lease);
...
AllocEngine::allocateOrReuseLease4(const IOAddress& candidate, ClientContext4& ctx,
    ...
    Lease4Collection leases;
    if (ctx.subnet_->getAllocationType() != Subnet::ALLOCATION_TYPE_DEFAULT) {
        findClientLeases(ctx, leases);
        applyPrivacy(ctx, leases, ctx.private_lease_);
        if (leases.size()) {
            exist_lease = *leases.begin();
        } else if (exist_lease) {
            return (Lease4Ptr());
        }
    }
...
AllocEngine::allocateUnreservedLease4(ClientContext4& ctx) {
    ...
    while (subnet) {
        ...
        for (uint64_t i = 0; i < max_attempts; ++i) {
            ...
            IOAddress candidate(IOAddress::IPV4_ZERO_ADDRESS());
            try {
                candidate = allocator->pickAddress(subnet, ctx.query_->getClasses(), client_id,
                                                   ctx.requested_address_);
            } catch (NoPools const& exception) {
                std::string notification;
                if (allocator->type() == Lease::TYPE_NA) {
                    notification = "address-pool-utilization-threshold-exceeded";
                } else if (allocator->type() == Lease::TYPE_PD) {
                    notification = "prefix-pool-utilization-threshold-exceeded";
                }
                if (!notification.empty()) {
                    for (PoolPtr p : exception.pools_) {
                        NotificationsManager n;
                        n.notify(notification, p->toNotificationElement());
                    }
                }
                throw;
            }
            ...
            if (!exist_lease) {
                ...
                if ((check_reservation_first || !addressReserved(candidate, ctx)) && !ctx.subnet_->isExcludedAddress(candidate)) {
                    // Not reserved use it.
...
#ifdef TERASTREAM
void AllocEngine::removeNonmatchingPoolLeases6(ClientContext6& ctx,
                                               Lease6Collection& existing_leases) {
    // This method removes leases that are not reserved for this subnet.
    if (existing_leases.empty() || !ctx.subnet_) {
        return;
    }
...
template <typename ContextType, typename LeaseCollection, typename LeasePtrType>
void AllocEngine::applyPrivacy(ContextType& ctx,
                               LeaseCollection& existing_leases,
                               LeasePtrType& private_lease) {
    if (existing_leases.empty() || !ctx.subnet_) {
        return;
    }
    ...
    time_t now = time(NULL);
    LeasePtrType candidate;

    typedef typename LeaseCollection::iterator LeaseCollectionIt;
    for (LeaseCollectionIt lease = existing_leases.begin(); lease != existing_leases.end();
         ++lease) {
        if (private_lease) {
            if (private_lease->getExpirationTime() < (*lease)->getExpirationTime()) {
                private_lease = *lease;
            }
        } else {
            private_lease = *lease;
    ...
    if (private_lease) {
        if (private_lease->privacy_hash_ && !private_lease->privacy_hash_->empty()) {
            equal_hashes =
                util::nullOrEqualValues(private_lease->privacy_hash_, ctx.query_->getPrivacyHash());
        }

        if ((private_lease->privacy_expire_ >= now) ||
            (CfgMgr::instance().getCurrentCfg()->getPrivacyValidLifetime() == 0)) {
            time_expired = false;
        }

        if (!time_expired && equal_hashes) {
            candidate = private_lease;
        }
    }
    ...
    // Remove all elements that we previously marked for deletion (those that
    // have NULL value).
    existing_leases.erase(
        std::remove(existing_leases.begin(), existing_leases.end(), LeasePtrType()),
        existing_leases.end());
}

template <typename ContextType>
bool AllocEngine::isReservedPrivateAddress(ContextType& ctx,
                                            const isc::asiolink::IOAddress& current_lease) {
    if (ctx.private_lease_) {
        std::vector<std::string> history;
        boost::split(history, ctx.private_lease_->privacy_history_, boost::is_any_of(" "));
        uint32_t count = CfgMgr::instance().getCurrentCfg()->getPrivacyHistorySize();
        std::string candidate_str = current_lease.toText();
        for (std::vector<std::string>::const_iterator it = history.begin();
             it != history.end() && count; ++it) {
            if (*it == candidate_str) {
                return true;
            }
            count--;
