...
TEST_F(CfgOptionTest, replace) {
    ...
    OptionStringPtr opstr = std::dynamic_pointer_cast<OptionString>(desc.option_);
    ASSERT_TRUE(opstr);
    EXPECT_EQ("one", opstr->getValue());

    desc = cfg.get("isc", 2);
    ASSERT_TRUE(desc.option_);
    opstr = std::dynamic_pointer_cast<OptionString>(desc.option_);
    ASSERT_TRUE(opstr);
...
TEST_F(CfgOptionTest, validMerge) {
    ...
    ASSERT_TRUE(desc.option_);
    OptionUint8Ptr opint = std::dynamic_pointer_cast<OptionUint8>(desc.option_);
    ASSERT_TRUE(opint);
    ...
    ASSERT_TRUE(desc.option_);
    opint = std::dynamic_pointer_cast<OptionUint8>(desc.option_);
    ASSERT_TRUE(opint);
...
TEST_F(CfgOptionTest, createDescriptorOptionValid) {
    ...
    ASSERT_NO_THROW(updated = CfgOption::createDescriptorOption(defs, space, *desc));
    ASSERT_TRUE(updated);
    OptionCustomPtr opcustom = std::dynamic_pointer_cast<OptionCustom>(desc->option_);
    ...
    ASSERT_NO_THROW(updated = CfgOption::createDescriptorOption(defs, space, *desc));
    ASSERT_TRUE(updated);
    Option4AddrLstPtr opaddrs = std::dynamic_pointer_cast<Option4AddrLst>(desc->option_);
    ASSERT_TRUE(opaddrs);
    ...
    ASSERT_NO_THROW(updated = CfgOption::createDescriptorOption(defs, space, *desc));
    ASSERT_TRUE(updated);
    OptionUint8ArrayPtr oparray = std::dynamic_pointer_cast<OptionUint8Array>(desc->option_);
    ASSERT_TRUE(oparray);
...
TEST_F(CfgOptionTest, encapsulate) {
    ...
    for (uint16_t code = 1000; code < 1040; ++code) {
        ...
        OptionUint16Ptr option = std::dynamic_pointer_cast<
        ...
        BOOST_FOREACH(first_level_opt, first_level) {
            ...
            OptionUint8Ptr first_level_uint8 = std::dynamic_pointer_cast<
            ...
            BOOST_FOREACH(second_level_opt, second_level) {
                OptionUint8Ptr second_level_uint8 = std::dynamic_pointer_cast<
