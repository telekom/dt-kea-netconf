...
TEST_F(SrvConfigTest, hooksLibraries) {
    ...
    ElementPtr elem0;
    libraries.add("foo", "parameters", elem0);
    std::string config = "{ \"library\": \"bar\" }";
    ElementPtr elem1 = Element::fromJSON(config);
    libraries.add("bar", "parameters", elem1);
...
TEST_F(SrvConfigTest, configuredGlobals) {
    ...
    ElementPtr srv_globals = conf.getConfiguredGlobals();
...
TEST_F(SrvConfigTest, unparseHR) {
    ...
    // Unparse the config
    ElementPtr unparsed4 = conf4.toElement();
    ...
    // Unparse the config
    ElementPtr unparsed6 = conf6.toElement();
...
TEST_F(SrvConfigTest, moveDdnsParamsTest) {
    ...
    for (auto scenario : scenarios) {
        ...
        ElementPtr exp_cfg;
        {
            ...
            ASSERT_NO_THROW(input_cfg = Element::fromJSON(scenario.input_cfg))
...
TEST_F(SrvConfigTest, getDdnsParamsTest4) {
    ...
    subnet1->setFetchGlobalsFn([conf]() -> ElementPtr {
    ...
    // parameters to a subnet we need to set a callback function for each
    // subnet to allow for fetching global parameters.
    subnet2->setFetchGlobalsFn([conf]() -> ElementPtr {
        return (conf.getConfiguredGlobals());
    });
