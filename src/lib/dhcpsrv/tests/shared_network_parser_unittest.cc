...
public:
    ...
    virtual Network& parseIntoNetwork(ElementPtr test_config) = 0;
};
...
public:
    ...
    virtual Network& parseIntoNetwork(ElementPtr test_config) {
        // Parse configuration.
...
TEST_F(SharedNetwork4ParserTest, parse) {
    ...
    Option4AddrLstPtr dns_servers = std::dynamic_pointer_cast<
...
TEST_F(SharedNetwork4ParserTest, parseWithInvalidRenewRebind) {
    ...
    std::string config = getWorkingConfig();
    ElementPtr config_element = Element::fromJSON(config);
    ElementPtr valid_element = config_element->get("rebind-timer");
    int64_t value = valid_element->intValue();
    valid_element = config_element->get("renew-timer");
    ElementPtr mutable_element = std::const_pointer_cast<Element>(valid_element);
...
TEST_F(SharedNetwork6ParserTest, parse) {
    ...
    OptionDescriptor opt_dns_servers = cfg_option->get("dhcp6",
                                                       D6O_NAME_SERVERS);
    ASSERT_TRUE(opt_dns_servers.option_);
    Option6AddrLstPtr dns_servers = std::dynamic_pointer_cast<
