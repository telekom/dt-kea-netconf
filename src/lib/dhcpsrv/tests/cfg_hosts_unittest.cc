...
TEST_F(CfgHostsTest, unparsed4) {
    ...
    for (unsigned i = 0; i < 25; ++i) {
        ...
        for (unsigned j = 0; j < unparsed->size(); ++j) {
            ElementPtr host = unparsed->get(j);
            ASSERT_TRUE(host);
