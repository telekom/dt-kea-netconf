...
public:
    ...
    virtual VersionTuple getVersion() const {
    ...
    virtual bool lock4(std::vector<uint8_t>&) {return true;};

    /// @brief Unlock4
    ///
    /// Remove Lock4 from database.
    virtual bool unlock4(std::vector<uint8_t>&) {return true;};

    /// @brief Lock6
    ///
    /// Insert Lock6 into database.
    virtual bool lock6(std::vector<uint8_t>&) {return true;};

    /// @brief Unlock6
    ///
    /// Remove Lock6 from database.
    virtual bool unlock6(std::vector<uint8_t>&) {return true;};
...
TEST_F(LeaseMgrTest, getLease6) {
    ...
    std::unique_ptr<ConcreteLeaseMgr> mgr(new ConcreteLeaseMgr(pmap));
