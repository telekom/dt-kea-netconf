...
TEST_F(HostReservationParserTest, options4) {
    ...
    Option4AddrLstPtr opt_dns = std::dynamic_pointer_cast<
    ...
    Option4AddrLstPtr opt_log = std::dynamic_pointer_cast<
    ...
    OptionUint8Ptr opt_ttl = std::dynamic_pointer_cast<
        OptionUint8>(retrieveOption(*hosts[0], DHCP4_OPTION_SPACE, DHO_DEFAULT_IP_TTL));
    ASSERT_TRUE(opt_ttl);
    EXPECT_EQ(64, opt_ttl->getValue());

    // Canonize the config
    ElementPtr option = config_element->get("option-data")->get(0);
    ...
    option = config_element->get("option-data")->get(1);
    option = config_element->get("option-data")->get(2);
...
TEST_F(HostReservationParserTest, options6) {
    ...
    Option6AddrLstPtr opt_dns = std::dynamic_pointer_cast<
    ...
    Option6AddrLstPtr opt_nis = std::dynamic_pointer_cast<
    ...
    OptionUint8Ptr opt_prf = std::dynamic_pointer_cast<
