...
GenericLeaseMgrTest::GenericLeaseMgrTest()
    : lmptr_(NULL), cassandra_denormalized_(false) {
...
#ifdef TERASTREAM
    if (lease) {
        lease->privacy_expire_ = lease->cltt_;
    }
#endif  // TERASTREAM
...
GenericLeaseMgrTest::testLease4NullClientId() {
    ...
    if (!cassandra_denormalized_) {
        // No altering of client ID, subnet ID, hardware address is permitted
        // alongside address in CASSANDRA_DENORMALIZED_TABLES.
        ++leases[1]->subnet_id_;
    }
...
GenericLeaseMgrTest::testGetLease4HWAddr2() {
    ...
    if (cassandra_denormalized_) {
        // CASSANDRA_DENORMALIZED_TABLES uses explicit NULL hardware address
        // which is exactly 00:00:00:00:00:00 so change it to 00:00:00:00:00:FF
...
#ifdef TERASTREAM
    lease->privacy_expire_ = DatabaseConnection::MAX_DB_TIME;
#endif  // TERASTREAM
...
GenericLeaseMgrTest::testGetLeases4Paged() {
    ...
    std::unique_ptr<LeasePageSize> lease_page_size;
...
#ifdef TERASTREAM
    empty_lease->privacy_expire_ = 100;
#endif  // TERASTREAM
...
GenericLeaseMgrTest::testConcurrentUpdateLease4() {
    ...
    Lease4Ptr initialLease = std::make_shared<Lease4>(*leases[1]);
...
GenericLeaseMgrTest::testUpdateLease6() {
    ...
    // Modify some fields in lease 1 (not the address) and update it.
    if (!cassandra_denormalized_) {
        // No altering of lease_type, DUID, IAID, subnet ID is permitted
        // alongside address in CASSANDRA_DENORMALIZED_TABLES.
        ++leases[1]->iaid_;
...
GenericLeaseMgrTest::testConcurrentUpdateLease6() {
    ...
    // Save initial lease to be used for concurrent update
    Lease6Ptr initialLease = std::make_shared<Lease6>(*leases[1]);
    detailCompareLease(leases[1], initialLease);
...
GenericLeaseMgrTest::testVersion(int major, int minor) {
    auto const& version(lmptr_->getVersion());
    EXPECT_EQ(major, std::get<0>(version));
    EXPECT_EQ(minor, std::get<1>(version));
}
...
#ifdef TERASTREAM
        leases[i]->privacy_expire_ = leases[i]->cltt_;
#endif  // TERASTREAM
...
GenericLeaseMgrTest::testDeleteExpiredReclaimedLeases4() {
    ...
    for (size_t i = 0; i < leases.size(); ++i) {
        ...
        if (leases[i]->stateExpiredReclaimed() &&
            ...
            if (cassandra_denormalized_) {
                // Leases do not expire in CASSANDRA_DENORMALIZED_TABLES.
                continue;
            }
