...
public:
    ...
    /// @return returns an ElementPtr containing the numeric result
    /// code and outcome comment.
    isc::data::ElementPtr
    parseElementSet(isc::data::ElementPtr config_set, bool v6) {
        ...
        try {
            ...
            std::map<std::string, ElementPtr>::const_iterator
    ...
    void setAllDefaults(ElementPtr config, bool v6) {
        ...
        ElementPtr d2_client = config->get("dhcp-ddns");
        if (d2_client) {
    ...
    int parseConfiguration(const std::string& config, bool v6 = false,
        ...
        if (json) {
            ...
            ElementPtr status = parseElementSet(json, v6);
            ElementPtr comment = parseAnswer(rcode_, status);
...
public:
    ...
    ElementPtr toElement() const {
        ...
        // Set option-def
        ElementPtr option_def = cfg_option_def_->toElement();
        if (!option_def->empty()) {
            result->set("option-def", option_def);
        }
        // Set option-data
        ElementPtr option_data = cfg_option_->toElement();
    ...
    void runCfgOptionsTest(uint16_t family, ElementPtr expected) {
        ElementPtr option_def = expected->get("option-def");
        if (option_def) {
        ...
        ElementPtr option_data = expected->get("option-data");
        if (option_data) {
...
TEST_F(ParseConfigTest, minimalOptionDataTest) {
    ...
    ElementPtr opt_data = expected->get("option-data")->get(0);
...
TEST_F(ParseConfigTest, optionDataCSVFormatWithOptionDef) {
    ...
    // Verify that the option data is correct.
    OptionCustomPtr addr_opt = std::dynamic_pointer_cast<
    ...
    // Verify that the option data is correct.
    addr_opt = std::dynamic_pointer_cast<
        OptionCustom>(getOptionPtr(DHCP4_OPTION_SPACE, 16));
    ASSERT_TRUE(addr_opt);
    EXPECT_EQ("192.0.2.0", addr_opt->readAddress().toText());
...
TEST_F(ParseConfigTest, encapsulatedOptionData) {
    ...
    // Verify that the option data is correct.
    OptionCustomPtr s46_rule = std::dynamic_pointer_cast<OptionCustom>
...
TEST_F(ParseConfigTest, optionDataCSVFormatNoOptionDef) {
    ...
    opt_data = expected->get("option-data")->get(0);
    opt_data->remove("name");
    CfgOptionsTest cfg2(CfgMgr::instance().getStagingCfg());
...
TEST_F(ParseConfigTest, optionDataNoName) {
    ...
    ASSERT_NO_THROW(rcode = parseConfiguration(config, true));
    EXPECT_EQ(0, rcode);
    Option6AddrLstPtr opt = std::dynamic_pointer_cast<
...
TEST_F(ParseConfigTest, optionDataMinimal) {
    ...
    ASSERT_NO_THROW(rcode = parseConfiguration(config, true));
    EXPECT_EQ(0, rcode);
    opt = std::dynamic_pointer_cast<Option6AddrLst>(getOptionPtr(DHCP6_OPTION_SPACE,
                                                                   23));
    ASSERT_TRUE(opt);
    ASSERT_EQ(1, opt->getAddresses().size());
...
TEST_F(ParseConfigTest, emptyOptionData) {
    ...
    ASSERT_NO_THROW(rcode = parseConfiguration(config, true));
    EXPECT_EQ(0, rcode);
    const Option6AddrLstPtr opt = std::dynamic_pointer_cast<
...
TEST_F(ParseConfigTest, commaCSVFormatOptionData) {
    ...
    OptionStringPtr opt_str = std::dynamic_pointer_cast<OptionString>(opt);
    ASSERT_TRUE(opt_str);
...
TEST_F(ParseConfigTest, hexOptionData) {
    ...
    for (auto hex_str : valid_hexes) {
        ...
        Option4AddrLstPtr opt = std::dynamic_pointer_cast<Option4AddrLst>
...
TEST_F(ParseConfigTest, oneHooksLibrary) {
    ...
    EXPECT_EQ(CALLOUT_LIBRARY_1, std::get<LIBRARY>(libraries[0]));
...
TEST_F(ParseConfigTest, twoHooksLibraries) {
    ...
    EXPECT_EQ(CALLOUT_LIBRARY_1, std::get<LIBRARY>(libraries[0]));
    EXPECT_EQ(CALLOUT_LIBRARY_2, std::get<LIBRARY>(libraries[1]));
...
TEST_F(ParseConfigTest, reconfigureReverseHooksLibraries) {
    ...
    isc::hooks::HookLibsCollection libraries = getLibraries();
    ASSERT_EQ(2, libraries.size());
    EXPECT_EQ(CALLOUT_LIBRARY_2, std::get<LIBRARY>(libraries[0]));
    EXPECT_EQ(CALLOUT_LIBRARY_1, std::get<LIBRARY>(libraries[1]));
...
TEST_F(ParseConfigTest, invalidHooksLibraries) {
    ...
    EXPECT_EQ(CALLOUT_LIBRARY_1, std::get<LIBRARY>(libraries[0]));
    EXPECT_EQ(NOT_PRESENT_LIBRARY, std::get<LIBRARY>(libraries[1]));
    EXPECT_EQ(CALLOUT_LIBRARY_2, std::get<LIBRARY>(libraries[2]));
...
TEST_F(ParseConfigTest, HooksLibrariesParameters) {
    ...
    EXPECT_EQ(CALLOUT_LIBRARY_1, std::get<LIBRARY>(libraries[0]));
    EXPECT_EQ(CALLOUT_LIBRARY_2, std::get<LIBRARY>(libraries[1]));
    EXPECT_EQ(CALLOUT_PARAMS_LIBRARY, std::get<LIBRARY>(libraries[2]));
    ...
    EXPECT_FALSE(std::get<PARAMETERS_VALUE>(libraries[0]));
    EXPECT_TRUE(std::get<PARAMETERS_VALUE>(libraries[1]));
    ASSERT_TRUE(std::get<PARAMETERS_VALUE>(libraries[2]));

    // Ok, get the parameter for the third library.
    ElementPtr params = std::get<PARAMETERS_VALUE>(libraries[2]);
    ...
    ElementPtr svalue = params->get("svalue");
    ElementPtr ivalue = params->get("ivalue");
    ElementPtr bvalue = params->get("bvalue");
