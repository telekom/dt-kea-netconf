...
typedef std::shared_ptr<LeaseStatsQuery> LeaseStatsQueryPtr;

/// @brief Defines a pointer to a LeaseStatsRow.
typedef std::shared_ptr<LeaseStatsRow> LeaseStatsRowPtr;
...
public:
    ...
    virtual db::VersionTuple getVersion() const = 0;

#ifdef TERASTREAM_LOCK
    /// @brief Lock4
    ///
    /// Insert Lock4 into database.
    virtual bool lock4(std::vector<uint8_t>& lock_key) = 0;

    /// @brief Unlock4
    ///
    /// Remove Lock4 from database.
    virtual bool unlock4(std::vector<uint8_t>& lock_key) = 0;

    /// @brief Lock6
    ///
    /// Insert Lock6 into database.
    virtual bool lock6(std::vector<uint8_t>& lock_key) = 0;

    /// @brief Unlock6
    ///
    /// Remove Lock6 from database.
    virtual bool unlock6(std::vector<uint8_t>& lock_key) = 0;

    template <DhcpSpaceType D>
    bool lock(std::vector<uint8_t>& lock_key);

    template <DhcpSpaceType D>
    bool unlock(std::vector<uint8_t>& lock_key);
