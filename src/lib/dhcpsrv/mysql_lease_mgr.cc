...
#ifdef TERASTREAM
/// @brief Max Privacy History length.
static const size_t MAX_PRIVACY_HISTORY_LEN = 4096;
#endif  // TERASTREAM
...
public:
    ...
    MySqlLease4Exchange() : addr4_(0), hwaddr_length_(0), hwaddr_null_(MLM_FALSE),
        ...
        columns_[8] = "hostname";
#ifdef TERASTREAM
        columns_[9] = "privacy_expire";
        columns_[10] = "privacy_hash";
        columns_[11] = "privacy_history";
        columns_[12] = "sw_4o6_src_address";
        columns_[13] = "state";
        columns_[14] = "user_context";
...
#ifdef TERASTREAM
            // privacy_expire: timestamp
            MySqlConnection::convertToDatabaseTime(lease_->privacy_expire_, 0, privacy_expire_);
            bind_[9].buffer_type = MYSQL_TYPE_TIMESTAMP;
            bind_[9].buffer = reinterpret_cast<char*>(&privacy_expire_);
            bind_[9].buffer_length = sizeof(privacy_expire_);
            // bind_[9].is_null = &MLM_FALSE; // commented out for performance
                                              // reasons, see memset() above

            // privacy_hash: varbinary(128)
            privacy_hash_length_ = lease_->privacy_hash_->size();
            bind_[10].buffer_type = MYSQL_TYPE_BLOB;
            bind_[10].buffer = reinterpret_cast<char*>(&((*(lease_->privacy_hash_))[0]));
            bind_[10].buffer_length = privacy_hash_length_;
            bind_[10].length = &privacy_hash_length_;
            // bind_[10].is_null = &MLM_FALSE; // commented out for performance
                                               // reasons, see memset() above

            // privacy_history: longtext
            privacy_history_length_ = lease->privacy_history_.length();
            bind_[11].buffer_type = MYSQL_TYPE_STRING;
            bind_[11].buffer = const_cast<char*>(lease->privacy_history_.c_str());
            bind_[11].buffer_length = privacy_history_length_;
            bind_[11].length = &privacy_history_length_;
            // bind_[11].is_null = &MLM_FALSE; // commented out for performance
                                               // reasons, see memset() above

            // sw_4o6_src_address: varchar(39)
            sw_4o6_src_address_ = lease_->sw_4o6_src_address_.toText();
            sw_4o6_src_address_length_ = sw_4o6_src_address_.size();

            bind_[12].buffer_type = MYSQL_TYPE_STRING;
            bind_[12].buffer = const_cast<char*>(sw_4o6_src_address_.c_str());
            bind_[12].buffer_length = sw_4o6_src_address_length_;
            bind_[12].length = &sw_4o6_src_address_length_;
            // bind_[12].is_null = &MLM_FALSE; // commented out for performance
                                               // reasons, see memset() above

            // state: uint32_t
            bind_[13].buffer_type = MYSQL_TYPE_LONG;
            bind_[13].buffer = reinterpret_cast<char*>(&lease_->state_);
            bind_[13].is_unsigned = MLM_TRUE;
            // bind_[13].is_null = &MLM_FALSE; // commented out for performance
                                               // reasons, see memset() above

            // user_context: text
            ElementPtr ctx = lease->getContext();
            if (ctx) {
                bind_[14].buffer_type = MYSQL_TYPE_STRING;
                string ctx_txt = ctx->str();
                strncpy(user_context_, ctx_txt.c_str(), USER_CONTEXT_MAX_LEN - 1);
                bind_[14].buffer = user_context_;
                bind_[14].buffer_length = ctx_txt.length();
                // bind_[14].is_null = &MLM_FALSE; // commented out for performance
                                                   // reasons, see memset() above
            } else {
    ...
    std::vector<MYSQL_BIND> createBindForReceive() {
        ...
        // privacy_hash: varbinary(128)
        privacy_hash_length_ = sizeof(privacy_hash_buffer_);
        bind_[10].buffer_type = MYSQL_TYPE_BLOB;
        bind_[10].buffer = reinterpret_cast<char*>(privacy_hash_buffer_);
        bind_[10].buffer_length = privacy_hash_length_;
        bind_[10].length = &privacy_hash_length_;
        // bind_[10].is_null = &MLM_FALSE; // commented out for performance
                                           // reasons, see memset() above
        ...
        // privacy_history: longtext
        privacy_history_length_ = sizeof(privacy_history_buffer_);
        bind_[11].buffer_type = MYSQL_TYPE_STRING;
        bind_[11].buffer = reinterpret_cast<char*>(privacy_history_buffer_);
        bind_[11].buffer_length = privacy_history_length_;
        bind_[11].length = &privacy_history_length_;
        // bind_[11].is_null = &MLM_FALSE; // commented out for performance
                                           // reasons, see memset() above
        ...
        // sw_4o6_src_address: varchar(39)
        sw_4o6_src_address_length_ = sizeof(sw_4o6_src_address_buffer_) - 1;
        bind_[12].buffer_type = MYSQL_TYPE_STRING;
        bind_[12].buffer = sw_4o6_src_address_buffer_;
        bind_[12].buffer_length = sw_4o6_src_address_length_;
        bind_[12].length = &sw_4o6_src_address_length_;
        // bind_[12].is_null = &MLM_FALSE; // commented out for performance
                                           // reasons, see memset() above
        ...
        // state: uint32_t
        bind_[13].buffer_type = MYSQL_TYPE_LONG;
        bind_[13].buffer = reinterpret_cast<char*>(&state_);
        bind_[13].is_unsigned = MLM_TRUE;
        // bind_[13].is_null = &MLM_FALSE; // commented out for performance
                                           // reasons, see memset() above
        ...
        bind_[14].buffer_type = MYSQL_TYPE_STRING;
        bind_[14].buffer = reinterpret_cast<char*>(user_context_);
        bind_[14].buffer_length = user_context_length_;
        bind_[14].length= &user_context_length_;
        bind_[14].is_null = &user_context_null_;
#else
        // state: uint32_t
        bind_[9].buffer_type = MYSQL_TYPE_LONG;
...
#ifdef TERASTREAM
        time_t privacy_expire = 0;
        MySqlConnection::convertFromDatabaseTime(privacy_expire_, 0, privacy_expire);
        lease->privacy_expire_ = privacy_expire;
        lease->privacy_hash_ = PrivacyHashPtr(new PrivacyHash(privacy_hash_buffer_,
                privacy_hash_buffer_ + privacy_hash_length_));
        lease->privacy_history_.assign(privacy_history_buffer_,
                privacy_history_buffer_ + privacy_history_length_);
        sw_4o6_src_address_buffer_[sw_4o6_src_address_length_] = '\0';
        sw_4o6_src_address_ = sw_4o6_src_address_buffer_;
        lease->sw_4o6_src_address_ = isc::asiolink::IOAddress(sw_4o6_src_address_);
#endif  // TERASTREAM
...
#ifdef TERASTREAM
    MYSQL_TIME             privacy_expire_;            ///< Lease privacy expire
    std::vector<uint8_t>   privacy_hash_;              ///< Privacy Hash
    uint8_t                privacy_hash_buffer_[Lease::MAX_PRIVACY_HASH_LEN]; ///< Buffer form of Privacy Hash
    unsigned long          privacy_hash_length_;       ///< Length of the Privacy Hash
    std::string            privacy_history_;           ///< Privacy History
    uint8_t                privacy_history_buffer_[MAX_PRIVACY_HISTORY_LEN]; ///< Buffer form of Privacy History
    unsigned long          privacy_history_length_;    ///< Length of the Privacy History
    std::string            sw_4o6_src_address_;        ///< String form of address
    char                   sw_4o6_src_address_buffer_[ADDRESS6_TEXT_MAX_LEN + 1]; ///< Character
                                                       ///< array form of V6 address
    unsigned long          sw_4o6_src_address_length_; ///< Length of the address
#endif  // TERASTREAM
...
class MySqlLease6Exchange : public MySqlLeaseExchange {
    /// @brief Set number of database columns for this lease structure
#ifdef TERASTREAM
    static const size_t LEASE_COLUMNS = 20;
#else
...
public:
...
#ifdef TERASTREAM
        columns_[15] = "privacy_expire";
        columns_[16] = "privacy_hash";
        columns_[17] = "privacy_history";
        columns_[18] = "state";
        columns_[19] = "user_context";
        BOOST_STATIC_ASSERT(19 < LEASE_COLUMNS);
#else
        columns_[15] = "state";
    ...
    std::vector<MYSQL_BIND> createBindForSend(const Lease6Ptr& lease) {
        ...
        try {
            ...
            // privacy_expire: timestamp
            MySqlConnection::convertToDatabaseTime(lease_->privacy_expire_, 0, privacy_expire_);
            bind_[15].buffer_type = MYSQL_TYPE_TIMESTAMP;
            bind_[15].buffer = reinterpret_cast<char*>(&privacy_expire_);
            bind_[15].buffer_length = sizeof(privacy_expire_);
            // bind_[15].is_null = &MLM_FALSE; // commented out for performance
                                               // reasons, see memset() above

            // privacy_hash: varbinary(128)
            privacy_hash_length_ = lease_->privacy_hash_->size();
            bind_[16].buffer_type = MYSQL_TYPE_BLOB;
            bind_[16].buffer = reinterpret_cast<char*>(&((*(lease_->privacy_hash_))[0]));
            bind_[16].buffer_length = privacy_hash_length_;
            bind_[16].length = &privacy_hash_length_;
            ...
            bind_[17].buffer = const_cast<char*>(lease->privacy_history_.c_str());
            bind_[17].buffer_length = privacy_history_length_;
            bind_[17].length = &privacy_history_length_;
            // bind_[17].is_null = &MLM_FALSE; // commented out for performance
                                              // reasons, see memset() above

            // state: uint32_t
            bind_[18].buffer_type = MYSQL_TYPE_LONG;
            bind_[18].buffer = reinterpret_cast<char*>(&lease_->state_);
            ...
            ElementPtr ctx = lease->getContext();
            if (ctx) {
                bind_[19].buffer_type = MYSQL_TYPE_STRING;
                string ctx_txt = ctx->str();
                strncpy(user_context_, ctx_txt.c_str(), USER_CONTEXT_MAX_LEN - 1);
                bind_[19].buffer = user_context_;
                bind_[19].buffer_length = ctx_txt.length();
                // bind_[19].is_null = &MLM_FALSE; // commented out for performance
                                                   // reasons, see memset() above
            } else {
                bind_[19].buffer_type = MYSQL_TYPE_NULL;
            }
#else
            // state: uint32_t
    ...
    std::vector<MYSQL_BIND> createBindForReceive() {
        ...
        privacy_hash_length_ = sizeof(privacy_hash_buffer_);
        bind_[16].buffer_type = MYSQL_TYPE_BLOB;
        bind_[16].buffer = reinterpret_cast<char*>(privacy_hash_buffer_);
        bind_[16].buffer_length = privacy_hash_length_;
        bind_[16].length = &privacy_hash_length_;
        // bind_[16].is_null = &MLM_FALSE; // commented out for performance
                                          // reasons, see memset() above
        ...
        privacy_history_length_ = sizeof(privacy_history_buffer_);
        bind_[17].buffer_type = MYSQL_TYPE_STRING;
        bind_[17].buffer = reinterpret_cast<char*>(privacy_history_buffer_);
        bind_[17].buffer_length = privacy_history_length_;
        bind_[17].length = &privacy_history_length_;
        // bind_[17].is_null = &MLM_FALSE; // commented out for performance
                                          // reasons, see memset() above
        ...
        // state: uint32_t
        bind_[18].buffer_type = MYSQL_TYPE_LONG;
        bind_[18].buffer = reinterpret_cast<char*>(&state_);
        bind_[18].is_unsigned = MLM_TRUE;
        // bind_[18].is_null = &MLM_FALSE; // commented out for performance
                                           // reasons, see memset() above
        ...
        user_context_length_ = sizeof(user_context_);
        bind_[19].buffer_type = MYSQL_TYPE_STRING;
        bind_[19].buffer = reinterpret_cast<char*>(user_context_);
        bind_[19].buffer_length = user_context_length_;
        bind_[19].length= &user_context_length_;
        bind_[19].is_null = &user_context_null_;
#else
        // state: uint32_t
        bind_[15].buffer_type = MYSQL_TYPE_LONG;
    ...
    Lease6Ptr getLeaseData() {
        ...
        Lease6Ptr result(std::make_shared<Lease6>(type, addr, duid_ptr, iaid_,
                                                    pref_lifetime_,
        ...
        time_t privacy_expire = 0;
        MySqlConnection::convertFromDatabaseTime(privacy_expire_, 0, privacy_expire);
        result->privacy_expire_ = privacy_expire;
        result->privacy_hash_ = PrivacyHashPtr(new PrivacyHash(privacy_hash_buffer_,
                privacy_hash_buffer_ + privacy_hash_length_));
        result->privacy_history_.assign(privacy_history_buffer_,
                privacy_history_buffer_ + privacy_history_length_);
#endif  // TERASTREAM
...
bool
MySqlLeaseMgr::lock4(std::vector<uint8_t>&) {
    return true;
}

bool
MySqlLeaseMgr::unlock4(std::vector<uint8_t>&) {
    return true;
}

bool
MySqlLeaseMgr::lock6(std::vector<uint8_t>&) {
    return true;
}

bool
MySqlLeaseMgr::unlock6(std::vector<uint8_t>&) {
    return true;
}
...
MySqlLeaseMgr::MySqlLeaseMgr(const MySqlConnection::ParameterMap& parameters)
    ...
    VersionTuple const code_version(MYSQL_SCHEMA_VERSION_MAJOR, MYSQL_SCHEMA_VERSION_MINOR);
    VersionTuple const& db_version(getVersion());
    if (code_version != db_version) {
        isc_throw(DbOpenError,
                  "MySQL schema version mismatch: need version: "
                      << std::get<0>(code_version) << "." << std::get<1>(code_version)
...
MySqlLeaseMgr::updateLease4(const Lease4Ptr& lease) {
    StatementIndex const stindex = UPDATE_LEASE4;
...
MySqlLeaseMgr::updateLease6(const Lease6Ptr& lease) {
    StatementIndex const stindex = UPDATE_LEASE6;
