...
Pool::Pool(Lease::Type type, const isc::asiolink::IOAddress& first,
    ...
    :id_(getNextID()), first_(first), last_(last), type_(type),
     capacity_(0), cfg_option_(new CfgOption()), client_class_(std::string()),
     last_allocated_(first), last_allocated_valid_(false), initial_first_(first),
     initial_last_(last) {
}
...
Pool4::Pool4(const isc::asiolink::IOAddress& first,
             const isc::asiolink::IOAddress& last,
             uint8_t const threshold_percent /* = 100 */)
    : Pool(Lease::TYPE_V4, first, last) {
    // check if specified address boundaries are sane
    if (!first_.isV4() || !last_.isV4()) {
        isc_throw(BadValue, "Invalid Pool4 address boundaries: not IPv4");
    }

    std::tie(first_, last_) = applyThresholdToRange(first, last, threshold_percent);
    ...
    // info.
    capacity_ = addrsInRange(first_, last_);
}
...
Pool4::Pool4(const isc::asiolink::IOAddress& prefix,
    ...
    uint8_t const prefix_length(applyThresholdToRange(prefix_len, 32, threshold_percent));

    // Let's now calculate the last address in defined pool
    last_ = lastAddrInPrefix(prefix, prefix_length);
...
Pool4::create(const IOAddress& first, const IOAddress& last) {
    return (std::make_shared<Pool4>(first, last));
}

Pool4Ptr
Pool4::create(const IOAddress& prefix, uint8_t prefix_len) {
    return (std::make_shared<Pool4>(prefix, prefix_len));
}
...
Pool::toNotificationElement() const {
    ElementPtr const& result(Element::createMap());
    uint64_t initial_capacity = addrsInRange(initial_first_, initial_last_);
    uint64_t allocated_capacity = addrsInRange(first_, last_allocated_);
    result->set("pool-id", id_);
    result->set("total-pool-addresses", initial_capacity);
    result->set("total-pool-prefixes", initial_capacity);
    result->set("max-allocated-addresses", capacity_);
    result->set("max-allocated-prefixes", capacity_);
    result->set("allocated-address-count", allocated_capacity);
    result->set("allocated-prefixes-count", allocated_capacity);
    return result;
}
...
Pool6::Pool6(Lease::Type type,
             const isc::asiolink::IOAddress& prefix,
             const uint8_t prefix_len,
             const uint8_t delegated_len /* = 128 */,
             uint8_t const threshold_percent /* = 100 */)
    : Pool(type, prefix, IOAddress::IPV6_ZERO_ADDRESS()), prefix_len_(delegated_len),
      pd_exclude_option_() {

    init(type, prefix, prefix_len, delegated_len,
         IOAddress::IPV6_ZERO_ADDRESS(), 0, threshold_percent);
}
...
Pool6::Pool6(const asiolink::IOAddress& prefix, const uint8_t prefix_len,
    ...
    init(Lease::TYPE_PD, prefix, prefix_len, delegated_len, excluded_prefix,
         excluded_prefix_len, threshold_percent);
...
Pool6::create(Lease::Type type, const IOAddress& first, const IOAddress& last) {
    return (std::make_shared<Pool6>(type, first, last));
}

Pool6Ptr
Pool6::create(Lease::Type type, const IOAddress& prefix,
              uint8_t prefix_len, uint8_t delegated_len) {
    return (std::make_shared<Pool6>(type, prefix, prefix_len, delegated_len));
}
...
Pool6::create(const IOAddress& prefix, const uint8_t prefix_len,
              ...
              const uint8_t excluded_prefix_len) {
    return (std::make_shared<Pool6>(prefix, prefix_len,
...
Pool6::init(const Lease::Type& type,
    ...
    uint8_t const prefix_length(
        applyThresholdToRange(prefix_len, delegated_len, threshold_percent));
    ...
    capacity_ = prefixesInRange(prefix_length, delegated_len);
