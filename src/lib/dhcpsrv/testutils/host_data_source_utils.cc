...
HostPtr
HostDataSourceUtils::initializeHost6(std::string const& address,
...
HostDataSourceUtils::compareHosts(const ConstHostPtr& host1,
    ...
    ElementPtr ctx1 = host1->getContext();
    ElementPtr ctx2 = host2->getContext();
    if (ctx1) {
        EXPECT_TRUE(ctx2);
        if (ctx2) {
            EXPECT_EQ(*ctx1, *ctx2);
...
HostDataSourceUtils::compareOptions(const ConstCfgOptionPtr& cfg1,
    ...
    BOOST_FOREACH (std::string space, option_spaces) {
        ...
        BOOST_FOREACH (OptionDescriptor desc1, *options1) {
            ...
            ElementPtr ctx1 = desc1.getContext();
            ElementPtr ctx2 = desc2.getContext();
            if (ctx1) {
                EXPECT_TRUE(ctx2);
                if (ctx2) {
                    EXPECT_EQ(*ctx1, *ctx2)
