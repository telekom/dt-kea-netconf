...
void GenericHostDataSourceBenchmark::setUp(::benchmark::State& state, size_t const& host_count) {
    state.PauseTiming();
    SetUp(state);
...
void GenericHostDataSourceBenchmark::setUpWithInserts(::benchmark::State& state,
...
OptionDescriptor GenericHostDataSourceBenchmark::createEmptyOption(const Option::Universe& universe,
...
void GenericHostDataSourceBenchmark::addTestOptions(const HostPtr& host,
                                                    const bool formatted,
...
void GenericHostDataSourceBenchmark::prepareHosts(size_t const& host_count) {
...
void GenericHostDataSourceBenchmark::insertHosts() {
    for (HostPtr host : hosts_) {
        hdsptr_->add(host);
    }
}

void GenericHostDataSourceBenchmark::updateHosts() {
    for (HostPtr host : hosts_) {
        hdsptr_->add(host);
    }
}

void GenericHostDataSourceBenchmark::benchGetAll() {
    for (HostPtr host : hosts_) {
...
void GenericHostDataSourceBenchmark::getAllv4Resv() {
    for (HostPtr host : hosts_) {
        hdsptr_->getAll4(host->getIPv4Reservation());
    }
}

void GenericHostDataSourceBenchmark::benchGet4IdentifierSubnetId() {
    for (HostPtr host : hosts_) {
        std::vector<uint8_t> hwaddr = host->getIdentifier();
...
void GenericHostDataSourceBenchmark::benchGet4SubnetIdv4Resrv() {
    for (HostPtr host : hosts_) {
        hdsptr_->get4(host->getIPv4SubnetID(), host->getIPv4Reservation());
    }
}

void GenericHostDataSourceBenchmark::benchGet6IdentifierSubnetId() {
    for (HostPtr host : hosts_) {
        std::vector<uint8_t> hwaddr = host->getIdentifier();
