...
public:
    ...
    isc::data::ElementPtr const toNotificationElement() const;
...
protected:
    ...
    static constexpr uint8_t DEFAULT_THRRESHOLD_PERCENT = 100;

    /// @brief The first address in a pool before threshold percent is applied
    isc::asiolink::IOAddress const initial_first_;

    /// @brief The last address in a pool before threshold percent is applied
    isc::asiolink::IOAddress const initial_last_;
};

class Pool4;

/// @brief a pointer an IPv4 Pool
typedef std::shared_ptr<Pool4> Pool4Ptr;
...
public:
    ...
    Pool4(const isc::asiolink::IOAddress& first,
          const isc::asiolink::IOAddress& last,
          uint8_t const threshold_percent = DEFAULT_THRRESHOLD_PERCENT);
...
typedef std::shared_ptr<Pool6> Pool6Ptr;
...
private:
    ...
    void init(const Lease::Type& type,
              ...
              const asiolink::IOAddress& excluded_prefix,
              const uint8_t excluded_prefix_len,
              uint8_t const threshold_percent);
