...
struct NoPools;
struct NoPools : AllocFailed {
    NoPools(char const* file, size_t line, char const* what, PoolCollection const& pools)
        : AllocFailed(file, line, what), pools_(pools) {
    }

    static NoPools create(char const* file, size_t line, char const* what, PoolCollection const& pools) {
        return NoPools(file, line, what, pools);
    }
...
protected:
    ...
    public:
        ...
        virtual isc::asiolink::IOAddress pickAddress(const SubnetPtr& subnet,
            ...
            auto const& [psid_offset, psid_length] = Subnet4::getPSIDOffsetAndLength(subnet);

            if (isc::util::MultiThreadingMgr::instance().getMode()) {
                std::lock_guard<std::mutex> lock(mutex_);
                return pickAddressInternal(subnet, client_classes, duid, hint, psid_offset, psid_length);
            } else {
                return pickAddressInternal(subnet, client_classes, duid, hint, psid_offset, psid_length);
            }
        }
    ...
    protected:
        ...
        /// @param psid_offset not used
        /// @param psid_len length of the the PSID to increment with
        /// @return result address or prefix
        static isc::asiolink::IOAddress
        increaseAddress(const isc::asiolink::IOAddress& address,
                        bool prefix, const uint8_t prefix_len,
                        uint8_t const psid_offset = 0, uint8_t const psid_len = 0);
    };
...
public:
    ...
    struct ClientContext6 : public boost::noncopyable {
        ...
        /// @brief private lease
        ///
        /// The private lease when using private allocation
        Lease6Ptr private_lease_;
#endif  // TERASTREAM
...
public:
    ...
    typedef std::shared_ptr<ClientContext4> ClientContext4Ptr;
...
#ifdef TERASTREAM
    void removeNonmatchingPoolLeases6(ClientContext6& ctx, Lease6Collection& existing_leases);
    template<typename ContextType, typename LeaseCollection, typename LeasePtrType>
    static void applyPrivacy(ContextType& ctx, LeaseCollection& existing_leases, LeasePtrType& private_lease);
    template<typename ContextType>
    static bool isReservedPrivateAddress(ContextType& ctx, const isc::asiolink::IOAddress& current_lease);
#endif  // TERASTREAM
...
typedef std::shared_ptr<AllocEngine> AllocEnginePtr;
