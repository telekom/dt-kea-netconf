...
std::unique_ptr<ConfigBackendDHCPv6Mgr>&
ConfigBackendDHCPv6Mgr::getConfigBackendDHCPv6MgrPtr() {
    static std::unique_ptr<ConfigBackendDHCPv6Mgr> cb_dhcp6_mgr;
    return (cb_dhcp6_mgr);
}
...
ConfigBackendDHCPv6Mgr::instance() {
    std::unique_ptr<ConfigBackendDHCPv6Mgr>& cb_dhcp6_mgr = getConfigBackendDHCPv6MgrPtr();
    if (!cb_dhcp6_mgr) {
        create();
    }
    return (*cb_dhcp6_mgr);
