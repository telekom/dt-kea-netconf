...
#ifdef TERASTREAM
    /// @brief Max Privacy Hash length.
    static const size_t MAX_PRIVACY_HASH_LEN = 128;
#endif  // TERASTREAM
...
#ifdef TERASTREAM
    /// @brief privacy expire time
    ///
    /// The expiration timestamp for the private address
    time_t privacy_expire_;

    /// @brief privacy hash
    ///
    /// The privacy hash for the private address
    PrivacyHashPtr privacy_hash_;

    /// @brief privacy history
    ///
    /// The privacy history list of address
...
using Lease4Ptr = isc::db::Ptr<Lease4>;
...
struct Lease4 : public Lease {
    ...
    Lease4() : Lease(0U, 0, 0, 0, false, false, "", HWAddrPtr())
#ifdef TERASTREAM
      , sw_4o6_src_address_(isc::asiolink::IOAddress::IPV6_ZERO_ADDRESS())
#endif  // TERASTREAM
    {
    ...
    static Lease4Ptr fromElement(const data::ElementPtr& element);
...
typedef std::shared_ptr<Lease4Collection> Lease4CollectionPtr;

struct Lease6;

/// @brief Pointer to a Lease6 structure.
using Lease6Ptr = isc::db::Ptr<Lease6>;
...
struct Lease6 : public Lease {
    ...
    Lease6(Lease::Type type, const isc::asiolink::IOAddress& addr, DuidPtr duid,
           ...
           uint8_t prefixlen = 128, time_t alloc_time = time(nullptr));
    ...
    Lease6(Lease::Type type, const isc::asiolink::IOAddress& addr, DuidPtr duid,
           ...
           time_t alloc_time = time(nullptr));
    ...
    /// @param element pointer to the data element object to be parsed.
    /// @return Pointer to the created lease.
    static Lease6Ptr fromElement(const data::ElementPtr& element);
};

/// @brief Pointer to a const Lease6 structure.
typedef std::shared_ptr<const Lease6> ConstLease6Ptr;
...
typedef std::shared_ptr<Lease6Collection> Lease6CollectionPtr;
...
template <>
struct adapter_Lease<DHCP_SPACE_V4> {
    using type = Lease4;
};

template <>
struct adapter_Lease<DHCP_SPACE_V6> {
    using type = Lease6;
};
...
template <DhcpSpaceType D>
using LeasePtrT = isc::db::Ptr<LeaseT<D>>;

template <DhcpSpaceType D>
using LeaseCollectionT = isc::db::Collection<LeaseT<D>>;
