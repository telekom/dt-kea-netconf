...
public:
    ...
    std::unique_ptr<PgSqlLease4Exchange> exchange4_; ///< Exchange object
    std::unique_ptr<PgSqlLease6Exchange> exchange6_; ///< Exchange object
...
typedef std::shared_ptr<PgSqlLeaseContext> PgSqlLeaseContextPtr;
...
typedef std::shared_ptr<PgSqlLeaseContextPool> PgSqlLeaseContextPoolPtr;
...
public:
    ...
    /// @throw isc::db::DbOperationError An operation on the open database has
    ///        failed.
    virtual db::VersionTuple getVersion() const;

#ifdef TERASTREAM_LOCK
    /// @brief Lock4
    ///
    /// Insert Lock4 into database.
    virtual bool lock4(std::vector<uint8_t>& lock_key);

    /// @brief Unlock4
    ///
    /// Remove Lock4 from database.
    virtual bool unlock4(std::vector<uint8_t>& lock_key);

    /// @brief Lock6
    ///
    /// Insert Lock6 into database.
    virtual bool lock6(std::vector<uint8_t>& lock_key);

    /// @brief Unlock6
    ///
    /// Remove Lock6 from database.
    virtual bool unlock6(std::vector<uint8_t>& lock_key);
