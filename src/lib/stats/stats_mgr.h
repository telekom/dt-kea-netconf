...
public:
    ...
    /// @return answer confirming success of this operation
    isc::data::ElementPtr
    statisticSetMaxSampleAgeAllHandler(const isc::data::ElementPtr& params);
    ...
    /// @return answer confirming success of this operation
    isc::data::ElementPtr
    statisticSetMaxSampleCountAllHandler(const isc::data::ElementPtr& params);

    /// @brief Reads /proc/stat and sums up all the timings
    /// @return Cpu total time in jiffies
    static std::optional<size_t> getCpuTimes();

    /// @brief Reads /proc/self/stat
    /// @return user-time and sys-time of the current process
    static std::optional<std::tuple<size_t, size_t>> getProcessCpuTimes();

    /// @brief Calculates cpu usage in percent
    /// @param total_time_before total cpu time from /pid/stat
    /// @param total_time_after total cpu time from /pid/stat
    /// @param proc_times_before user-time and sys-time
    /// @param proc_times_after user-time and sys-time
    /// @return percentage of cpu used by the current process
    ...
    /// @brief Returns the cpu usage in percent
    std::optional<double> getCpuUsage();

    /// @brief Returns the memory usage in MB
    std::optional<int64_t> getMemoryUsage();

    /// @brief handler for processing 'resource-get' command
    ///
    /// This handler processes resource-get command, which returns
    /// over the control channel the requested resources
    /// @param command (parameter ignored)
    /// @param args parameter containing a map with the resource parameter
    ///
    /// @return result of the command
    isc::data::ElementPtr commandResourceGetHandler(const std::string& command,
...
private:
    ...
    ///
    /// @return JSON structures representing a single statistic
    isc::data::ElementPtr getInternal(const std::string& name) const;
    ...
    ///
    /// @return JSON structures representing all statistics
    isc::data::ElementPtr getAllInternal() const;
    ...
    static bool getStatName(const isc::data::ElementPtr& params,
                            std::string& name,
                            std::string& reason);
    ...
    /// @param reason [out] failure reason (if error is detected)
    /// @return true (if everything is ok), false otherwise
    static bool getStatDuration(const isc::data::ElementPtr& params,
    ...
    /// @param reason [out] failure reason (if error is detected)
    /// @return true (if everything is ok), false otherwise
    static bool getStatMaxSamples(const isc::data::ElementPtr& params,
    ...
    /// @brief Sigar library manager pointer
#ifdef HAVE_SIGAR
    static sigar_t* sigar_;
#else
    /// @brief Cached cpu usage value used where sigar is not present
    /// values are total_cpu_time, proc_user_time, proc_sys_time
    std::optional<std::tuple<size_t, size_t, size_t>> cached_cpu_values_;
#endif
