...
TEST_F(StatsMgrTest, getGetAll) {
    ...
    ElementPtr rep_alpha = StatsMgr::instance().get("alpha");
    ElementPtr rep_beta = StatsMgr::instance().get("beta");
    ElementPtr rep_gamma = StatsMgr::instance().get("gamma");
    ElementPtr rep_delta = StatsMgr::instance().get("delta");
    ...
    ElementPtr rep_all = StatsMgr::instance().getAll();
    ASSERT_TRUE(rep_all);
...
TEST_F(StatsMgrTest, commandStatisticGet) {
    ...
    ElementPtr rsp = StatsMgr::instance().statisticGetHandler("statistic-get",
                                                                   params);
...
TEST_F(StatsMgrTest, commandGetAll) {
    ...
    // Check that all of them can be reported at once
    ElementPtr rsp = StatsMgr::instance().statisticGetAllHandler(
        "statistic-get-all", ElementPtr());
    ASSERT_TRUE(rsp);
    int status_code = -1;
    ElementPtr rep_all = parseAnswer(status_code, rsp);
...
TEST_F(StatsMgrTest, commandResetAll) {
    ...
    ElementPtr rsp = StatsMgr::instance().statisticResetAllHandler(
...
TEST_F(StatsMgrTest, commandRemoveAll) {
    ...
    // Check that all of them can be reset at once
    ElementPtr rsp = StatsMgr::instance().statisticRemoveAllHandler(
