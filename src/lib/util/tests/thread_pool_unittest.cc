...
public:
    ...
    void startThreads(uint32_t thread_count, bool signal = true) {
        ...
        for (uint32_t i = 0; i < thread_count; ++i) {
            threads_.push_back(std::make_shared<std::thread>(runFunction, this));
        }
    }
...
TEST_F(ThreadPoolTest, addAndCount) {
    ...
    for (uint32_t i = 0; i < items_count; ++i) {
        bool ret = true;
        EXPECT_NO_THROW(ret = thread_pool.add(std::make_shared<CallBack>(call_back)));
        EXPECT_TRUE(ret);
    }
...
TEST_F(ThreadPoolTest, addFront) {
    ...
    bool ret = true;
    for (uint32_t i = 0; i < items_count; ++i) {
        EXPECT_NO_THROW(ret = thread_pool.addFront(std::make_shared<CallBack>(call_back)));
        EXPECT_TRUE(ret);
    }
