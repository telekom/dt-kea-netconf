...
inline void uppercase(std::string& text) {
    std::transform(text.begin(), text.end(), text.begin(), isc::util::str::toUpper);
}
...
inline void lowercase(std::string& text) {
    std::transform(text.begin(), text.end(), text.begin(), isc::util::str::toLower);
}

inline void unquote(std::string& input) {
    size_t const size(input.size());
    if (size > 0 && input[0] == '"' && input[size - 1] == '"') {
        input = input.substr(1, size - 2);
    }
}

inline std::string
endsWithOrAdd(std::string const& input, std::string const& ends_with, std::string const& add) {
    if (boost::algorithm::ends_with(input, ends_with)) {
        return input;
    }
    return input + add;
}
...
NumType tokenToNum(const std::string& num_token) {
    ...
    try {
        ...
        isc_throw(StringTokenError, "Invalid SRV numeric parameter: " << num_token);
    }
    if (num < 0 || num >= (static_cast<NumType>(1) << BitSize)) {
        isc_throw(StringTokenError, "Numeric SRV parameter out of range: " << num);
    }
    return (num);
}
...
void decodeSeparatedHexString(const std::string& hex_string,
...
template <typename T>
void erase_all(std::string& recipient, T const& erasee) {
    while (true) {
        size_t const i = recipient.find(erasee);
        if (i == std::string::npos) {
            break;
        }
        recipient.replace(i, 1, "");
    }
}
...
typedef std::shared_ptr<StringSanitizer> StringSanitizerPtr;
...
inline bool isPrintable(const std::string& content) {
...
/// \return True if empty or contains only printable characters, False otherwise
inline bool isPrintable(const std::vector<uint8_t>& content) {
    for (const auto& ch : content) {
        if (isprint(static_cast<int>(ch)) == 0) {
            return (false);
        }
...
template <typename T>
std::string fromTuple(T tuple) {
    std::ostringstream result;
    bool first(true);
    boost::fusion::for_each(tuple, [&result, &first](auto& i) {
        if (first) {
            first = false;
        } else {
            result << ", ";
