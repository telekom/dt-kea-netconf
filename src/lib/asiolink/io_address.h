...
public:
    ...
    /// @brief Convenience function to check if it is an IPv6 zero address.
    ///
    /// @return true if the address is the zero IPv6 address.
    bool isV6Zero() const {
    ...
    static IOAddress add(IOAddress const& a, IOAddress const& b);
    static IOAddress add(IOAddress const& address, uint64_t address_count);
