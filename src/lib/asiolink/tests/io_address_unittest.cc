...
TEST(IOAddressTest, add) {
    IOAddress addr1("192.0.2.12");
    IOAddress addr2("0.0.0.5");
    IOAddress addr3("0.0.2.10");
    IOAddress addr4("0.0.2.1");
    IOAddress any4("0.0.0.0");
    IOAddress bcast("255.255.255.255");

    EXPECT_EQ("192.0.2.17", IOAddress::add(addr1, addr2).toText());
    EXPECT_EQ("192.0.4.22", IOAddress::add(addr1, addr3).toText());

    // Adding 0.0.0.0 is like subtracting 0.
    EXPECT_EQ("192.0.2.12", IOAddress::add(addr1, any4).toText());
    EXPECT_EQ("192.0.2.11", IOAddress::add(addr1, bcast).toText());
    EXPECT_EQ("0.0.4.11", IOAddress::add(addr3, addr4).toText());

    IOAddress addr6("fe80::abcd");
    IOAddress addr7("::4321");
    IOAddress addr8("::1234:0000");
    IOAddress addr9("2001:db8::face");
    IOAddress addr10("2001:db8::ffff:ffff:ffff:ffff");

    EXPECT_EQ(IOAddress("fe80::eeee"), IOAddress::add(addr6, addr7));
    EXPECT_EQ(IOAddress("fe80::1234:abcd"), IOAddress::add(addr6, addr8));
    EXPECT_EQ("4002:1a70:100:0:ffff:ffff:ffff:f9ce", IOAddress::add(addr9, addr10).toText());
}
