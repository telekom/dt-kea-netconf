...
using isc::util::losslessPercentage;
...
IOAddress firstAddrInPrefix6(const IOAddress& prefix, uint8_t len) {
    if (len > 128) {
        isc_throw(isc::BadValue,
                  "Netmask " << uint16_t(len) << " is too large. 0..128 is allowed in IPv6");
    }
...
IOAddress firstAddrInPrefix4(const IOAddress& prefix, uint8_t len) {
    if (len > 32) {
        isc_throw(isc::BadValue,
                  "Netmask " << uint16_t(len) << " is too large. 0..32 is allowed in IPv4");
    }
...
IOAddress getNetmask4(uint8_t len) {
    if (len > 32) {
        isc_throw(BadValue, "Invalid netmask size " << static_cast<unsigned>(len)
...
uint64_t addrsInRange(const IOAddress& min, const IOAddress& max) {
    ...
    if (max < min) {
        isc_throw(BadValue, min.toText() << " must not be greater than " << max.toText());
    }
...
std::tuple<IOAddress, IOAddress>
applyThresholdToRange(IOAddress const& min, IOAddress const& max, uint8_t const threshold) {
    // 0% means deactivated pool.
    if (threshold == 0) {
        // Let's use OutOfRange to signal this, it should not be used elsewhere.
        isc_throw(OutOfRange, "A threshold of 0% means a deactivated pool");
    }

    uint64_t address_count(addrsInRange(min, max));

    address_count = losslessPercentage(address_count, threshold);

    // address_count == 0 means deactivated pool. Even though we should never reach here, let's
    // throw here as well to prevent underflow in the subtraction below.
    if (address_count == 0) {
        // Let's use OutOfRange to signal this, it should not be used elsewhere.
        isc_throw(OutOfRange, "A percent of: " << std::to_string(threshold)
                                               << "% in the address space " << min.toText()
                                               << " to " << max.toText()
                                               << " works out to 0 addresses.");
    }

    return std::make_tuple(min, IOAddress::add(min, address_count - 1));
}

uint8_t applyThresholdToRange(uint8_t const pool_length,
                              uint8_t const delegated_length,
    ...
    uint64_t prefix_count(prefixesInRange(pool_length, delegated_length));
    // pool_length == delegated_length, percentage out of 1 does not make sense
    if (prefix_count == 1) {
        return pool_length;
    }

    prefix_count = losslessPercentage(prefix_count, threshold);
    // 0 prefixes means deactivated pool. Even though we should never reach here, let's
    // throw here as well to prevent log2 of 0.
    if (prefix_count == 0) {
        // Let's use OutOfRange to signal this, it should not be used elsewhere.
        isc_throw(OutOfRange, "A percent of: " << std::to_string(threshold)
                                               << "% in a pool with delegated-len: "
                                               << std::to_string(delegated_length)
                                               << " and pool-len: " << std::to_string(pool_length)
                                               << " works out to 0 prefixes.");
    }

    return delegated_length - ceil(log2(prefix_count));
}
