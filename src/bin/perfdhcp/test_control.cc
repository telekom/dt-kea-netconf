...
TestControl::processReceivedPacket4(const Pkt4Ptr& pkt4) {
    if (pkt4->getType() == DHCPOFFER) {
        PktPtr const& pkt(stats_mgr_.passRcvdPacket(ExchangeType::DO, pkt4));
        Pkt4Ptr const& discover_pkt4(std::dynamic_pointer_cast<Pkt4>(pkt));
    ...
    if (options_.getAddrUnique()) {
        ...
        for (OptionCollection::iterator opt = pkt6->options_.begin();
            ...
            case D6O_IA_PD: {
                ...
                auto ret = current.emplace(std::dynamic_pointer_cast<
...
TestControl::validateIA(const Pkt6Ptr& pkt6) {
    ...
    if (pkt6->getOption(D6O_IA_PD)) {
        iapref = std::dynamic_pointer_cast<Option6IAPrefix>(
            pkt6->getOption(D6O_IA_PD)->getOption(D6O_IAPREFIX));
    }
    if (pkt6->getOption(D6O_IA_NA)) {
        iaaddr = std::dynamic_pointer_cast<Option6IAAddr>(
            pkt6->getOption(D6O_IA_NA)->getOption(D6O_IAADDR));
    }
...
TestControl::processReceivedPacket6(const Pkt6Ptr& pkt6) {
    ...
    if (packet_type == DHCPV6_ADVERTISE) {
        ...
        Pkt6Ptr solicit_pkt6(std::dynamic_pointer_cast<Pkt6>(pkt));
        CommandOptions::ExchangeMode xchg_mode = options_.getExchangeMode();
...
TestControl::consumeReceivedPackets() {
    ...
    while ((pkt = receiver_.getPkt())) {
        pkt_count += 1;
        if (options_.getIpVersion() == 4) {
            Pkt4Ptr pkt4 = std::dynamic_pointer_cast<Pkt4>(pkt);
            processReceivedPacket4(pkt4);
        } else {
            Pkt6Ptr pkt6 = std::dynamic_pointer_cast<Pkt6>(pkt);
...
TestControl::sendDiscover4(const std::vector<uint8_t>& template_buf,
    ...
    setDefaults4(std::static_pointer_cast<Pkt4>(pkt4));
    // Pack the input packet buffer to output buffer so as it can
    // be sent to server.
    pkt4->rawPack();
    socket_.send(std::static_pointer_cast<Pkt4>(pkt4));
    if (!preload) {
        // Update packet stats.
        stats_mgr_.passSentPacket(ExchangeType::DO,
                                    std::static_pointer_cast<Pkt4>(pkt4));
    }
    saveFirstPacket(pkt4);
}
...
TestControl::sendRequestFromAck() {
    ...
    Pkt4Ptr ack;
    if (options_.renewOnly()) {
        ack = ack_storage_.getNext();
    } else {
        ack = ack_storage_.getRandom();
    }
    if (!ack) {
        return (false);
    }
...
TestControl::sendMessageFromReply(const uint16_t msg_type) {
    ...
    Pkt6Ptr reply;
    if (options_.renewOnly()) {
        reply = reply_storage_.getNext();
    } else {
        reply = reply_storage_.getRandom();
    }
    if (!reply) {
        return (false);
    }
...
TestControl::sendRequest4(const std::vector<uint8_t>& template_buf,
    ...
    if (options_.isUseFirst() &&
        (first_packet_serverid_.size() > 0)) {
        std::shared_ptr<LocalizedOption>
...
TestControl::sendRequest6(const std::vector<uint8_t>& template_buf,
    ...
    // Set IA_NA
    std::shared_ptr<Option6IA> opt_ia_na_advertise =
        std::static_pointer_cast<Option6IA>(advertise_pkt6->getOption(D6O_IA_NA));
...
#ifdef TERASTREAM
    if (options_.getLocalPort()) {
        pkt->setLocalPort(options_.getLocalPort());
    }
#endif  // TERASTREAM
    // Server's port (67)
