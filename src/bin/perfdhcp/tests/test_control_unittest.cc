...
public:
    ...
    FakeTestControlPerfSocket() :
        iface_(std::make_shared<Iface>("fake", 0)),
        sent_cnt_(0),
        recv_cnt_(0) {};
...
public:
    ...
    typedef std::shared_ptr<IncrementalGenerator> IncrementalGeneratorPtr;
...
public:
    ...
    void testPkt4Exchange(int iterations_num,
        ...
        for (int i = 0; i < iterations_num; ++i) {
            ...
            // packet drops.
            if (i < receive_num) {
                std::shared_ptr<Pkt4> offer_pkt4(createOfferPkt4(transid));
    ...
    void testSendRenew4() {
        ...
        std::shared_ptr<NakedTestControl::IncrementalGenerator>
            generator(new NakedTestControl::IncrementalGenerator());
        tc.setTransidGenerator(generator);
    ...
    createAdvertisePkt6(NakedTestControl &tc, const uint32_t transid,
        ...
        std::shared_ptr<Pkt6> advertise(std::make_shared<Pkt6>(DHCPV6_ADVERTISE, transid));
...
TEST_F(TestControlTest, Options6) {
    ...
    uint16_t elapsed_time = opt_elapsed_time->getUint16();
...
TEST_F(TestControlTest, Packet4) {
    ...
    std::shared_ptr<Pkt4> pkt4(new Pkt4(DHCPDISCOVER, transid));
    ...
    EXPECT_EQ(10547, pkt4->getLocalPort());
...
TEST_F(TestControlTest, Packet6) {
    ...
    NakedTestControl tc(opt);
    uint32_t transid = 123;
    std::shared_ptr<Pkt6> pkt6(new Pkt6(DHCPV6_SOLICIT, transid));
    ...
    EXPECT_EQ(10547, pkt6->getLocalPort());
