...
TEST_F(PerfPkt4Test, Constructor) {
    ...
    std::unique_ptr<PerfPkt4> pkt1(new PerfPkt4(data,
                                                  sizeof(data),
                                                  offset_transid[0]));
    ...
    std::unique_ptr<PerfPkt4> pkt2(new PerfPkt4(data, sizeof(data),
                                                  offset_transid[1],
                                                  transid));
    ...
    std::unique_ptr<PerfPkt4> pkt3(new PerfPkt4(data, sizeof(data)));
...
TEST_F(PerfPkt4Test, RawPack) {
    ...
    std::unique_ptr<PerfPkt4> pkt(new PerfPkt4(&buf[0], buf.size()));
...
TEST_F(PerfPkt4Test, RawUnpack) {
    ...
    opt_merit = std::dynamic_pointer_cast<LocalizedOption>
        (pkt->getOption(DHO_MERIT_DUMP));
    opt_msg_type = std::dynamic_pointer_cast<LocalizedOption>
...
TEST_F(PerfPkt4Test, InvalidOptions) {
    // Create new packet.
    std::vector<uint8_t> buf = capture();
    std::unique_ptr<PerfPkt4> pkt1(new PerfPkt4(&buf[0], buf.size()));
    ...
    std::unique_ptr<PerfPkt4> pkt2(new PerfPkt4(&buf[0], buf.size()));
...
TEST_F(PerfPkt4Test, UnpackTransactionId) {
    ...
    std::unique_ptr<PerfPkt4> pkt1(new PerfPkt4(&in_data[0],
                                                  in_data.size(),
                                                  offset_transid[0]));
    ...
    std::unique_ptr<PerfPkt4> pkt2(new PerfPkt4(&in_data[0],
                                                  in_data.size(),
                                                  offset_transid[1]));
