...
typedef std::shared_ptr<Pkt4Modifiable> Pkt4ModifiablePtr;
...
public:
    ...
    /// or sent (if false)
    void passMultiplePackets6(const std::shared_ptr<StatsMgr> stats_mgr,
    ...
    void passDOPacketsWithDelay(const std::shared_ptr<StatsMgr> stats_mgr,
                                unsigned int delay,
                                uint32_t transid) {
        std::shared_ptr<Pkt4> sent_packet(createPacket4(DHCPDISCOVER,
                                                      transid));
        ASSERT_NO_THROW(
        ...
        std::shared_ptr<Pkt4> rcvd_packet(createPacket4(DHCPOFFER,
                                                      transid));
        ASSERT_NO_THROW(
    ...
    void testSendReceiveCollected(const size_t transid_index) {
        CommandOptions opt;
        std::unique_ptr<StatsMgr> stats_mgr(new StatsMgr(opt));
...
TEST_F(StatsMgrTest, Constructor) {
    ...
    EXPECT_DOUBLE_EQ(0.0, stats_mgr->getAvgDelay(ExchangeType::DO));
    EXPECT_DOUBLE_EQ(0.0, stats_mgr->getStdDevDelay(ExchangeType::DO));
...
TEST_F(StatsMgrTest, MultipleExchanges) {
    CommandOptions opt;
    std::shared_ptr<StatsMgr> stats_mgr(new StatsMgr(opt));
...
TEST_F(StatsMgrTest, Orphans) {
    ...
    for (int i = 0; i < packets_num; i += 2) {
        std::shared_ptr<Pkt4> sent_packet(createPacket4(DHCPDISCOVER, i));
        ASSERT_NO_THROW(
            stats_mgr->passSentPacket(ExchangeType::DO, sent_packet)
        );
    }
    ...
    for (int i = 0; i < packets_num; ++i) {
        std::shared_ptr<Pkt4> rcvd_packet(createPacket4(DHCPOFFER, i));
        ASSERT_NO_THROW(
            stats_mgr->passRcvdPacket(ExchangeType::DO, rcvd_packet);
        );
    }
...
TEST_F(StatsMgrTest, PrintStats) {
    ...
    std::shared_ptr<StatsMgr> stats_mgr2(new StatsMgr(opt));
