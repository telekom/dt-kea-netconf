...
template <>
struct adapter_Pkt<isc::dhcp::DHCP_SPACE_V4> {
    using type = isc::dhcp::Pkt4;
};

template <>
struct adapter_Pkt<isc::dhcp::DHCP_SPACE_V6> {
    using type = isc::dhcp::Pkt6;
};
...
template <isc::dhcp::DhcpSpaceType D>
using PktTPtr = std::shared_ptr<PktT<D>>;
/// @}
...
public:
    ...
    typedef std::shared_ptr<NumberGenerator> NumberGeneratorPtr;
    ...
    uint64_t replyStorageSize() {
        if (options_.getIpVersion() == 4) {
            return ack_storage_.size();
        } else {
            return reply_storage_.size();
        }
    }

    void loadReplyStorageFromFile() {
        if (options_.getIpVersion() == 4) {
            loadReplyStorageFromFileT<isc::dhcp::DHCP_SPACE_V4>(ack_storage_);
        } else {
            loadReplyStorageFromFileT<isc::dhcp::DHCP_SPACE_V6>(reply_storage_);
        }
    }

    void saveReplyStorageToFile() {
        if (options_.getIpVersion() == 4) {
            saveReplyStorageToFileT<isc::dhcp::DHCP_SPACE_V4>(ack_storage_);
        } else {
            saveReplyStorageToFileT<isc::dhcp::DHCP_SPACE_V6>(reply_storage_);
        }
    }

    template <isc::dhcp::DhcpSpaceType D>
    void addToReplyStorage(PacketStorage<PktT<D>>& storage, PktTPtr<D> const& packet) {
        storage.append(packet);
    }

    static constexpr char DELIMITER = 0x4d;
    static constexpr size_t DELIMITER_COUNT = 16;
    template <isc::dhcp::DhcpSpaceType D>
    void loadReplyStorageFromFileT(PacketStorage<PktT<D>>& storage) {
        isc::util::Dir make_sure_it_exists("/var/db");
        isc::util::File f("/var/db/kea-reply-storage.bin");
        std::string content;
        f >> content;
        ...
        while (std::getline(ss, to, DELIMITER)) {
            if (to.empty()) {
                // Might be inside delimiter.
                i = (i + 1) % (DELIMITER_COUNT - 1);
                if (i == 0) {
                    PktTPtr<D> packet(std::make_shared<PktT<D>>(
                        reinterpret_cast<const uint8_t*>(line.data()), line.length()));
                    bool unpacked(true);
                    try {
                        packet->unpack();
                    } catch (isc::Unexpected const&) {
                        unpacked = false;
                    }
                    if (unpacked) {
                        addToReplyStorage<D>(storage, packet);
                    }
                    line = std::string();
                }
            } else {
                // Certainly not inside delimiter.
                if (!line.empty()) {
                    // Add all previous fake delimiters to the packet and reset i.
                    for (; i > 0; --i) {
                        line += DELIMITER;
                    }
                }
    ...
    template <isc::dhcp::DhcpSpaceType D>
    void saveReplyStorageToFileT(PacketStorage<PktT<D>>& storage) {
        std::string content;
        forAllPacketsInReplyStorage<D>(storage, [&](PktTPtr<D>& packet) {
            packet->pack();
            isc::util::OutputBuffer ob(packet->getBuffer());
            std::string s(reinterpret_cast<const char*>(ob.getData()), ob.getLength());
            content += s;
        ...
        isc::util::Dir make_sure_it_exists("/var/db");
        isc::util::File f("/var/db/kea-reply-storage.bin", content);
    }

    template <isc::dhcp::DhcpSpaceType D>
    void forAllPacketsInReplyStorage(PacketStorage<PktT<D>>& storage,
                                     std::function<void(PktTPtr<D>&)> f) {
        isc::dhcp::PktPtr first_packet;
        while (true) {
            PktTPtr<D> packet(storage.getNext());
            if (!packet) {
                break;
            }
            if (first_packet) {
                if (std::addressof(*first_packet) == std::addressof(*packet)) {
                    break;
                }
            } else {
                first_packet = packet;
