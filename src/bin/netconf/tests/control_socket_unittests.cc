...
static ChangeCollection empty_changes;
...
TEST(StdoutControlSocketTest, createControlSocket) {
    ...
    ASSERT_TRUE(cfg);
    ControlSocketBasePtr cs(
        createControlSocket(std::make_shared<CfgModel>("dhcp", cfg)));
    ASSERT_TRUE(cs);
...
TEST(StdoutControlSocketTest, configGet) {
    ...
    ASSERT_TRUE(scs);
    EXPECT_THROW(scs->configGet("foo", empty_changes, nullptr), NotImplemented);
}
...
TEST(StdoutControlSocketTest, configTest) {
    ...
    ASSERT_TRUE(scs);
    ElementPtr answer;
    ASSERT_NO_THROW(answer = scs->configTest(ElementPtr(), {}, {}, "foo"));
...
TEST(StdoutControlSocketTest, configSet) {
    ...
    ElementPtr answer;
    ASSERT_NO_THROW(answer = tscs->configSet(json, nullptr, nullptr, {}, "foo"));
...
TEST_F(UnixControlSocketTest, configGet) {
    ...
    // Try configGet.
    ElementPtr reflected;
    EXPECT_NO_THROW(reflected = ucs->configGet("foo", empty_changes, nullptr));
    ASSERT_TRUE(reflected);
    ASSERT_EQ(Element::map, reflected->getType());
    ElementPtr command = reflected->get("received");
    ASSERT_TRUE(command);
...
TEST_F(UnixControlSocketTest, configSet) {
    ...
    ElementPtr reflected;
    EXPECT_NO_THROW(reflected = ucs->configSet(json, nullptr, nullptr, {}, "foo"));
    ASSERT_TRUE(reflected);
    ASSERT_EQ(Element::map, reflected->getType());
    ElementPtr command = reflected->get("received");
    ASSERT_TRUE(command);
...
TEST_F(UnixControlSocketTest, timeout) {
    ...
    EXPECT_THROW(ucs->configGet("foo", empty_changes, nullptr), ControlSocketError);
    signalReady();
}
...
using GenericResponsePtr = std::shared_ptr<GenericResponse>;
...
protected:
    ...
    createDynamicHttpResponse(const ConstHttpRequestPtr& request) {
        ...
        ConstPostHttpRequestJsonPtr request_json =
            std::dynamic_pointer_cast<const PostHttpRequestJson>(request);
        if (!request_json) {
            isc_throw(Unexpected, "request is not JSON");
        }
        ElementPtr body = request_json->getBodyAsJson();
        if (!body) {
            isc_throw(Unexpected, "can't get JSON from request");
        }

        // Check for the special partial JSON.
        ElementPtr arguments = body->get("arguments");
...
TEST_F(HttpControlSocketTest, configGet) {
    ...
    // Try configGet.
    ElementPtr reflected;
    EXPECT_NO_THROW(reflected = hcs->configGet("foo", empty_changes, nullptr));
    stop();
...
TEST_F(HttpControlSocketTest, configGetCA) {
    ...
    // Try configGet.
    ElementPtr reflected;
    EXPECT_NO_THROW(reflected = hcs->configGet("ca", empty_changes, nullptr));
    stop();
...
TEST_F(HttpControlSocketTest, configSet) {
    ...
    // Try configSet.
    ElementPtr reflected;
    EXPECT_NO_THROW(reflected = hcs->configSet(json, nullptr, nullptr, {}, "foo"));
    stop();
...
TEST_F(HttpControlSocketTest, configSetCA) {
    ...
    // Try configSet.
    ElementPtr reflected;
    EXPECT_NO_THROW(reflected = hcs->configSet(json, nullptr, nullptr, {}, "ca"));
    stop();
...
TEST_F(HttpControlSocketTest, connectionRefused) {
    ...
    // Try configGet: it should get a communication error,
    try {
        hcs->configGet("foo", empty_changes, nullptr);
...
TEST_F(HttpControlSocketTest, partial) {
    ...
    try {
        hcs->configSet(json, nullptr, nullptr, {}, "foo");
    } catch (const ControlSocketError& ex) {
