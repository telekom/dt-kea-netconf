...
public:
    ...
    /// - derives global parameters to models (flags for now)
    /// @param global scope to be modified if needed
    /// @return number of default values derived
    static size_t deriveParameters(isc::data::ElementPtr global);

    /// @brief Adds default values to a model entry.
    ///
    /// Adds model specific defaults, e.g. the default model.
    ///
    /// @param model server element / entry value
    /// @return returns the number of default values added
    static size_t setModelDefaults(isc::data::ElementPtr const& model);
    ...
    static const isc::data::SimpleDefaults SUBSCRIPTION_DEFAULTS;
    static const isc::data::ParamsList INHERIT_TO_MODELS;
};
