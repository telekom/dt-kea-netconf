...
DControllerBasePtr& NetconfController::instance() {
...
void NetconfController::processSignal(int signum) {
    if (signum == SIGHUP) {
...
DProcessBase* NetconfController::createProcess() {
...
isc::data::ElementPtr NetconfController::parseFile(const string& name) {
    ParserContext parser;
    return parser.parseFile(name, ParserContext::PARSER_NETCONF);
}

NetconfController::NetconfController() : DControllerBase(netconf_app_name_, netconf_bin_name_) {
}

NetconfProcessPtr NetconfController::getNetconfProcess() {
    return dynamic_pointer_cast<NetconfProcess>(getProcess());
}
