...
using NetconfConfigPtr = std::shared_ptr<NetconfConfig>;
...
public:
    ...
    /// @brief Returns non-const reference to the model map.
    ///
    /// @return non-const reference to the model map.
    CfgModelsListPtr& getModelConfigs() {
        return models_list_;
    }

    /// @brief Returns const reference to the model map.
    ///
    /// @return const reference to the model map.
    const CfgModelsListPtr& getModelConfigs() const {
        return models_list_;
    }
...
public:
    ...
    NetconfConfigPtr getNetconfConfig() {
        return std::dynamic_pointer_cast<NetconfConfig>(getContext());
    }
...
protected:
    ...
    virtual isc::data::ElementPtr parse(isc::data::ElementPtr config, bool check_only);
...
using NetconfCfgMgrPtr = std::shared_ptr<NetconfCfgMgr>;
