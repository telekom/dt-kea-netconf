...
CtrlAgentCfgMgr::getConfigSummary(const uint32_t /*selection*/) {
    ...
    for (auto const& lib : libs) {
        s << std::get<isc::hooks::LIBRARY>(lib) << " ";
    }
...
isc::data::ElementPtr
CtrlAgentCfgMgr::parse(isc::data::ElementPtr config_set, bool check_only) {
...
CtrlAgentCfgContext::getControlSocketInfo(const std::string& service) const {
    ...
    return ((si != ctrl_sockets_.end()) ? si->second : ElementPtr());
}

void
CtrlAgentCfgContext::setControlSocketInfo(const isc::data::ElementPtr& control_socket,
...
CtrlAgentCfgContext::toElement() const {
    ...
    for (auto si = ctrl_sockets_.cbegin(); si != ctrl_sockets_.cend(); ++si) {
        ElementPtr socket = UserContext::toElement(si->second);
