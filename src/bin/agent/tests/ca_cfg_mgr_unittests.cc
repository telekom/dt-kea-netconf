...
TEST(CtrlAgentCfgMgr, construction) {
    std::unique_ptr<CtrlAgentCfgMgr> cfg_mgr;
...
TEST(CtrlAgentCfgMgr, contextSocketInfo) {
    ...
    ElementPtr socket1 = Element::fromJSON("{ \"socket-type\": \"unix\",\n"
                                                "  \"socket-name\": \"socket1\" }");
    ElementPtr socket2 = Element::fromJSON("{ \"socket-type\": \"unix\",\n"
                                                "  \"socket-name\": \"socket2\" }");
    ElementPtr socket3 = Element::fromJSON("{ \"socket-type\": \"unix\",\n"
                                                "  \"socket-name\": \"socket3\" }");
...
TEST(CtrlAgentCfgMgr, contextSocketInfoCopy) {
    ...
    ElementPtr exp_param(new StringElement("myparam"));
    libs.add(exp_name, "parameters", exp_param);

    // Make a copy.
    ConfigPtr copy_base(ctx.clone());
    CtrlAgentCfgContextPtr copy = std::dynamic_pointer_cast<CtrlAgentCfgContext>(copy_base);
    ASSERT_TRUE(copy);
    ...
    EXPECT_EQ(exp_name, std::get<LIBRARY>(libs2[0]));
    ASSERT_TRUE(std::get<PARAMETERS_VALUE>(libs2[0]));
    EXPECT_EQ(exp_param->str(), std::get<PARAMETERS_VALUE>(libs2[0])->str());
}
...
public:
    ...
    void configParse(const char* config, int expected_answer) {
        ...
        ElementPtr json = parser.parseString(config, ParserContext::PARSER_SUB_AGENT);
...
TEST_F(AgentParserTest, configParseSocketDhcp4) {
    ...
    CtrlAgentCfgContextPtr ctx = cfg_mgr_.getCtrlAgentCfgContext();
    ASSERT_TRUE(ctx);
    ElementPtr socket = ctx->getControlSocketInfo("dhcp4");
    ASSERT_TRUE(socket);
...
TEST_F(AgentParserTest, configParseSocketD2) {
    ...
    CtrlAgentCfgContextPtr ctx = cfg_mgr_.getCtrlAgentCfgContext();
    ASSERT_TRUE(ctx);
    ElementPtr socket = ctx->getControlSocketInfo("d2");
    ASSERT_TRUE(socket);
...
TEST_F(AgentParserTest, configParseSocketDhcp6) {
    ...
    CtrlAgentCfgContextPtr ctx = cfg_mgr_.getCtrlAgentCfgContext();
    ASSERT_TRUE(ctx);
    ElementPtr socket = ctx->getControlSocketInfo("dhcp6");
    ASSERT_TRUE(socket);
...
TEST_F(AgentParserTest, configParse3Sockets) {
    ...
    CtrlAgentCfgContextPtr ctx = cfg_mgr_.getCtrlAgentCfgContext();
    ASSERT_TRUE(ctx);
    ElementPtr socket2 = ctx->getControlSocketInfo("d2");
    ElementPtr socket4 = ctx->getControlSocketInfo("dhcp4");
    ElementPtr socket6 = ctx->getControlSocketInfo("dhcp6");
    ASSERT_TRUE(socket2);
    EXPECT_EQ("{ \"socket-name\": \"/tmp/socket-d2\", \"socket-type\": \"unix\" }",
              socket2->str());
    ASSERT_TRUE(socket4);
...
TEST_F(AgentParserTest, comments) {
    ...
    ElementPtr ctx = agent_ctx->getContext();
    ASSERT_TRUE(ctx);
    ...
    ElementPtr socket4 = agent_ctx->getControlSocketInfo("dhcp4");
    ASSERT_TRUE(socket4);

    // Check DHCP4 control socket user context.
    ElementPtr ctx4 = socket4->get("user-context");
    ASSERT_TRUE(ctx4);
    ...
    ElementPtr socket6 = agent_ctx->getControlSocketInfo("dhcp6");
    ASSERT_TRUE(socket6);

    // Check DHCP6 control socket user context.
    ElementPtr ctx6 = socket6->get("user-context");
    ASSERT_TRUE(ctx6);
