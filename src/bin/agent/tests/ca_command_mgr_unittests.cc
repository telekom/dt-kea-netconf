...
public:
    ...
    /// the answer.
    void checkAnswer(const ElementPtr& answer, const int expected_code0 = 0,
    ...
    ElementPtr createCommand(const std::string& command_name,
    ...
    void testForward(const std::string& configured_service,
        ...
        ElementPtr command = createCommand("foo", service);
        ElementPtr answer = mgr_.handleCommand("foo", ElementPtr(),
                                                    command);
...
TEST_F(CtrlAgentCommandMgrTest, noClientSocket) {
    ElementPtr command = createCommand("foo", "dhcp4");
    ElementPtr answer = mgr_.handleCommand("foo", ElementPtr(),
                                                command);
...
TEST_F(CtrlAgentCommandMgrTest, noServerSocket) {
    ...
    ElementPtr command = createCommand("foo", "dhcp6");
    ElementPtr answer = mgr_.handleCommand("foo", ElementPtr(),
                                                command);
...
TEST_F(CtrlAgentCommandMgrTest, forwardListCommands) {
    ...
    ElementPtr command = createCommand("list-commands", "dhcp4");
    ElementPtr answer = mgr_.handleCommand("list-commands", ElementPtr(),
                                                command);
