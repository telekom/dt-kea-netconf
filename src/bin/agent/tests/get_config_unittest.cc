...
void
pathReplacer(ElementPtr ca_cfg) {
    ElementPtr hooks_libs = ca_cfg->get("hooks-libraries");
    if (!hooks_libs || hooks_libs->empty()) {
        return;
    }
    ElementPtr first_lib = hooks_libs->get(0);
    std::string lib_path(BASIC_CALLOUT_LIBRARY);
    first_lib->set("library", Element::create(lib_path));
    first_lib->set("library-path", Element::create(lib_path));
}
...
public:
    ...
    executeConfiguration(const std::string& config, const char* operation) {
        ...
        ElementPtr ca = json->get("Control-agent");
        if (!ca) {
    ...
    std::unique_ptr<NakedAgentCfgMgr> srv_; ///< CA server under test
