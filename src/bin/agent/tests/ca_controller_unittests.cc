...
public:
    ...
    CtrlAgentProcessPtr getCtrlAgentProcess() {
        return (std::dynamic_pointer_cast<CtrlAgentProcess>(getProcess()));
    }
    ...
    void testUnixSocketInfo(const std::string& service,
        ...
        ElementPtr sock_info = ctx->getControlSocketInfo(service);
...
TEST_F(CtrlAgentControllerTest, basicInstanceTesting) {
    ...
    ASSERT_NO_THROW(std::dynamic_pointer_cast<CtrlAgentController>(controller));
...
TEST_F(CtrlAgentControllerTest, registeredCommands) {
    ...
    const CtrlAgentControllerPtr& ctrl =
        std::dynamic_pointer_cast<CtrlAgentController>(base);
    ASSERT_TRUE(ctrl);
...
TEST_F(CtrlAgentControllerTest, configWrite) {
    ...
    ElementPtr cmd = Element::fromJSON(cmd_txt);
    ElementPtr params = Element::fromJSON("{\"filename\": \"" + file + "\" }");
    CtrlAgentCommandMgr& mgr_ =  CtrlAgentCommandMgr::instance();

    // Send the command
    ElementPtr answer = mgr_.handleCommand("config-write", params, cmd);
    ...
    ElementPtr from_file = Element::fromJSONFile(file, true);
    ASSERT_TRUE(from_file);
    ElementPtr ca = from_file->get("Control-agent");
    ASSERT_TRUE(ca);
...
TEST_F(CtrlAgentControllerTest, statusGet) {
    ...
    ElementPtr response;
    ASSERT_NO_THROW(response = ctrl->statusGetHandler("status-get", ElementPtr()));
...
TEST_F(CtrlAgentControllerTest, shutdown) {
    ...
    ElementPtr cmd = Element::fromJSON("{ \"command\": \"shutdown\"}");
    ElementPtr params;
    ElementPtr answer;
...
TEST_F(CtrlAgentControllerTest, shutdownExitValue) {
    ...
    ElementPtr cmd = Element::fromJSON("{ \"command\": \"shutdown\"}");
    ElementPtr params = Element::fromJSON("{ \"exit-value\": 77 }");
    ElementPtr answer;
    answer = CtrlAgentCommandMgr::instance().handleCommand("shutdown",
                                                           params, cmd);
