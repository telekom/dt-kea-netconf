...
CtrlAgentProcess::run() {
    ...
    try {
        ...
        CtrlAgentControllerPtr controller =
            std::dynamic_pointer_cast<CtrlAgentController>(
...
isc::data::ElementPtr
CtrlAgentProcess::shutdown(isc::data::ElementPtr /*args*/) {
    setShutdownFlag(true);
    return (isc::config::createAnswer(0, "Control Agent is shutting down"));
}

isc::data::ElementPtr
CtrlAgentProcess::configure(isc::data::ElementPtr config_set,
                            bool check_only) {
    ...
    ElementPtr answer = getCfgMgr()->simpleParseConfig(config_set,
                                                            check_only,
                                                            [this]() {
        ConfigPtr base_ctx = getCfgMgr()->getContext();
        CtrlAgentCfgContextPtr
            ctx = std::dynamic_pointer_cast<CtrlAgentCfgContext>(base_ctx);
...
CtrlAgentProcess::getCtrlAgentCfgMgr() {
    return (std::dynamic_pointer_cast<CtrlAgentCfgMgr>(getCfgMgr()));
}
