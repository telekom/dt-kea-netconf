...
size_t AgentSimpleParser::setAllDefaults(const isc::data::ElementPtr& global) {
    ...
    ElementPtr sockets = global->get("control-sockets");
    if (sockets) {
        ElementPtr d2(sockets->get("d2"));
        if (d2) {
            cnt += SimpleParser::setDefaults(d2, SOCKET_DEFAULTS);
        }

        ElementPtr d4(sockets->get("dhcp4"));
        if (d4) {
            cnt += SimpleParser::setDefaults(d4, SOCKET_DEFAULTS);
        }

        ElementPtr d6(sockets->get("dhcp6"));
        if (d6) {
...
AgentSimpleParser::parse(const CtrlAgentCfgContextPtr& ctx,
    ...
    ElementPtr ctrl_sockets = config->get("control-sockets");
    if (ctrl_sockets) {
