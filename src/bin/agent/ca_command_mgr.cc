...
CtrlAgentCommandMgr::handleCommandInternal(std::string cmd_name,
    ...
    if (services->empty()) {
        ...
        ElementPtr answer(HookedCommandMgr::handleCommand(cmd_name, params, original_cmd));

        try {
            // Check what error code was returned by the handler.
            int rcode = 0;
            ElementPtr text = parseAnswer(rcode, answer);
...
CtrlAgentCommandMgr::forwardCommand(const std::string& service,
    ...
    // configuration.
    std::shared_ptr<CtrlAgentController> controller =
        std::dynamic_pointer_cast<CtrlAgentController>(CtrlAgentController::instance());
    if (controller) {
    ...
    ElementPtr socket_info = ctx->getControlSocketInfo(service);
    if (!socket_info) {
