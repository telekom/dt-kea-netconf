...
typedef std::shared_ptr<CtrlAgentCfgContext> CtrlAgentCfgContextPtr;
...
public:
    ...
    /// That data is stored as ElementPtr (a shared pointer) to the actual data.
    ///
...
private:
    ...
    std::map<std::string, isc::data::ElementPtr> ctrl_sockets_;
...
public:
    ...
    CtrlAgentCfgContextPtr getCtrlAgentCfgContext() {
        return (std::dynamic_pointer_cast<CtrlAgentCfgContext>(getContext()));
    }
...
typedef std::shared_ptr<CtrlAgentCfgMgr> CtrlAgentCfgMgrPtr;
