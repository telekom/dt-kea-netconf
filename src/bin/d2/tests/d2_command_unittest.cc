...
typedef std::shared_ptr<NakedD2Controller> NakedD2ControllerPtr;
...
public:
    ...
    D2ProcessPtr getProcess() {
        return (std::dynamic_pointer_cast<D2Process>(DControllerBase::getProcess()));
    }
...
public:
    ...
    void createUnixChannelServer() {
        ...
        ASSERT_TRUE(proc);
        ElementPtr answer = proc->configure(config, false);
        ASSERT_TRUE(answer);
        ASSERT_NO_THROW(d2Controller()->registerCommands());

        int status = 0;
        ElementPtr txt = parseAnswer(status, answer);
    ...
    void checkListCommands(const ElementPtr& rsp, const string command) {
        ElementPtr params;
        int status_code = -1;
    ...
    static ElementPtr
    longResponseHandler(const string&, const ElementPtr&) {
...
TEST_F(CtrlChannelD2Test, longResponse) {
    ...
    string reference_response = longResponseHandler("foo", ElementPtr())->str();
