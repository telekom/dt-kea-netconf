...
public:
    ...
    void runInvalidTimeoutTest() {
        ...
        D2UpdateMessage update_message(D2UpdateMessage::OUTBOUND);
        ASSERT_NO_THROW(update_message.setRcode(Rcode(Rcode::NOERROR_CODE)));
        ASSERT_NO_THROW(update_message.setZone(Name("example.com"), RRClass::IN()));
        ...
        EXPECT_NO_THROW(dns_client_->doUpdate(service_, IOAddress(TEST_ADDRESS),
                                           TEST_PORT, update_message, timeout));
    ...
    void runTSIGTest(TSIGKeyPtr client_key, TSIGKeyPtr server_key,
        ...
        expected_++;
        dns_client_->doUpdate(service_, IOAddress(TEST_ADDRESS), TEST_PORT,
                              update_message, timeout, client_key);
