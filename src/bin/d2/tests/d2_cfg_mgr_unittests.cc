...
public:
    ...
    ::testing::AssertionResult runConfigOrFail(const std::string& json,
        ...
        try {
            ...
            data::ElementPtr elem(parser_context.parseString(json, D2ParserContext::
                                                  PARSER_SUB_DHCPDDNS));
...
TEST(D2CfgMgr, construction) {
    std::unique_ptr<D2CfgMgr> cfg_mgr;
...
TEST_F(D2CfgMgrTest, fullConfig) {
    ...
    ElementPtr ctrl_sock = context->getControlSocketInfo();
...
TEST_F(D2CfgMgrTest, configPermutations) {
    ...
    BOOST_FOREACH(test, tests->get("test-list")->listValue()) {
        ...
        isc::data::ElementPtr data = test->get("data");
...
TEST_F(D2CfgMgrTest, comments) {
    ...
    ElementPtr ctx = d2_context->getContext();
    ASSERT_TRUE(ctx);
    ...
    ElementPtr ctrl_sock = d2_context->getControlSocketInfo();
    ASSERT_TRUE(ctrl_sock);
    ...
    ElementPtr key_ctx = key->getContext();
    ASSERT_TRUE(key_ctx);
    ...
    ElementPtr srv_ctx = server->getContext();
    ASSERT_TRUE(srv_ctx);
