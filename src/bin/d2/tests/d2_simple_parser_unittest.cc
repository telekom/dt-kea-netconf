...
void checkIntegerValue(const ElementPtr& element,
...
/// @param element defaulted element to check
/// @param deflt SimpleDefault which supplied the default value
void checkBooleanValue(const ElementPtr& element,
                       const SimpleDefault& deflt)  {
    ASSERT_TRUE(element);
...
/// @param element defaulted element to check
/// @param deflt SimpleDefault which supplied the default value
void checkStringValue(const ElementPtr& element,
                      const SimpleDefault& deflt)  {
    ASSERT_TRUE(element);
...
public:
    ...
    ::testing::AssertionResult parseOrFail(const std::string& json,
        ...
        try {
            ...
            data::ElementPtr elem(context.parseString(json, parser_type_));
            // Add any defaults
            setDefaults(elem);
...
protected:
    ...
    virtual void parseElement(data::ElementPtr config) {
        static_cast<void>(config);
    }
...
public:
    ...
    void parseElement(data::ElementPtr config) {
...
TEST_F(DdnsDomainParserTest, validDomain) {
    ...
    for (size_t i = 0; i < servers_json->size(); ++i) {
        ...
        ASSERT_NO_THROW(server_json = servers_json->get(i));
