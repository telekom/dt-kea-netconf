...
getProtocol(ElementPtr map, const std::string& name) {
    ElementPtr value = map->get(name);
    if (!value) {
...
getFormat(ElementPtr map, const std::string& name) {
    ElementPtr value = map->get(name);
    if (!value) {
        isc_throw(D2CfgError, "Mandatory parameter " << name
                  << " not found (" << map->getPosition() << ")");
...
D2SimpleParser::setDdnsDomainDefaults(ElementPtr domain,
    ...
    if (domain->find("dns-servers")) {
        ...
        ElementPtr servers = domain->get("dns-servers");
...
D2SimpleParser::setManagerDefaults(ElementPtr global,
    ...
    if (!global->find(mgr_name)) {
        ...
        ElementPtr mgr(global->get(mgr_name));
        ...
        if (mgr->find("ddns-domains")) {
            ElementPtr domains = mgr->get("ddns-domains");
...
void D2SimpleParser::parse(const D2CfgContextPtr& ctx,
    ...
    ElementPtr keys = config->get("tsig-keys");
    if (keys) {
        TSIGKeyInfoListParser parser;
        ctx->setKeys(parser.parse(keys));
    }

    ElementPtr fwd = config->get("forward-ddns");
    if (fwd) {
    ...
    ElementPtr rev = config->get("reverse-ddns");
    if (rev) {
        DdnsDomainListMgrParser parser;
    ...
    ElementPtr user = config->get("user-context");
    if (user) {
        ctx->setContext(user);
    }

    ElementPtr socket = config->get("control-socket");
    if (socket) {
