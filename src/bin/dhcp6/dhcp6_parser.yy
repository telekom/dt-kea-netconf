...
%token
  ...
  LAWFUL_INTERCEPTION_PARAMETERS "lawful-interception-parameters"
  MASTER_DATABASE "master-database"
  MAX_ADDRESS_UTILIZATION "max-address-utilization"
  MAX_PD_SPACE_UTILIZATION "max-pd-space-utilization"
  MAX_STATEMENT_TRIES "max-statement-tries"
  NOTIFICATIONS "notifications"
  ON_BOOT_REQUEST_CONFIGURATION_FROM_KEA_NETCONF "on-boot-request-configuration-from-kea-netconf"
  OPTION_SET_DESCRIPTION "option-set-description"
  OPTION_SET_ID "option-set-id"
  POLICY_ENGINE_PARAMETERS "policy-engine-parameters"
  PRIVACY_HISTORY_SIZE "privacy-history-size"
  PRIVACY_VALID_LIFETIME "privacy-valid-lifetime"
  PROTOCOL "protocol"
  SSL_CERT "ssl-cert"
...
global_param: data_directory
            ...
            | option_set_id
            | privacy_history_size
            | privacy_valid_lifetime
            | unknown_map_entry
            ;
...
hooks_library: LCURLY_BRACKET {
    ...
    ctx.require("library-path", ctx.loc2pos(@1), ctx.loc2pos(@4));
    ctx.stack_.pop_back();
};
...
notifications_control_socket_params: notifications_control_socket_param
                                   | notifications_control_socket_params COMMA notifications_control_socket_param
                                   ;

notifications_control_socket_param: socket_type
                                  | socket_name
                                  | user_context
                                  | comment
...
on-boot-request-configuration-from-kea-netconf: ON_BOOT_REQUEST_CONFIGURATION_FROM_KEA_NETCONF COLON BOOLEAN {
    ElementPtr b(new BoolElement($3, ctx.loc2pos(@3)));
...
allocation_type: ALLOCATION_TYPE {
    ...
    ElementPtr allocation_type(new StringElement($4, ctx.loc2pos(@4)));
...
lawful-interception-parameters: LAWFUL_INTERCEPTION_PARAMETERS {
    ctx.enter(ctx.NO_KEYWORD);
} COLON value {
    ctx.stack_.back()->set("lawful-interception-parameters", $4);
    ctx.leave();
};
...
max_address_utilization: MAX_ADDRESS_UTILIZATION COLON INTEGER {
    ElementPtr n(new IntElement($3, ctx.loc2pos(@3)));
    ctx.stack_.back()->set("max-address-utilization", n);
};

max_pd_space_utilization: MAX_PD_SPACE_UTILIZATION COLON INTEGER {
    ElementPtr n(new IntElement($3, ctx.loc2pos(@3)));
...
notifications: NOTIFICATIONS {
    ...
    ctx.stack_.push_back(m);
    ctx.enter(ctx.CONTROL_SOCKET);
} COLON LCURLY_BRACKET notifications_control_socket_params RCURLY_BRACKET {
    ctx.stack_.pop_back();
    ctx.leave();
};

option_set_description: OPTION_SET_DESCRIPTION COLON STRING {
    ElementPtr l(new StringElement($3, ctx.loc2pos(@3)));
    ctx.stack_.back()->set("option-set-description", l);
};

option_set_id: OPTION_SET_ID COLON INTEGER {
    ElementPtr l(new IntElement($3, ctx.loc2pos(@3)));
    ctx.stack_.back()->set("option-set-id", l);
};

policy-engine-parameters: POLICY_ENGINE_PARAMETERS {
    ctx.enter(ctx.NO_KEYWORD);
} COLON value {
    ctx.stack_.back()->set("policy-engine-parameters", $4);
    ctx.leave();
};

privacy_history_size: PRIVACY_HISTORY_SIZE COLON INTEGER {
    ElementPtr prf(new IntElement($3, ctx.loc2pos(@3)));
    ctx.stack_.back()->set("privacy-history-size", prf);
};

privacy_valid_lifetime: PRIVACY_VALID_LIFETIME COLON INTEGER {
    ElementPtr prf(new IntElement($3, ctx.loc2pos(@3)));
