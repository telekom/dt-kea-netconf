...
protected:
    ...
    declineIA(const Pkt6Ptr& decline, const DuidPtr& duid, int& general_status,
              std::shared_ptr<Option6IA> ia, Lease6Collection& new_leases);
    ...
    void setStatusCode(std::shared_ptr<Option6IA>& container,
...
private:
    ...
    /// @brief Updates processing time statistics for received packets
    /// @param query packet received
    /// @param processing_time in microseconds
    static void processTimeStats(const Pkt6Ptr& query, int64_t processing_time);
...
protected:
    ...
    std::shared_ptr<AllocEngine> alloc_engine_;
