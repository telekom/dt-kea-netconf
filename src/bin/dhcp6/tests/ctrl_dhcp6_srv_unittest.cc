...
public:
    ...
    std::shared_ptr<NakedControlledDhcpv6Srv> server_;
    ...
    void createUnixChannelServer() {
        ...
        ElementPtr answer = server_->processConfig(config);
        ...
        int status = 0;
        ElementPtr txt = isc::config::parseAnswer(status, answer);
    ...
    void sendUnixCommand(const std::string& command, std::string& response) {
        response = "";
        std::unique_ptr<UnixControlClient> client;
    ...
    void checkListCommands(const ElementPtr& rsp, const std::string& command) {
        ElementPtr params;
        int status_code = -1;
    ...
    void checkConfigWrite(const std::string& response_txt, int exp_status,
        ...
        int status;
        ElementPtr params = parseAnswer(status, rsp);
    ...
    static ElementPtr longResponseHandler(const std::string&,
                                               const ElementPtr&) {
...
TEST_F(CtrlDhcpv6SrvTest, commands) {
    ...
    ElementPtr result = ControlledDhcpv6Srv::processCommand("blah", params);
    ElementPtr comment = parseAnswer(rcode, result);
    ...
    ElementPtr x(new isc::data::IntElement(77));
...
TEST_F(CtrlChannelDhcpv6SrvTest, libreload) {
    ...
    libraries.push_back({CALLOUT_LIBRARY_1, "parameters", ElementPtr()});
    libraries.push_back({CALLOUT_LIBRARY_2, "parameters", ElementPtr()});
...
TEST_F(CtrlChannelDhcpv6SrvTest, configGet) {
    ...
    int status;
    ElementPtr cfg = parseAnswer(status, rsp);
...
TEST_F(CtrlChannelDhcpv6SrvTest, statusGet) {
    ...
    ElementPtr result = response->get("result");
    ASSERT_TRUE(result);
    ASSERT_EQ(Element::integer, result->getType());
    EXPECT_EQ(0, result->intValue());
    ElementPtr arguments = response->get("arguments");
...
TEST_F(CtrlChannelDhcpv6SrvTest, concurrentConnections) {
    ...
    std::unique_ptr<UnixControlClient> client1(new UnixControlClient());
    ASSERT_TRUE(client1);

    std::unique_ptr<UnixControlClient> client2(new UnixControlClient());
    ASSERT_TRUE(client2);
...
TEST_F(CtrlChannelDhcpv6SrvTest, longCommand) {
    ...
    std::thread th([this, &response, &command]() {
        ...
        std::unique_ptr<UnixControlClient> client(new UnixControlClient());
        ASSERT_TRUE(client);
...
TEST_F(CtrlChannelDhcpv6SrvTest, longResponse) {
    ...
    std::string reference_response = longResponseHandler("foo", ElementPtr())->str();
