...
public:
    ...
    std::unique_ptr<NakedDhcpv6Srv> srv_;
    ...
    static std::shared_ptr<Option6IA> callback_ia_na_;
...
std::shared_ptr<Option6IA> HooksDhcpv6SrvTest::callback_ia_na_;
...
public:
    ...
    std::shared_ptr<NakedDhcpv6Srv> server_;
...
TEST_F(HooksDhcpv6SrvTest, subnet6SselectChange) {
    ...
    ASSERT_TRUE(tmp);
    std::shared_ptr<Option6IAAddr> addr_opt =
        std::dynamic_pointer_cast<Option6IAAddr>(tmp);
...
TEST_F(HooksDhcpv6SrvTest, basicLease6ReleasePD) {
    ...
    Pkt6Ptr req = Pkt6Ptr(new Pkt6(DHCPV6_RELEASE, 1234));
    req->setRemoteAddr(IOAddress("fe80::abcd"));
    std::shared_ptr<Option6IA> ia = generateIA(D6O_IA_PD, iaid, 1500, 3000);
...
TEST_F(LoadUnloadDhcpv6SrvTest, unloadLibraries) {
    ...
    libraries.push_back({std::string(CALLOUT_LIBRARY_1), "parameters",
                                  ElementPtr()});
    libraries.push_back({std::string(CALLOUT_LIBRARY_2), "parameters",
                                  ElementPtr()});
...
TEST_F(LoadUnloadDhcpv6SrvTest, failLoadIncompatibleLibraries) {
    ...
    libraries.clear();
    libraries.push_back({std::string(CALLOUT_LIBRARY_3), "parameters",
                                  ElementPtr()});
...
TEST_F(LoadUnloadDhcpv6SrvTest, Dhcpv6SrvConfigured) {
    std::shared_ptr<ControlledDhcpv6Srv> srv(new ControlledDhcpv6Srv(0));
    ...
    ElementPtr config = Element::fromJSON(config_str);
