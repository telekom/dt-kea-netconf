...
TEST_F(NakedDhcpv6SrvTest, RequestNoSubnet) {
    ...
    std::shared_ptr<Option6IA> ia = generateIA(D6O_IA_NA, 234, 1500, 3000);
...
TEST_F(NakedDhcpv6SrvTest, RenewNoSubnet) {
    ...
    req->setRemoteAddr(IOAddress("fe80::abcd"));
    std::shared_ptr<Option6IA> ia = generateIA(D6O_IA_NA, iaid, 1500, 3000);
...
TEST_F(Dhcpv6SrvTest, DUID) {
    ...
    case DUID::DUID_UUID: {
        // not supported yet
        cout << "Test not implemented for DUID-UUID." << endl;
        break;
    }
    default:
...
TEST_F(Dhcpv6SrvTest, advertiseOptions) {
    ...
    std::shared_ptr<OptionIntArray<uint16_t> >
    ...
    std::shared_ptr<Option6AddrLst> reply_nameservers =
        std::dynamic_pointer_cast<Option6AddrLst>(tmp);
...
TEST_F(Dhcpv6SrvTest, SolicitBasic) {
    ...
    std::shared_ptr<Option6IAAddr> addr = checkIA_NA(reply, 234, subnet_->getT1(),
                                                subnet_->getT2());
    ASSERT_TRUE(addr);
...
TEST_F(Dhcpv6SrvTest, pdSolicitBasic) {
    ...
    // check that IA_NA was returned and that there's an address included
    std::shared_ptr<Option6IAPrefix> prefix = checkIA_PD(reply, 234, subnet_->getT1(),
                                                           subnet_->getT2());
    ASSERT_TRUE(prefix);
...
TEST_F(Dhcpv6SrvTest, hintZeroLifetimeSolicit) {
    ...
    std::shared_ptr<Option6IAPrefix> prefix = checkIA_PD(reply, 234,
                                                           subnet_->getT1(),
                                                           subnet_->getT2());
    ASSERT_TRUE(prefix);
...
TEST_F(Dhcpv6SrvTest, ManySolicits) {
    ...
    // check that IA_NA was returned and that there's an address included
    std::shared_ptr<Option6IAAddr> addr1 = checkIA_NA(reply1, 1, subnet_->getT1(),
                                                subnet_->getT2());
    std::shared_ptr<Option6IAAddr> addr2 = checkIA_NA(reply2, 2, subnet_->getT1(),
                                                subnet_->getT2());
    std::shared_ptr<Option6IAAddr> addr3 = checkIA_NA(reply3, 3, subnet_->getT1(),
                                                subnet_->getT2());
    ASSERT_TRUE(addr1);
    ASSERT_TRUE(addr2);
    ASSERT_TRUE(addr3);
...
TEST_F(Dhcpv6SrvTest, RequestBasic) {
    ...
    // check that IA_NA was returned and that there's an address included
    std::shared_ptr<Option6IAAddr> addr = checkIA_NA(reply, 234,
                                                       subnet_->getT1(),
                                                       subnet_->getT2());
    ASSERT_TRUE(addr);
...
TEST_F(Dhcpv6SrvTest, pdRequestBasic) {
    ...
    req->setIface("eth0");
    req->setIndex(ETH0_INDEX);
    std::shared_ptr<Option6IA> ia = generateIA(D6O_IA_PD, 234, 1500, 3000);
    ...
    // check that IA_NA was returned and that there's an address included
    std::shared_ptr<Option6IAPrefix> prf = checkIA_PD(reply, 234,
                                                        subnet_->getT1(),
                                                        subnet_->getT2());
    ASSERT_TRUE(prf);
...
// Test checks if pkt6-confirm-received is bumped up correctly.
TEST_F(Dhcpv6SrvTest, receiveConfirmStat) {
    testReceiveStats(DHCPV6_CONFIRM, "pkt6-confirm-received");
}
