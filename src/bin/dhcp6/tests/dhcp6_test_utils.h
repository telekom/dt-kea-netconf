...
public:
    ...
    std::shared_ptr<isc::dhcp::Option6IA> generateIA
                          ...
                          uint32_t expected_t1, uint32_t expected_t2)
    {
        ...
        std::shared_ptr<isc::dhcp::Option6IA> ia =
            std::dynamic_pointer_cast<isc::dhcp::Option6IA>(option_ia_na);
        ASSERT_TRUE(ia);
...
public:
    ...
    std::shared_ptr<isc::dhcp::Option6IAAddr>
    ...
    std::shared_ptr<isc::dhcp::Option6IAPrefix>
    ...
    /// expected value (zero value means that do not check).
    void checkIAAddr(const std::shared_ptr<isc::dhcp::Option6IAAddr>& addr,
    ...
    isc::dhcp::Lease6Ptr checkLease
        (const isc::dhcp::DuidPtr& duid, const isc::dhcp::OptionPtr& ia_na,
         std::shared_ptr<isc::dhcp::Option6IAAddr> addr);
    ...
    isc::dhcp::Lease6Ptr checkPdLease
      (const isc::dhcp::DuidPtr& duid, const isc::dhcp::OptionPtr& ia_pd,
       std::shared_ptr<isc::dhcp::Option6IAPrefix> prefix);
...
inline void
disableIfacesReDetect(isc::data::ElementPtr json) {
    isc::data::ElementPtr ifaces_cfg = json->get("interfaces-config");
    if (ifaces_cfg) {
        ifaces_cfg->set("re-detect", isc::data::Element::create(false));
    }
}
