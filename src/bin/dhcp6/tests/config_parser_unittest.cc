...
public:
    ...
    void checkResult(ElementPtr status, int expected_code) {
        ASSERT_TRUE(status);
    ...
    void configure(std::string config, int expected_code,
        ...
        int rcode;
        ElementPtr comment = parseAnswer(rcode, status);
    ...
    void testInvalidOptionParam(const std::string& param_value,
        ...
        ElementPtr json = parseDHCP6(config);
    ...
    ElementPtr comment_; ///< Comment (see @ref isc::config::parseAnswer)
...
TEST_F(Dhcp6ParserTest, interfaceGlobal) {
    ...
    ElementPtr json = parseJSON(config);
...
TEST_F(Dhcp6ParserTest, pdPoolBasics) {
    ...
    // Returned value must be non-empty ElementPtr to config result.
    ...
    Pool6Ptr p6;
    ASSERT_NO_THROW(p6 = std::dynamic_pointer_cast<Pool6>(pc[0]));
    ASSERT_TRUE(p6);
...
TEST_F(Dhcp6ParserTest, pdPoolList) {
    ...
    for (unsigned int i = 0; i < 3; i++) {
        Pool6Ptr p6;
        ASSERT_NO_THROW(p6 = std::dynamic_pointer_cast<Pool6>(pc[i]));
        ASSERT_TRUE(p6);
...
TEST_F(Dhcp6ParserTest, optionDataMultiplePools) {
    ...
    ASSERT_TRUE(pool);
    Pool6Ptr pool6 = std::dynamic_pointer_cast<Pool6>(pool);
    ASSERT_TRUE(pool6);
    ...
    ASSERT_TRUE(pool);
    pool6 = std::dynamic_pointer_cast<Pool6>(pool);
    ASSERT_TRUE(pool6);
...
TEST_F(Dhcp6ParserTest, stdOptionData) {
    ...
    std::shared_ptr<Option6IA> optionIA =
        std::dynamic_pointer_cast<Option6IA>(option);
...
TEST_F(Dhcp6ParserTest, rdnssOption) {
    ...
    std::string config = createConfigWithOption(params);
    ElementPtr json = parseDHCP6(config, true);
    ...
    std::shared_ptr<OptionCustom> optionCustom =
...
TEST_F(Dhcp6ParserTest, DISABLED_stdOptionDataEncapsulate) {
    ...
    // object of this type.
    std::shared_ptr<OptionInt<uint32_t> > option_foo_uint32 =
        std::dynamic_pointer_cast<OptionInt<uint32_t> >(option_foo);
    ...
    OptionCustomPtr option_foo2_v4 =
        std::dynamic_pointer_cast<OptionCustom>(option_foo2);
...
TEST_F(Dhcp6ParserTest, poolUserContextEmpty) {
    ...
    ASSERT_TRUE(pool);
    ElementPtr ctx = pool->getContext();
    ASSERT_TRUE(ctx);
...
TEST_F(Dhcp6ParserTest, poolUserContextlw4over6) {
    ...
    ElementPtr ratio   = ctx->get("lw4over6-sharing-ratio");
    ElementPtr v4pool  = ctx->get("lw4over6-v4-pool");
    ElementPtr exclude = ctx->get("lw4over6-sysports-exclude");
    ElementPtr v6len   = ctx->get("lw4over6-bind-prefix-len");
...
TEST_F(Dhcp6ParserTest, comments) {
    ...
    ElementPtr ctx = CfgMgr::instance().getStagingCfg()->getContext();
    ASSERT_TRUE(ctx);
    ...
    ElementPtr ctx_duid = duid->getContext();
    ...
    ElementPtr ctx_iface = iface->getContext();
    ...
    ElementPtr ctx_opt_def = opt_def->getContext();
    ...
    ElementPtr ctx_opt_desc = opt_desc.getContext();
    ...
    ElementPtr ctx_class = cclass->getContext();
    ...
    ElementPtr ctx_socket = socket->get("user-context");
    ...
    ElementPtr ctx_net = net->getContext();
    ASSERT_TRUE(ctx_net);
    ...
    ElementPtr ctx_sub = sub->getContext();
    ASSERT_TRUE(ctx_sub);
    ...
    ElementPtr ctx_pool = pool->getContext();
    ...
    ElementPtr ctx_pdpool = pdpool->getContext();
    ...
    ElementPtr ctx_host = host->getContext();
    ...
    ElementPtr ctx_host_desc = host_desc.getContext();
    ...
    ElementPtr ctx_d2 = d2->getContext();
    ASSERT_TRUE(ctx_d2);
...
TEST_F(Dhcp6ParserTest, dhcpQueueControl) {
    ...
    // Let's check the default. It should be empty.
    data::ElementPtr staged_control;
    ...
    for (auto scenario : scenarios) {
        SCOPED_TRACE(scenario.description_);
        {
            ...
            if (scenario.json_.empty()) {
                ...
                try {
                    exp_control = Element::fromJSON(scenario.json_);
                } catch (const std::exception& ex) {
...
TEST_F(Dhcp6ParserTest, multiThreadingDefaultSettings) {
    ...
    ElementPtr cfg = CfgMgr::instance().getStagingCfg()->getDHCPMultiThreading();
    ASSERT_TRUE(cfg);
