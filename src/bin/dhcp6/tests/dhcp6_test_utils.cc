...
Dhcpv6SrvTest::checkIA_NA(const Pkt6Ptr& rsp, uint32_t expected_iaid,
    ...
    if (!tmp) {
        ...
        return (std::shared_ptr<Option6IAAddr>());
    }

    std::shared_ptr<Option6IA> ia = std::dynamic_pointer_cast<Option6IA>(tmp);
    if (!ia) {
    ...
    std::shared_ptr<Option6IAAddr> addr = std::dynamic_pointer_cast<Option6IAAddr>(tmp);
    return (addr);
}
...
Dhcpv6SrvTest::checkIA_PD(const Pkt6Ptr& rsp, uint32_t expected_iaid,
    ...
    if (!tmp) {
        ...
        return (std::shared_ptr<Option6IAPrefix>());
    }
    ...
    std::shared_ptr<Option6IAPrefix> addr = std::dynamic_pointer_cast<Option6IAPrefix>(tmp);
    return (addr);
}
...
Dhcpv6SrvTest::checkLease(const DuidPtr& duid, const OptionPtr& ia_na,
                          std::shared_ptr<Option6IAAddr> addr) {
    std::shared_ptr<Option6IA> ia = std::dynamic_pointer_cast<Option6IA>(ia_na);
...
Dhcpv6SrvTest::checkPdLease(const DuidPtr& duid, const OptionPtr& ia_pd,
                            std::shared_ptr<Option6IAPrefix> prefix){
    std::shared_ptr<Option6IA> ia = std::dynamic_pointer_cast<Option6IA>(ia_pd);
...
Dhcpv6SrvTest::testRenewBasic(Lease::Type type,
    ...
    case Lease::TYPE_PD: {
        // Check that IA_NA was returned and that there's an address included
        std::shared_ptr<Option6IAPrefix> prefix_opt
...
Dhcpv6SrvTest::testReleaseReject(Lease::Type type, const IOAddress& addr) {
    ...
    // Check that IA_?? was returned and that there's proper status code
    ia = std::dynamic_pointer_cast<Option6IA>(tmp);
    ASSERT_TRUE(ia);
    checkIA_NAStatusCode(ia, STATUS_NoBinding, 0, 0);
    checkMsgStatusCode(reply, STATUS_NoBinding);
    ...
    ia = std::dynamic_pointer_cast<Option6IA>(rel->getOption(code));
...
Dhcpv6SrvTest::configure(const std::string& config, NakedDhcpv6Srv& srv) {
    ...
    ASSERT_TRUE(status);
    int rcode;
    ElementPtr comment = isc::config::parseAnswer(rcode, status);
...
NakedDhcpv6SrvTest::checkIA_NAStatusCode(
    const std::shared_ptr<isc::dhcp::Option6IA>& ia,
    ...
    bool check_addr)
{
    ...
    if (check_addr) {
        ...
        for (isc::dhcp::OptionCollection::iterator opt = options.begin();
            ...
            dhcp::Option6IAAddrPtr addr =
                std::dynamic_pointer_cast<isc::dhcp::Option6IAAddr>(opt->second);
            ASSERT_TRUE(addr);
    ...
    isc::dhcp::Option6StatusCodePtr status =
        std::dynamic_pointer_cast<isc::dhcp::Option6StatusCode>
