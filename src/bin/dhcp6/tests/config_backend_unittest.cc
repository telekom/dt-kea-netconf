...
public:
    ...
    std::unique_ptr<Dhcpv6Srv> srv_;  ///< DHCP6 server under test
...
TEST_F(Dhcp6CBTest, mergeOptions) {
    ...
    OptionStringPtr opstr = std::dynamic_pointer_cast<OptionString>(found_opt.option_);
    ASSERT_TRUE(opstr);
    ...
    ASSERT_TRUE(found_opt.option_);
    OptionUint32Ptr opint = std::dynamic_pointer_cast<OptionUint32>(found_opt.option_);
    ASSERT_TRUE(opint);
