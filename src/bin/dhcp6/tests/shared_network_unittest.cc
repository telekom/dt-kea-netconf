...
public:
    ...
    int64_t getStatsAssignedLeases(const std::string& stat_name) const {
        ...
        ElementPtr top_element = StatsMgr::instance().get(stat_name);
        if (top_element && (top_element->getType() == Element::map)) {
            // It contains two lists (nested).
            ElementPtr first_list = top_element->get(stat_name);
...
TEST_F(Dhcpv6SharedNetworkTest, variousFieldsInReservation) {
    ...
    Option6ClientFqdnPtr fqdn = std::dynamic_pointer_cast<Option6ClientFqdn>(opt_fqdn);
    ASSERT_TRUE(fqdn);
