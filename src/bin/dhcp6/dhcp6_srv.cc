...
createStatusCode(const Pkt6& pkt, const uint16_t status_code,
    ...
    if (status_code != STATUS_Success) {
        DuidPtr const& duid(pkt.getClientId());
        std::string duid_string;
        if (duid) {
            duid_string = duid->toText(false);
        }
        NotificationsManager n;
        n.notify("non-success-code-sent",
                 isc::data::Element::fromJSON("{ \"duid\": \"" + duid_string +
...
Pkt6Ptr Dhcpv6Srv::receivePacket(int timeout) {
    return (IfaceMgr::instance().receive6(timeout, 0));
}
...
Dhcpv6Srv::initContext(const Pkt6Ptr& pkt,
    ...
    if (!ctx.hwaddr_) {
        NotificationsManager n;
        n.notify("invalid-client-detected",
                 Element::fromJSON("{ \"duid\": \"" + ctx.duid_->toText(false) + "\" }"));
    }
...
void Dhcpv6Srv::run_one() {
    ...
    if (!network_state_->isServiceEnabled()) {
        ...
        if (MultiThreadingMgr::instance().getMode()) {
            ...
            std::shared_ptr<CallBack> call_back =
                std::make_shared<CallBack>(std::bind(&Dhcpv6Srv::processPacketAndSendResponseNoThrow,
                                                       this, query));
...
Dhcpv6Srv::processPacketAndSendResponse(Pkt6Ptr& query) {
    Pkt6Ptr rsp;
#ifdef ENABLE_DEBUG
    LOG_DEBUG(packet6_logger, isc::log::DBGLVL_TRACE_DETAIL_DATA,
              "[thread %1] processPacketAndSendResponse() started...")
    ...
    if (cpu_percentage) {
        StatsMgr::instance().setValue("pkt6-cpu-usage-per-packet", cpu_percentage.value());
    }
    if (memory_usage) {
        StatsMgr::instance().setValue("pkt6-memory-usage-per-packet", memory_usage.value());
    }

    processTimeStats(query,
    ...
    LOG_DEBUG(packet6_logger, isc::log::DBGLVL_TRACE_DETAIL_DATA,
        "[thread %1] processPacketAndSendResponse() ended after %2 ms...")
        .arg(std::this_thread::get_id())
...
Dhcpv6Srv::processDhcp6Query(Pkt6Ptr& query, Pkt6Ptr& rsp) {
    ...
    std::vector<uint8_t> lock_key(query->getClientId()->getDuid());
    uint32_t txid(query->getTransid());
    for (int i = 0; i < sizeof(txid); i++) {
        lock_key.push_back(*((reinterpret_cast<uint8_t *>(&txid)) + i));
    }
    PacketLock<DHCP_SPACE_V6> l(lock_key);
    if (!l.locked_) {
    ...
    try {
        if (ShardConfigMgrFactory<DHCP_SPACE_V6>::initialized()) {
            ShardConfigPtr database_configuration =
                ShardConfigMgrFactory<DHCP_SPACE_V6>::instance().selectTimestamp();
            if (database_configuration && database_configuration->timestamp_ !=
        ...
        if (MasterConfigMgrFactory<DHCP_SPACE_V6>::initialized()) {
            std::string server_id = CfgMgr::instance().getCurrentCfg()->getInstanceID();
            MasterConfigPtr master_database_configuration =
                MasterConfigMgrFactory<DHCP_SPACE_V6>::instance().selectTimestamp(server_id);
            if (master_database_configuration && master_database_configuration->timestamp_ !=
                    CfgMgr::instance().getCurrentCfg()->getMasterCfgTimestamp()) {
    ...
    if (packet_park) {
        ...
        [this, callout_handle, query, rsp]() mutable {
            if (MultiThreadingMgr::instance().getMode()) {
                typedef function<void()> CallBack;
                std::shared_ptr<CallBack> call_back = std::make_shared<CallBack>(
                    std::bind(&Dhcpv6Srv::sendResponseNoThrow, this, callout_handle, query, rsp));
...
void Dhcpv6Srv::sendResponseNoThrow(hooks::CalloutHandlePtr& callout_handle,
                                    Pkt6Ptr& query,
                                    Pkt6Ptr& rsp) {
    try {
        processPacketPktSend(callout_handle, query, rsp);
...
Dhcpv6Srv::appendRequestedOptions(const Pkt6Ptr& question, Pkt6Ptr& answer,
    ...
    std::shared_ptr<OptionIntArray<uint16_t> > option_oro =
        std::dynamic_pointer_cast<OptionIntArray<uint16_t> >
...
Dhcpv6Srv::appendRequestedVendorOptions(const Pkt6Ptr& question,
    ...
    std::shared_ptr<OptionVendor> vendor_req =
        std::dynamic_pointer_cast<OptionVendor>(question->getOption(D6O_VENDOR_OPTS));
    if (vendor_req) {
    ...
    std::shared_ptr<OptionVendor> vendor_rsp =
        std::dynamic_pointer_cast<OptionVendor>(answer->getOption(D6O_VENDOR_OPTS));
    if (vendor_rsp) {
    ...
    std::shared_ptr<OptionUint16Array> oro;
    if (vendor_req) {
        OptionPtr oro_generic = vendor_req->getOption(DOCSIS3_V6_ORO);
        if (oro_generic) {
            oro = std::dynamic_pointer_cast<OptionUint16Array>(oro_generic);
            if (oro) {
...
Dhcpv6Srv::processClientFqdn(const Pkt6Ptr& question, const Pkt6Ptr& answer,
    ...
    Option6ClientFqdnPtr fqdn = std::dynamic_pointer_cast<
...
Dhcpv6Srv::createNameChangeRequests(const Pkt6Ptr& answer,
    ...
    Option6ClientFqdnPtr opt_fqdn = std::dynamic_pointer_cast<
    ...
    for (auto answer_ia : answer->getOptions(D6O_IA_NA)) {
        ...
        Option6IAAddrPtr iaaddr = std::static_pointer_cast<
...
Dhcpv6Srv::assignIA_NA(const Pkt6Ptr& query, const Pkt6Ptr& answer,
    ...
    Option6IAAddrPtr hint_opt =
        std::dynamic_pointer_cast<Option6IAAddr>(ia->getOption(D6O_IAADDR));
    ...
    if (!subnet) {
        ...
        std::shared_ptr<Option6IA> ia_rsp(new Option6IA(D6O_IA_NA, ia->getIAID()));
...
Dhcpv6Srv::assignIA_PD(const Pkt6Ptr& query, const Pkt6Ptr& /*answer*/,
    ...
    Option6IAPrefixPtr hint_opt =
        std::dynamic_pointer_cast<Option6IAPrefix>(ia->getOption(D6O_IAPREFIX));
    IOAddress hint = IOAddress::IPV6_ZERO_ADDRESS();
    if (hint_opt) {
        hint = hint_opt->getAddress();
    }
    ...
    // Do not use OptionDefinition to create option's instance so
    // as we can initialize IAID using a constructor.
    std::shared_ptr<Option6IA> ia_rsp(new Option6IA(D6O_IA_PD, ia->getIAID()));
    ...
    if (!leases.empty()) {
        ...
        for (Lease6Collection::iterator l = leases.begin();
            ...
            std::shared_ptr<Option6IAPrefix>
            ...
            if (pd_exclude_requested) {
                ...
                Pool6Ptr pool = std::dynamic_pointer_cast<
...
Dhcpv6Srv::extendIA_NA(const Pkt6Ptr& query, const Pkt6Ptr& answer,
    ...
    for (OptionCollection::const_iterator it = addrs.begin();
        ...
        Option6IAAddrPtr iaaddr = std::dynamic_pointer_cast<Option6IAAddr>(it->second);
        if (!iaaddr) {
...
Dhcpv6Srv::extendIA_PD(const Pkt6Ptr& query,
    ...
    for (OptionCollection::const_iterator it = addrs.begin();
        ...
        Option6IAPrefixPtr prf = std::dynamic_pointer_cast<Option6IAPrefix>(it->second);
        if (!prf) {
            // That's weird. Option code was ok, but the object type was not.
            // This should never happen. The only case would be with badly
            // mis-implemented hook libraries that insert invalid option objects.
    ...
    for (OptionCollection::iterator opt = release->options_.begin();
        ...
        case D6O_IA_NA: {
            OptionPtr answer_opt = releaseIA_NA(ctx.duid_, release, general_status,
                                                std::dynamic_pointer_cast<Option6IA>(opt->second),
                                                old_lease);
            if (answer_opt) {
                reply->addOption(answer_opt);
            }
...
Dhcpv6Srv::releaseIA_NA(const DuidPtr& duid, const Pkt6Ptr& query,
                        int& general_status, std::shared_ptr<Option6IA> ia,
                        Lease6Ptr& old_lease) {
    ...
    Option6IAAddrPtr release_addr = std::dynamic_pointer_cast<Option6IAAddr>
...
Dhcpv6Srv::releaseIA_PD(const DuidPtr& duid, const Pkt6Ptr& query,
    ...
    std::shared_ptr<Option6IAPrefix> release_prefix =
        std::dynamic_pointer_cast<Option6IAPrefix>(ia->getOption(D6O_IAPREFIX));
    if (!release_prefix) {
        ia_rsp->addOption(createStatusCode(*query, *ia_rsp, STATUS_NoBinding,
    ...
    for (OptionCollection::const_iterator ia = ias.begin();
        ...
        for (OptionCollection::const_iterator opt = opts.begin();
            ...
            if (opt->second->getType() == D6O_IAADDR) {
                ...
                Option6IAAddrPtr iaaddr = std::dynamic_pointer_cast<
...
Dhcpv6Srv::processDecline(AllocEngine::ClientContext6& ctx) {
    ...
    if (declineLeases(decline, reply, ctx)) {
        ...
        // status to DROP. We'll just do as requested.
        return (Pkt6Ptr());
    }
}
...
Dhcpv6Srv::declineIA(const Pkt6Ptr& decline, const DuidPtr& duid,
    ...
    for (OptionCollection::const_iterator opt = opts.begin(); opt != opts.end();
        ...
        Option6IAAddrPtr decline_addr = std::dynamic_pointer_cast<Option6IAAddr>
            (opt->second);
        if (!decline_addr) {
            continue;
        }

        IOAddress const& address(decline_addr->getAddress());

        NotificationsManager n;
        n.notify("decline-received",
                 isc::data::Element::fromJSON("{ \"duid\": \"" +
                         duid->toText(false) + "\", \"declined-address\": \"" + address.toText() + "\" }"));
...
void
Dhcpv6Srv::setStatusCode(std::shared_ptr<isc::dhcp::Option6IA>& container,
...
Dhcpv6Srv::declineLease(const Pkt6Ptr& decline, const Lease6Ptr lease,
                        std::shared_ptr<Option6IA> ia_rsp) {
    ...
    Lease6Ptr old_values = std::make_shared<Lease6>(*lease);
...
void Dhcpv6Srv::classifyByVendor(const Pkt6Ptr& pkt, std::string& classes) {
    OptionVendorClassPtr vclass = std::dynamic_pointer_cast<
...
Dhcpv6Srv::updateReservedFqdn(AllocEngine::ClientContext6& ctx,
    ...
    Option6ClientFqdnPtr fqdn = std::dynamic_pointer_cast<Option6ClientFqdn>
...
void Dhcpv6Srv::processTimeStats(const Pkt6Ptr& query, int64_t processing_time) {
    StatsMgr::instance().setValue("pkt6-processing-time", processing_time);
    string stat_name("pkt6-unknown-processing-time");

    switch (query->getType()) {
    case DHCPV6_SOLICIT:
        stat_name = "pkt6-solicit-processing-time";
        break;
    case DHCPV6_ADVERTISE:
        // Should not happen, but let's keep a counter for it
        stat_name = "pkt6-advertise-processing-time";
        break;
    case DHCPV6_REQUEST:
        stat_name = "pkt6-request-processing-time";
        break;
    case DHCPV6_CONFIRM:
        stat_name = "pkt6-confirm-processing-time";
        break;
    case DHCPV6_RENEW:
        stat_name = "pkt6-renew-processing-time";
        break;
    case DHCPV6_REBIND:
        stat_name = "pkt6-rebind-processing-time";
        break;
    case DHCPV6_REPLY:
        // Should not happen, but let's keep a counter for it
        stat_name = "pkt6-reply-processing-time";
        break;
    case DHCPV6_RELEASE:
        stat_name = "pkt6-release-processing-time";
        break;
    case DHCPV6_DECLINE:
        stat_name = "pkt6-decline-processing-time";
        break;
    case DHCPV6_RECONFIGURE:
        stat_name = "pkt6-reconfigure-processing-time";
        break;
    case DHCPV6_INFORMATION_REQUEST:
        stat_name = "pkt6-infrequest-processing-time";
        break;
    case DHCPV6_DHCPV4_QUERY:
        stat_name = "pkt6-dhcpv4-query-processing-time";
        break;
    case DHCPV6_DHCPV4_RESPONSE:
        // Should not happen, but let's keep a counter for it
        stat_name = "pkt6-dhcpv4-response-processing-time";
        break;
    default:
            ; // do nothing
    }

    StatsMgr::instance().setValue(stat_name, processing_time);
}
...
void Dhcpv6Srv::processStatsSent(const Pkt6Ptr& response) {
    ...
    case DHCPV6_DHCPV4_RESPONSE:
        ...
        break;
    case DHCPV6_RECONFIGURE:
        stat_name = "pkt6-reconfigure-sent";
        break;
    default:
...
Dhcpv6Srv::requestedInORO(const Pkt6Ptr& query, const uint16_t code) const {
    OptionUint16ArrayPtr oro =
        std::dynamic_pointer_cast<OptionUint16Array>(query->getOption(D6O_ORO));
