...
/// "quotes".
struct RSOOListConfigParser : public isc::data::SimpleParser {
    ...
    void parse(const SrvConfigPtr& cfg, const isc::data::ElementPtr& value) {
        try {
...
struct Dhcp6ConfigParser : public isc::data::SimpleParser {
    ...
    void parse(const SrvConfigPtr& srv_config, const ElementPtr& global) {
        ...
        srv_config->setPrivacyHistorySize(privacy_history_size);

        // Set private allocation renew count.
        uint32_t privacy_valid_lifetime = getUint32(global, "privacy-valid-lifetime");
        srv_config->setPrivacyValidLifetime(privacy_valid_lifetime);
#endif  // TERASTREAM
...
configureDhcp6Server(Dhcpv6Srv& server, isc::data::ElementPtr config_set,
    ...
    LOG_DEBUG(dhcp6_logger, DBG_DHCP6_COMMAND,
              DHCP6_CONFIG_START).arg(prettyPrint(config_set));
    ...
    ElementPtr mutable_cfg = config_set;
    ...
    try {
        ...
        srv_config->moveDdnsParams(config_set);

        // Set all default values if not specified by the user.
        SimpleParser6::setAllDefaults(config_set);
        ...
        ElementPtr data_directory = mutable_cfg->get("data-directory");
        if (data_directory) {
        ...
        ElementPtr mac_sources = mutable_cfg->get("mac-sources");
        if (mac_sources) {
        ...
        ElementPtr server_id = mutable_cfg->get("server-id");
        if (server_id) {
        ...
        ElementPtr subnet6 = mutable_cfg->get("subnet6");
        if (subnet6) {
        ...
        ElementPtr rsoo_list = mutable_cfg->get("relay-supplied-options");
        if (rsoo_list) {
        ...
        ElementPtr enabled = mutable_cfg->get("enabled");
        if (enabled) {
            if (enabled->getType() != Element::boolean) {
                isc_throw(BadValue, "unsupported enabled Element type: expected boolean");
            }
            if (enabled->boolValue()) {
                server.getNetworkState()->enableService();
            } else {
                server.getNetworkState()->disableService();
            }
            mutable_cfg->remove("enabled");
        } else if (!server.getNetworkState()->isServiceEnabled()) {
            server.getNetworkState()->enableService();
        }
        ...
        BOOST_FOREACH(config_pair, values_map) {
            ...
            if (config_pair.first == "notifications") {
                ControlSocketParser parser;
                parser.parseForNotifications(*srv_config, config_pair.second);
                continue;
            }
        ...
        global_parser.parse(srv_config, config_set);
        ...
        global_parser.sanityChecks(srv_config, config_set);
        ...
        answer = isc::config::createAnswer(CONTROL_RESULT_ERROR,
                                           "undefined configuration processing error");
    ...
    ElementPtr const& notifications_socket(srv_config->getNotificationsSocketInfo());
    if (notifications_socket) {
        ...
        if (on_boot && on_boot->boolValue() && IfaceMgr::instance().isServerMode() &&
            ...
            std::thread(
                exceptionLoggedBind(  //
                    dhcp6_logger, DHCP6_ERROR, PRETTY_FUNCTION_NAME(),
                    [=] {
                        ...
                        while (!response) {
                            ...
                            try {
                                ...
                                // Retry.
                                LOG_WARN(dhcp6_logger, DHCP6_WARNING).arg(exception.what());
                            }
                        }

                        LOG_INFO(dhcp6_logger, DHCP6_INFO).arg("request-configuration successful");
                    }))
                .detach();
        }
    }
...
isc::data::ElementPtr
configureDhcp6ServerId(isc::data::ElementPtr config_set) {
    ElementPtr answer;

    // Check if an 'instance-id' parameter is provided.
    ElementPtr server_id = config_set->get("instance-id");
    if (server_id) {
        std::string id;
...
isc::data::ElementPtr
configureDhcp6ServerConfigSource(isc::data::ElementPtr config_set) {
    ElementPtr answer;

    SrvConfigPtr srv_config = CfgMgr::instance().getStagingCfg();
    SrvConfigType& configuration_type(srv_config->getConfigurationType());

    // check if a 'configuration-type' parameter is provided
    ElementPtr cfg_type_elem = config_set->get("configuration-type");
    if (cfg_type_elem) {
        std::string type;
        if (cfg_type_elem->getValue(type)) {
            ...
            if (type == "database") {
                ...
                ElementPtr config_database = config_set->get("master-database");
                if (config_database) {
                    // Update the config manager with the server configuration type
                    configuration_type = SrvConfigType::MASTER_DATABASE;
                    DbAccessParser parser;
                    std::string access_string;
                    parser.parse(access_string, config_database);
                    CfgDbAccessPtr cfg_db_access = srv_config->getCfgDbAccess();
                    cfg_db_access->setMasterCfgDbAccessString(access_string);
                } else {
                    // There has not been specified a "master-database" parameter.
                    // Check if a "config-database" parameter has been specified
                    ElementPtr config_database = config_set->get("config-database");
                    if (config_database) {
                        // Update the config manager with the server configuration type
                        configuration_type = SrvConfigType::CONFIG_DATABASE;
                        DbAccessParser parser;
                        std::string access_string;
                        parser.parse(access_string, config_database);
                        CfgDbAccessPtr cfg_db_access = srv_config->getCfgDbAccess();
                ...
                // Update the config manager with the server configuration type
                configuration_type = SrvConfigType::FILE;
            } else {
                // Update the config manager with the server configuration type
                configuration_type = SrvConfigType::UNKNOWN;
