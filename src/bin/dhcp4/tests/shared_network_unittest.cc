...
public:
    ...
    void
    doDORA(Dhcp4Client& client, std::string exp_addr, std::string hint = string(),
           bool ack = true) {

        if (hint.empty()) {
            ASSERT_NO_THROW(client.doDORA());
        } else {
            std::shared_ptr<IOAddress> addr(new IOAddress(hint));
    ...
    int64_t getStatsAssignedAddresses(const SubnetID& subnet_id) const {
        ...
        ElementPtr top_element = StatsMgr::instance().get(stats_name);
        if (top_element && (top_element->getType() == Element::map)) {
            // It contains two lists (nested).
            ElementPtr first_list = top_element->get(stats_name);
            if (first_list && (first_list->getType() == Element::list) &&
                ...
                ElementPtr second_list = first_list->get(0);
                if (second_list && (second_list->getType() == Element::list)) {
                    ElementPtr addresses_element = second_list->get(0);
...
TEST_F(Dhcpv4SharedNetworkTest, parse) {
    ...
    ASSERT_TRUE(status);
    int rcode;
    ElementPtr comment = config::parseAnswer(rcode, status);
    ...
    ElementPtr context = network->getContext();
    ASSERT_TRUE(context);
...
TEST_F(Dhcpv4SharedNetworkTest, hintWithinSharedNetwork) {
    ...
    testAssigned([&client] {
        ASSERT_NO_THROW(client.doDiscover(std::shared_ptr<IOAddress>(new IOAddress("10.0.0.23"))));
    });
...
TEST_F(Dhcpv4SharedNetworkTest, variousFieldsInReservation) {
    ...
    hostname = std::dynamic_pointer_cast<OptionString>(resp->getOption(DHO_HOST_NAME));
