...
void Dhcpv4SrvTest::checkAddressParams(const Pkt4Ptr& rsp,
    ...
    if (!opt) {
        ...
        EXPECT_EQ(opt->getValue(), subnet->getValid().get());
    }
    ...
    if (t1_present) {
        ASSERT_TRUE(opt) << "Required T1 option missing or it has"
            " an unexpected type";
        EXPECT_EQ(opt->getValue(), subnet->getT1().get());
    } else {
        EXPECT_FALSE(opt);
    }
    ...
    if (t2_present) {
        ASSERT_TRUE(opt) << "Required T2 option missing or it has"
            " an unexpected type";
        EXPECT_EQ(opt->getValue(), subnet->getT2().get());
    } else {
        EXPECT_FALSE(opt);
    }
}
...
Dhcpv4SrvTest::testDiscoverRequest(const uint8_t msg_type) {
    ...
    std::unique_ptr<NakedDhcpv4Srv> srv(new NakedDhcpv4Srv(0));
