...
TEST_F(DORATest, selectingRequestAddress) {
    ...
    ASSERT_NO_THROW(client.doDORA(std::shared_ptr<
...
TEST_F(DORATest, ciaddr) {
    ...
    // Obtain a lease from the server using the 4-way exchange.
    ASSERT_NO_THROW(client.doDiscover(std::shared_ptr<
                                      IOAddress>(new IOAddress("10.0.0.50"))));
    // Make sure that the server responded.
    ASSERT_TRUE(client.getContext().response_);
    Pkt4Ptr resp = client.getContext().response_;
...
TEST_F(DORATest, reservation) {
    ...
    // address.
    ASSERT_NO_THROW(clientA.doDORA(std::shared_ptr<
    ...
    // the dynamic pool.
    ASSERT_NO_THROW(clientB.doDORA(std::shared_ptr<
                                  IOAddress>(new IOAddress("0.0.0.0"))));
    // Make sure that the server responded.
...
TEST_F(DORATest, changingCircuitId) {
    ...
    // Send DHCPDISCOVER.
    std::shared_ptr<IOAddress> requested_address(new IOAddress("10.0.0.9"));
...
TEST_F(DORATest, storeExtendedInfoEnabled) {
    ...
    // Client A performs 4-way exchange and should obtain a reserved
    // address.
    ASSERT_NO_THROW_LOG(client.doDORA(std::shared_ptr<
                                      IOAddress>(new IOAddress("0.0.0.0"))));
    // Make sure that the server responded.
    ASSERT_TRUE(client.getContext().response_);
    Pkt4Ptr resp = client.getContext().response_;
