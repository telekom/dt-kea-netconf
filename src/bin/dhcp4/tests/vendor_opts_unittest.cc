...
TEST_F(VendorOptsTest, vendorOptionsDocsis) {
    ...
    std::shared_ptr<OptionVendor> vendor_opt =
        std::dynamic_pointer_cast<OptionVendor>(vendor_opt_response);
    ...
    Option4AddrLstPtr tftp_servers =
        std::dynamic_pointer_cast<Option4AddrLst>(tftp_servers_generic);
...
TEST_F(VendorOptsTest, docsisVendorOptionsParse) {
    ...
    std::shared_ptr<OptionVendor> vendor = std::dynamic_pointer_cast<OptionVendor>(opt);
    ASSERT_TRUE(vendor);
...
TEST_F(VendorOptsTest, docsisVendorORO) {
    ...
    OptionUint8ArrayPtr oro = std::dynamic_pointer_cast<OptionUint8Array>(opt);
    EXPECT_TRUE(oro);
}
...
TEST_F(VendorOptsTest, vendorOptionsORO) {
    ...
    std::shared_ptr<Pkt4> dis(new Pkt4(DHCPDISCOVER, 1234));
    ...
    std::shared_ptr<OptionUint8Array> vendor_oro(new OptionUint8Array(Option::V4,
                                                                        DOCSIS3_V4_ORO));
    ...
    std::shared_ptr<OptionVendor> vendor_resp =
        std::dynamic_pointer_cast<OptionVendor>(tmp);
    ASSERT_TRUE(vendor_resp);

    OptionPtr docsis2 = vendor_resp->getOption(DOCSIS3_V4_TFTP_SERVERS);
    ASSERT_TRUE(docsis2);

    Option4AddrLstPtr tftp_srvs = std::dynamic_pointer_cast<Option4AddrLst>(docsis2);
...
TEST_F(VendorOptsTest, vivsoInResponseOnly) {
    ...
    OptionVendorPtr rsp_vivso = std::dynamic_pointer_cast<OptionVendor>(rsp);
...
TEST_F(VendorOptsTest, option43BadRaw) {
    ...
    OptionCustomPtr custom = std::dynamic_pointer_cast<OptionCustom>(vopt);
    EXPECT_TRUE(custom);
...
TEST_F(VendorOptsTest, option43Class) {
    ...
    ASSERT_TRUE(sopt);
    EXPECT_EQ(1, sopt->getType());
    OptionUint32Ptr sopt32 = std::dynamic_pointer_cast<OptionUint32>(sopt);
    ASSERT_TRUE(sopt32);
...
TEST_F(VendorOptsTest, option43ClassPriority) {
    ...
    OptionPtr opt = offer->getOption(DHO_VENDOR_CLASS_IDENTIFIER);
    EXPECT_TRUE(opt);
    OptionStringPtr id = std::dynamic_pointer_cast<OptionString>(opt);
    ASSERT_TRUE(id);
...
TEST_F(VendorOptsTest, vendorOpsSubOption0) {
    ...
    OptionStringPtr sopt0 =
        std::dynamic_pointer_cast<OptionString>(sopt);
    ASSERT_TRUE(sopt0);
