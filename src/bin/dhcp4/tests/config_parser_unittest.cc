...
public:
    ...
    void testInvalidOptionParam(const std::string& param_value,
        ...
        ElementPtr json = parseDHCP4(config);
    ...
    std::unique_ptr<Dhcpv4Srv> srv_;  ///< DHCP4 server under test
    int rcode_;                         ///< Return code from element parsing
    ElementPtr comment_;           ///< Reason for parse fail
...
TEST_F(Dhcp4ParserTest, nextServerGlobal) {
    ...
    subnet->setFetchGlobalsFn([]() -> ElementPtr {
...
TEST_F(Dhcp4ParserTest, matchClientIdNoGlobal) {
    ...
    ASSERT_TRUE(subnet1);
    // Reset the fetch global function to staging (vs current) config.
    subnet1->setFetchGlobalsFn([]() -> ElementPtr {
        return (CfgMgr::instance().getStagingCfg()->getConfiguredGlobals());
    });
    ...
    ASSERT_TRUE(subnet2);
    // Reset the fetch global function to staging (vs current) config.
    subnet2->setFetchGlobalsFn([]() -> ElementPtr {
        return (CfgMgr::instance().getStagingCfg()->getConfiguredGlobals());
    });
...
TEST_F(Dhcp4ParserTest, optionDataSinglePool) {
    ...
    ASSERT_TRUE(pool);
    Pool4Ptr pool4 = std::dynamic_pointer_cast<Pool4>(pool);
    ASSERT_TRUE(pool4);
...
TEST_F(Dhcp4ParserTest, optionDataMultiplePools) {
    ...
    ASSERT_TRUE(pool1);
    Pool4Ptr pool41 = std::dynamic_pointer_cast<Pool4>(pool1);
    ASSERT_TRUE(pool41);
    ...
    ASSERT_TRUE(pool2);
    Pool4Ptr pool42 = std::dynamic_pointer_cast<Pool4>(pool2);
    ASSERT_TRUE(pool42);
...
TEST_F(Dhcp4ParserTest, stdOptionData) {
    ...
    std::shared_ptr<Option4AddrLst> option_addrs =
        std::dynamic_pointer_cast<Option4AddrLst>(option);
...
TEST_F(Dhcp4ParserTest, poolUserContextData) {
    ...
    ElementPtr int_param  = ctx->get("integer-param");
    ElementPtr str_param  = ctx->get("string-param");
    ElementPtr bool_param = ctx->get("bool-param");
