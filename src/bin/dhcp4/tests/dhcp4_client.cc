...
Dhcp4Client::Dhcp4Client(const Dhcp4Client::State& state) :
    ...
    srv_(std::shared_ptr<NakedDhcpv4Srv>(new NakedDhcpv4Srv(0))),
    state_(state),
    use_relay_(false),
    circuit_id_() {
}

Dhcp4Client::Dhcp4Client(std::shared_ptr<NakedDhcpv4Srv> srv,
...
Dhcp4Client::applyConfiguration() {
    ...
    OptionVendorPtr opt_vendor = std::dynamic_pointer_cast<
...
void
Dhcp4Client::doDiscover(const std::shared_ptr<IOAddress>& requested_addr) {
...
void
Dhcp4Client::doDORA(const std::shared_ptr<IOAddress>& requested_addr) {
