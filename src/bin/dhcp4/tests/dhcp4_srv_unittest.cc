...
TEST_F(Dhcpv4SrvTest, adjustIfaceDataRelay) {
    ...
    std::shared_ptr<Pkt4> req(new Pkt4(DHCPDISCOVER, 1234));
...
TEST_F(Dhcpv4SrvTest, initResponse) {
    ...
    OptionCustomPtr resp_custom =
        std::dynamic_pointer_cast<OptionCustom>(resp_sbnsel);
...
TEST_F(Dhcpv4SrvTest, appendServerID) {
    ...
    Option4AddrLstPtr opt_server_id =
        std::dynamic_pointer_cast<Option4AddrLst>(opt);
...
TEST_F(Dhcpv4SrvTest, basic) {
    ...
    std::unique_ptr<NakedDhcpv4Srv> naked_srv;
    ASSERT_NO_THROW(
...
TEST_F(Dhcpv4SrvTest, DiscoverBasic) {
    ...
    std::unique_ptr<NakedDhcpv4Srv> srv;
    ...
    for (auto test = tests.begin(); test != tests.end(); ++test) {
        {
            ...
            // Check T1 timer
            OptionUint32Ptr opt = std::dynamic_pointer_cast
...
TEST_F(Dhcpv4SrvTest, RenewBasic) {
    ...
    Lease4Ptr used(std::make_shared<Lease4>(IOAddress("192.0.2.106"), hwaddr2,
    ...
    Lease4Ptr lease(std::make_shared<Lease4>());
    lease->addr_ = addr;
...
void prepare(struct ctx& c) {
    ...
    c.used.reset(new Lease4(c.addr, c.hwaddr, &c.test->client_id_->getDuid()[0],
                            c.test->client_id_->getDuid().size(), c.temp_valid, c.temp_timestamp,
...
TEST_F(Dhcpv4SrvTest, privateOption) {
    ...
    OptionUint32Ptr opt32 = std::dynamic_pointer_cast<OptionUint32>(opt2);
    EXPECT_TRUE(opt32);
    ...
    opt32 = std::dynamic_pointer_cast<OptionUint32>(opt);
    ASSERT_TRUE(opt32);
