...
#ifdef TERASTREAM
    OptionCustomPtr saddrOption = std::dynamic_pointer_cast<
...
Dhcpv4Exchange::initResponse4o6() {
    Pkt4o6Ptr query = std::dynamic_pointer_cast<Pkt4o6>(getQuery());
    if (!query) {
        return;
    }
...
void Dhcpv4Exchange::classifyByVendor(const Pkt4Ptr& pkt) {
    ...
    std::shared_ptr<OptionString> vendor_class =
        std::dynamic_pointer_cast<OptionString>(pkt->getOption(DHO_VENDOR_CLASS_IDENTIFIER));
...
Dhcpv4Srv::selectSubnet4o6(const Pkt4Ptr& query, bool& drop,
    ...
    Pkt4o6Ptr query4o6 = std::dynamic_pointer_cast<Pkt4o6>(query);
    if (!query4o6) {
    ...
    if (sbnsel) {
        OptionCustomPtr oc = std::dynamic_pointer_cast<OptionCustom>(sbnsel);
        if (oc) {
...
Dhcpv4Srv::receivePacket(int timeout) {
    return (IfaceMgr::instance().receive4(timeout, 0));
}
    ...
    if (!network_state_->isServiceEnabled()) {
        ...
        if (MultiThreadingMgr::instance().getMode()) {
            ...
            std::shared_ptr<CallBack> call_back =
                std::make_shared<CallBack>(std::bind(&Dhcpv4Srv::processPacketAndSendResponseNoThrow,
                                                       this, query));
...
Dhcpv4Srv::processPacketAndSendResponse(Pkt4Ptr& query) {
    Pkt4Ptr rsp;
#ifdef ENABLE_DEBUG
    LOG_DEBUG(packet4_logger, isc::log::DBGLVL_TRACE_DETAIL_DATA,
    ...
    if (cpu_percentage) {
        StatsMgr::instance().setValue("pkt4-cpu-usage-per-packet", cpu_percentage.value());
    }
    if (memory_usage) {
        StatsMgr::instance().setValue("pkt4-memory-usage-per-packet", memory_usage.value());
    }
...
Dhcpv4Srv::processDhcp4Query(Pkt4Ptr& query, Pkt4Ptr& rsp,
    ...
    std::vector<uint8_t> lock_key(query->getHWAddr()->hwaddr_);
    ...
    PacketLock<DHCP_SPACE_V4> l(lock_key);
    if (!l.locked_) {
        LOG_DEBUG(dhcp4_logger, DBG_DHCP4_BASIC, DHCP4_PACKET_PROCESS_FAIL)
            .arg(query->getName())
            .arg(query->getRemoteAddr().toText())
            .arg("lock was taken");
        return;
    ...
    try {
        if (ShardConfigMgrFactory<DHCP_SPACE_V4>::initialized()) {
            ShardConfigPtr const& database_configuration(ShardConfigMgrFactory<DHCP_SPACE_V4>::instance().selectTimestamp());
        ...
        if (MasterConfigMgrFactory<DHCP_SPACE_V4>::initialized()) {
            std::string server_id = CfgMgr::instance().getCurrentCfg()->getInstanceID();
            MasterConfigPtr const& master_database_configuration(MasterConfigMgrFactory<DHCP_SPACE_V4>::instance().selectTimestamp(server_id));
        ...
        [this, callout_handle, query, rsp]() mutable {
            if (MultiThreadingMgr::instance().getMode()) {
                ...
                std::shared_ptr<CallBack> call_back = std::make_shared<CallBack>(
                    std::bind(&Dhcpv4Srv::sendResponseNoThrow, this, callout_handle, query, rsp));
...
void Dhcpv4Srv::sendResponseNoThrow(hooks::CalloutHandlePtr& callout_handle,
                                    Pkt4Ptr& query,
                                    Pkt4Ptr& rsp) {
    try {
        processPacketPktSend(callout_handle, query, rsp);
        ...
        LOG_ERROR(packet4_logger, DHCP4_PACKET_PROCESS_STD_EXCEPTION).arg(e.what());
    } catch (...) {
        LOG_ERROR(packet4_logger, DHCP4_PACKET_PROCESS_EXCEPTION);
    }
}
...
void Dhcpv4Srv::updateDhcpV4AddressPlusPortPacket(Pkt4Ptr& query) {
    OptionUint8ArrayPtr option_prl = std::dynamic_pointer_cast<OptionUint8Array>
    ...
    if (option_prl) {
        ...
        if (std::find(requested_opts.begin(), requested_opts.end(),
            ...
            if (portparams) {
                OptionCustomPtr oc = std::dynamic_pointer_cast<OptionCustom>(portparams);
                if (oc) {
...
Dhcpv4Srv::srvidToString(const OptionPtr& srvid) {
    ...
    std::shared_ptr<Option4AddrLst> generated =
        std::dynamic_pointer_cast<Option4AddrLst>(srvid);
    if (!srvid) {
...
Dhcpv4Srv::appendRequestedOptions(Dhcpv4Exchange& ex) {
    ...
    OptionUint8ArrayPtr option_prl = std::dynamic_pointer_cast<
...
Dhcpv4Srv::appendRequestedVendorOptions(Dhcpv4Exchange& ex) {
    ...
    std::shared_ptr<OptionVendor> vendor_req = std::dynamic_pointer_cast<
    ...
    std::shared_ptr<OptionVendor> vendor_rsp = std::dynamic_pointer_cast<
    ...
    if (vendor_req) {
        oro = std::dynamic_pointer_cast<OptionUint8Array>(vendor_req->getOption(DOCSIS3_V4_ORO));
    }
...
Dhcpv4Srv::processClientName(Dhcpv4Exchange& ex) {
    ...
    try {
        Pkt4Ptr resp = ex.getResponse();
        Option4ClientFqdnPtr fqdn = std::dynamic_pointer_cast<Option4ClientFqdn>
...
Dhcpv4Srv::processClientFqdnOption(Dhcpv4Exchange& ex) {
    ...
    Option4ClientFqdnPtr fqdn = std::dynamic_pointer_cast<
...
Dhcpv4Srv::processHostnameOption(Dhcpv4Exchange& ex) {
    ...
    OptionStringPtr opt_hostname = std::dynamic_pointer_cast<OptionString>
    ...
    if (ctx->currentHost() && !ctx->currentHost()->getHostname().empty()) {
        ...
        if (!should_send_hostname) {
            OptionUint8ArrayPtr
                option_prl = std::dynamic_pointer_cast<OptionUint8Array>
...
Dhcpv4Srv::assignLease(Dhcpv4Exchange& ex) {
    ...
    if (fqdn) {
        ...
        opt_hostname = std::dynamic_pointer_cast<OptionString>
...
Dhcpv4Srv::processDecline(Pkt4Ptr& decline, AllocEngine::ClientContext4Ptr& context) {
    ...
    n.notify("decline-received",
             isc::data::Element::fromJSON("{ \"address\": \"" + addr.toText() + "\" }"));
...
Dhcpv4Srv::acceptServerId(const Pkt4Ptr& query) const {
    ...
    OptionCustomPtr option_custom =
        std::dynamic_pointer_cast<OptionCustom>(option);
    ...
    OptionCustomPtr opt_server_id = std::dynamic_pointer_cast<OptionCustom>
...
void Dhcpv4Srv::processTimeStats(const Pkt4Ptr& query, int64_t processing_time) {
    StatsMgr::instance().setValue("pkt4-processing-time", processing_time);
    string stat_name("pkt4-unknown-processing-time");

    switch (query->getType()) {
    case DHCPACK:
        stat_name = "pkt4-ack-processing-time";
        break;
    case DHCPDECLINE:
        stat_name = "pkt4-decline-processing-time";
        break;
    case DHCPDISCOVER:
        stat_name = "pkt4-discover-processing-time";
        break;
    case DHCPINFORM:
        stat_name = "pkt4-inform-processing-time";
        break;
    case DHCPNAK:
        stat_name = "pkt4-nak-processing-time";
        break;
    case DHCPOFFER:
        stat_name = "pkt4-offer-processing-time";
        break;
    case DHCPRELEASE:
        stat_name = "pkt4-release-processing-time";
        break;
    case DHCPREQUEST:
        stat_name = "pkt4-request-processing-time";
        break;
    default:
            ; // do nothing
    }
