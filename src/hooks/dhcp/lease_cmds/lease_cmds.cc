...
public:
    ...
    Parameters getParameters(bool v6, const ElementPtr& args);
    ...
    IOAddress getAddressParam(ElementPtr params, const std::string name,
...
LeaseCmdsImpl::getParameters(bool v6, const ElementPtr& params) {
    Parameters x;
    ...
    if (params->contains("update-ddns")) {
        ElementPtr tmp = params->get("update-ddns");
    ...
    ElementPtr tmp = params->get("ip-address");
    if (tmp) {
    ...
    ElementPtr type = params->get("identifier-type");
    ElementPtr ident = params->get("identifier");
...
LeaseCmdsImpl::leaseGetHandler(CalloutHandle& handle) {
    ...
    if (v4 && lease4) {
        ...
        ElementPtr response = createAnswer(CONTROL_RESULT_SUCCESS,
...
LeaseCmdsImpl::leaseGetAllHandler(CalloutHandle& handle) {
    ...
    try {
        ...
        // be returned.
        if (cmd_args_) {
            ElementPtr subnets = cmd_args_->get("subnets");
            if (!subnets) {
...
LeaseCmdsImpl::leaseGetPageHandler(CalloutHandle& handle) {
    ...
    try {
        ...
        ElementPtr from = cmd_args_->get("from");
        if (!from) {
        ...
        std::unique_ptr<IOAddress> from_address;
        try {
        ...
        ElementPtr page_limit = cmd_args_->get("limit");
        if (!page_limit) {
...
LeaseCmdsImpl::leaseGetByHwAddressHandler(CalloutHandle& handle) {
    try {
        ...
        ElementPtr hw_address = cmd_args_->get("hw-address");
        if (!hw_address) {
...
LeaseCmdsImpl::leaseGetByClientIdHandler(CalloutHandle& handle) {
    try {
        ...
        ElementPtr client_id = cmd_args_->get("client-id");
        if (!client_id) {
...
LeaseCmdsImpl::leaseGetByDuidHandler(CalloutHandle& handle) {
    try {
        ...
        ElementPtr duid = cmd_args_->get("duid");
        if (!duid) {
...
LeaseCmdsImpl::leaseGetByHostnameHandler(CalloutHandle& handle) {
    ...
    try {
        ...
        ElementPtr hostname = cmd_args_->get("hostname");
        if (!hostname) {
...
LeaseCmdsImpl::lease4WipeHandler(CalloutHandle& handle) {
    try {
        ...
        ElementPtr response = createAnswer(num ? CONTROL_RESULT_SUCCESS
...
IOAddress
LeaseCmdsImpl::getAddressParam(ElementPtr params, const std::string name,
                               short family) const {
    ElementPtr param = params->get(name);
    if (!param) {
