...
public:
    ...
    void addLib(const std::string& lib, ElementPtr params) {
        libraries_.push_back({lib, "parameters", params});
    }
    ...
    void checkCommandRegistered(const std::string& name, bool expect_true) {
        ...
        ElementPtr lst = Element::fromJSON("{ \"command\": \"list-commands\" }");
        ElementPtr rsp = CommandMgr::instance().processCommand(lst);

        ASSERT_TRUE(rsp);

        ElementPtr args = rsp->get("arguments");
        ASSERT_TRUE(args);
    ...
    ElementPtr testCommand(string cmd_txt, int exp_result, string exp_txt) {
        ...
        ElementPtr rsp = CommandMgr::instance().processCommand(cmd);
    ...
    void checkAnswer(isc::data::ElementPtr answer,
                     int exp_status,
                     string exp_txt = "") {
        int rcode = 0;
        isc::data::ElementPtr comment;
...
public:
    ...
    void checkLease4(ElementPtr l, std::string ip,
        ...
        if (l->getType() == Element::list) {
            ...
            for (auto it = e.begin(); it != e.end(); ++it) {
                ElementPtr ip_address = (*it)->get("ip-address");
    ...
    void checkLease6(ElementPtr l, std::string ip,
                     uint8_t prefixlen,
    ...
    void checkFailedLease(const ElementPtr& failed_leases_list,
...
TEST_F(LeaseCmdsTest, Lease4GetByAddrNotFound) {
    ...
    ElementPtr rsp = testCommand(cmd, CONTROL_RESULT_EMPTY, exp_rsp);
}
...
TEST_F(LeaseCmdsTest, Lease4GetByAddr) {
    ...
    ElementPtr rsp = testCommand(cmd, CONTROL_RESULT_SUCCESS, exp_rsp);

    // Now check that the lease parameters were indeed returned.
    ASSERT_TRUE(rsp);
    ElementPtr lease = rsp->get("arguments");
    ASSERT_TRUE(lease);
...
TEST_F(LeaseCmdsTest, Lease6GetByClientIdInvalidType) {
    ...
    ElementPtr rsp = testCommand(cmd, CONTROL_RESULT_ERROR, exp_rsp);
}
...
TEST_F(LeaseCmdsTest, Lease4GetAll) {
    ...
    ElementPtr leases = args->get("leases");
    ASSERT_TRUE(leases);
...
TEST_F(LeaseCmdsTest, Lease4GetPaged) {
    ...
    for (auto i = 0; i < 3; ++i) {
        ...
        ElementPtr page_count = args->get("count");
        ...
        if (!leases->empty()) {
            ...
            for (ElementPtr lease : leases->listValue()) {
                ASSERT_EQ(Element::map, lease->getType());
                ASSERT_TRUE(lease->contains("ip-address"));
                ElementPtr ip_address = lease->get("ip-address");
                ASSERT_EQ(Element::string, ip_address->getType());
...
TEST_F(LeaseCmdsTest, LeaseGetByHwAddressFind2) {
    ...
    // Now check that the lease parameters were indeed returned.
    ASSERT_TRUE(rsp);
    ElementPtr map = rsp->get("arguments");
    ASSERT_TRUE(map);
    ASSERT_EQ(Element::map, map->getType());
    ElementPtr leases = map->get("leases");
    ASSERT_TRUE(leases);
    ASSERT_EQ(Element::list, leases->getType());
    ASSERT_EQ(2, leases->size());

    // Let's check if the response makes any sense.
    ElementPtr lease = leases->get(0);
    ASSERT_TRUE(lease);
    checkLease4(lease, "192.0.2.1", 44, "08:08:08:08:08:08", false);
    lease = leases->get(1);
    ASSERT_TRUE(lease);
