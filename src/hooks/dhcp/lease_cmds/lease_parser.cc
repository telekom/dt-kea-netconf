...
Lease4Parser::parse(ConstSrvConfigPtr& cfg,
    ...
    // Handle user context.
    ElementPtr ctx = lease_info->get("user-context");
    ...
    // Handle comment.
    ElementPtr comment = lease_info->get("comment");
    if (comment) {
...
Lease6Parser::parse(ConstSrvConfigPtr& cfg,
    ...
    Pool6Ptr pool = std::dynamic_pointer_cast<
        Pool6>(subnet->getPool(l->type_, l->addr_, false));
    l->pool_id_ = pool ? pool->getId() : -1;
