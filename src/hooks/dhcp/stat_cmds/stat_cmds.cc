...
public:
    ...
    Parameters getParameters(const ElementPtr& cmd_args);
...
LeaseStatCmdsImpl::getParameters(const ElementPtr& cmd_args) {
    Parameters params;
    ...
    if (cmd_args->contains("subnet-id")) {
        ...
        ElementPtr value = cmd_args->get("subnet-id");
    ...
    if (cmd_args->contains("subnet-range")) {
        ...
        ElementPtr range = cmd_args->get("subnet-range");
        if (range->getType() != Element::map) {
            isc_throw(BadValue, "subnet-range parameter is not a map");
        }

        ElementPtr value = range->get("first-subnet-id");
