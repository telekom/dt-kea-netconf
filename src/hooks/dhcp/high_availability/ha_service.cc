...
HAService::asyncSendLeaseUpdate(const QueryPtrType& query,
                          ...
                          const ElementPtr& command,
                          const ParkingLotHandlePtr& parking_lot) {
    // Create HTTP/1.1 request including our command.
    PostHttpRequestJsonPtr request = std::make_shared<PostHttpRequestJson>
    ...
    HttpResponseJsonPtr response = std::make_shared<HttpResponseJson>();
    ...
    std::weak_ptr<typename QueryPtrType::element_type> weak_query(query);
...
HAService::logFailedLeaseUpdates(const PktPtr& query,
    ...
    auto log_proc = [](const PktPtr query, const ElementPtr& args,
        ...
        [this, partner_config]
            ...
            if (ec || !error_str.empty()) {
                ...
                try {
                    ...
                    // be a map.
                    int rcode = 0;
                    ElementPtr args = verifyAsyncResponse(response, rcode);
                    if (!args || args->getType() != Element::map) {
...
HAService::asyncSyncLeasesInternal(http::HttpClient& http_client,
    ...
    if (server_type_ == HAServerType::DHCPv4) {
        request->setBodyAsJson(CommandCreator::createLease4GetPage(
            std::dynamic_pointer_cast<Lease4>(last_lease), config_->getSyncPageLimit()));

    } else {
        request->setBodyAsJson(CommandCreator::createLease6GetPage(
            std::dynamic_pointer_cast<Lease6>(last_lease), config_->getSyncPageLimit()));
    }
    request->finalize();
                        ...
                        try {
                            ...
                            if (server_type_ == HAServerType::DHCPv4) {
                                ...
                                if ((leases_element.size() >= config_->getSyncPageLimit()) &&
                                    ...
                                    last_lease = std::dynamic_pointer_cast<Lease>(lease);
                                }
...
HAService::verifyAsyncResponse(const HttpResponsePtr& response, int& rcode) {
    ...
    HttpResponseJsonPtr json_response =
        std::dynamic_pointer_cast<HttpResponseJson>(response);
    if (!json_response) {
        isc_throw(CtrlChannelError, "no valid HTTP response found");
    }

    // Body holds the response to our command.
    ElementPtr body = json_response->getBodyAsJson();
    if (!body) {
    ...
    ElementPtr args = parseAnswer(rcode, body->get(0));
