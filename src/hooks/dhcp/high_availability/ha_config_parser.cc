...
HAConfigParser::parseInternal(const HAConfigPtr& config_storage,
    ...
    ElementPtr state_machine = c->get("state-machine");
    ElementPtr states_list;
    if (state_machine) {
...
template<typename T>
T HAConfigParser::getAndValidateInteger(const ElementPtr& config,
