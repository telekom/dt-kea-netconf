...
void
CommunicationState::setPartnerScopes(ElementPtr new_scopes) {
...
void
CommunicationState4::analyzeMessage(const std::shared_ptr<dhcp::Pkt>& message) {
    if (MultiThreadingMgr::instance().getMode()) {
        std::lock_guard<std::mutex> lk(*mutex_);
...
void
CommunicationState4::analyzeMessageInternal(const std::shared_ptr<dhcp::Pkt>& message) {
    // The DHCP message must successfully cast to a Pkt4 object.
    Pkt4Ptr msg = std::dynamic_pointer_cast<Pkt4>(message);
    if (!msg) {
...
void
CommunicationState6::analyzeMessage(const std::shared_ptr<dhcp::Pkt>& message) {
    if (MultiThreadingMgr::instance().getMode()) {
        std::lock_guard<std::mutex> lk(*mutex_);
        analyzeMessageInternal(message);
    } else {
...
void
CommunicationState6::analyzeMessageInternal(const std::shared_ptr<dhcp::Pkt>& message) {
    // The DHCP message must successfully cast to a Pkt6 object.
    Pkt6Ptr msg = std::dynamic_pointer_cast<Pkt6>(message);
    if (!msg) {
    ...
    OptionUint16Ptr elapsed_time = std::dynamic_pointer_cast<
