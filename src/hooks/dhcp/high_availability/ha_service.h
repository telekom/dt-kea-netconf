...
protected:
    ...
    void logFailedLeaseUpdates(const dhcp::PktPtr& query,
                               const data::ElementPtr& args) const;
...
public:
    ...
    data::ElementPtr processHeartbeat();
    ...
    data::ElementPtr processStatusGet() const;
...
public:
    ...
    data::ElementPtr processSynchronize(const std::string& server_name,
                                             const unsigned int max_period);
...
public:
    ...
    data::ElementPtr processMaintenanceNotify(const bool cancel);
    ...
    data::ElementPtr processMaintenanceStart();
    ...
    data::ElementPtr processMaintenanceCancel();
...
protected:
    ...
    data::ElementPtr verifyAsyncResponse(const http::HttpResponsePtr& response,
                                              int& rcode);
...
private:
    ...
    std::map<std::shared_ptr<dhcp::Pkt>, int> pending_requests_;
};

/// @brief Pointer to the @c HAService class.
typedef std::shared_ptr<HAService> HAServicePtr;
