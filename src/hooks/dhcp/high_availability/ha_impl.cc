...
void
HAImpl::configure(const ElementPtr& input_config) {
...
HAImpl::startService(const IOServicePtr& io_service,
    ...
    service_ = std::make_shared<HAService>(io_service, network_state,
                                             config_, server_type);
}
...
HAImpl::commandProcessed(hooks::CalloutHandle& callout_handle) {
    ...
    if (command_name == "dhcp-enable") {
        ...
        ElementPtr resp_args = response->get("arguments");
        ...
        resp_args->set("high-availability", ha_relationships);
    }
}

void
HAImpl::heartbeatHandler(CalloutHandle& callout_handle) {
    ElementPtr response = service_->processHeartbeat();
...
HAImpl::synchronizeHandler(hooks::CalloutHandle& callout_handle) {
    ...
    try {
        ...
        ElementPtr max_period = args->get("max-period");
        if (max_period) {
        ...
        ElementPtr response = createAnswer(CONTROL_RESULT_ERROR, ex.what());
        callout_handle.setArgument("response", response);
        return;
    }

    // Command parsing was successful, so let's process the command.
    ElementPtr response = service_->processSynchronize(server_name->stringValue(),
                                                            max_period_value);
    callout_handle.setArgument("response", response);
}
...
HAImpl::scopesHandler(hooks::CalloutHandle& callout_handle) {
    ...
    try {
        ...
        ElementPtr scopes = args->get("scopes");
        if (!scopes) {
    ...
    // Command parsing was successful, so let's process the command.
    ElementPtr response = service_->processScopes(scopes_vector);
    callout_handle.setArgument("response", response);
}

void
HAImpl::continueHandler(hooks::CalloutHandle& callout_handle) {
    ElementPtr response = service_->processContinue();
    callout_handle.setArgument("response", response);
}
...
HAImpl::maintenanceNotifyHandler(hooks::CalloutHandle& callout_handle) {
    ...
    ElementPtr cancel_op = args->get("cancel");
    if (!cancel_op) {
    ...
    ElementPtr response = service_->processMaintenanceNotify(cancel_op->boolValue());
    callout_handle.setArgument("response", response);
}

void
HAImpl::maintenanceStartHandler(hooks::CalloutHandle& callout_handle) {
    ElementPtr response = service_->processMaintenanceStart();
    callout_handle.setArgument("response", response);
}

void
HAImpl::maintenanceCancelHandler(hooks::CalloutHandle& callout_handle) {
    ElementPtr response = service_->processMaintenanceCancel();
    callout_handle.setArgument("response", response);
}
