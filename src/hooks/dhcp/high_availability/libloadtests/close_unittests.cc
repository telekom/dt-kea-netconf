...
TEST_F(CloseHATest, close4) {
    ...
    libraries.push_back({LIBDHCP_HA_SO, "parameters", params});
    ...
    ElementPtr command = createCommand("ha-heartbeat", "dhcp4");
    ElementPtr response;
    ...
    int code = 0;
    ElementPtr arguments = parseAnswer(code, response);
    EXPECT_EQ(0, code);
    ASSERT_TRUE(arguments);
    ASSERT_EQ(Element::map, arguments->getType());
    ElementPtr state = arguments->get("state");
    ASSERT_TRUE(state);
    ...
    std::weak_ptr<Pkt4> weak_query(query);
    query.reset();
...
TEST_F(CloseHATest, close6) {
    ...
    // Pretend to be the parter.
    ElementPtr command = createCommand("ha-heartbeat", "dhcp6");
    ElementPtr response;
    EXPECT_TRUE(HooksManager::commandHandlersPresent("ha-heartbeat"));
    {
        CalloutHandlePtr handle = HooksManager::createCalloutHandle();
    ...
    // Verify the HA owns the query.
    std::weak_ptr<Pkt6> weak_query(query);
    query.reset();
    EXPECT_FALSE(weak_query.expired());
