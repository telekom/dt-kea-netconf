...
ElementPtr getLeasesAsJson(const LeasesVec& leases) {
...
typedef std::shared_ptr<TestHAService> TestHAServicePtr;
...
private:
    ...
    createDynamicHttpResponse(const ConstHttpRequestPtr& request) {
        ...
        if (body && (body->getType() == Element::map)) {
            ElementPtr command = body->get("command");
...
typedef std::shared_ptr<TestHttpResponseCreator> TestHttpResponseCreatorPtr;
...
public:
    ...
    TestHttpResponseCreatorPtr getResponseCreator() const {
        return (std::dynamic_pointer_cast<TestHttpResponseCreator>(creator_));
    }
...
typedef std::shared_ptr<TestHttpResponseCreatorFactory>
...
public:
    ...
    ElementPtr getTestLeases4AsJson(const size_t first_index,
    ...
    /// @param first_index Index of the first lease to be returned.
    /// @param last_index Index of the last lease to be returned.
    ElementPtr getTestLeases6AsJson(const size_t first_index,
                                         const size_t last_index) const {
    ...
    void runProcessSynchronize4(ElementPtr& rsp) {
    ...
    ///
    /// @param [out] rsp pointer to the object where response will be stored.
    void runProcessSynchronize6(ElementPtr& rsp) {
        // Create lease manager.
...
TEST_F(HAServiceTest, hotStandbyScopeSelectionThisPrimary) {
    ...
    ElementPtr ha_servers = service.processStatusGet();
...
TEST_F(HAServiceTest, processHeartbeat) {
    ...
    ElementPtr state = args->get("state");
    ASSERT_TRUE(state);
    ...
    ElementPtr date_time = args->get("date-time");
...
typedef std::shared_ptr<HAPartner> HAPartnerPtr;
...
TEST_F(HAServiceStateMachineTest, waitingParterDownLoadBalancingPartnerDown) {
    ...
    // Check the reported info about servers.
    ElementPtr ha_servers = service_->processStatusGet();
    ASSERT_TRUE(ha_servers);
