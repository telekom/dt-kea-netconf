...
public:
    ...
    void testSynchronizeHandler(const std::string& ha_sync_command,
        ...
        ElementPtr command = Element::fromJSON(ha_sync_command);
...
TEST_F(HAImplTest, buffer4Receive) {
    ...
    ElementPtr ha_config = createValidJsonConfiguration(HAConfig::HOT_STANDBY);
...
TEST_F(HAImplTest, continueHandler) {
    ...
    ElementPtr command = Element::fromJSON("{ \"command\": \"ha-continue\" }");
...
TEST_F(HAImplTest, maintenanceNotify) {
    ...
    ElementPtr command = Element::fromJSON(
        "{"
